"""
Workday RaaS OAuth 2.0 Authentication Handler
GL-VCCI Scope 3 Platform

OAuth 2.0 client credentials flow with token caching and refresh logic.
Reuses token cache pattern from SAP connector for consistency.
...
from .config import OAuth2Config, WorkdayEnvironment
from .exceptions import WorkdayAuthenticationError, WorkdayConnectionError
...
class WorkdayAuthHandler:
    """
    OAuth 2.0 authentication handler for Workday RaaS.
    ...
    def __init__(
        ...
        Args:
            ...
            environment: Workday environment
            cache: Token cache instance (creates new if not provided)
        """
        self.oauth_config = oauth_config
        self.environment = environment
        self.cache = cache or TokenCache()
        self._cache_key = f"workday_token_{environment.value}"

        logger.info(f"Initialized Workday auth handler for environment: {environment.value}")
    ...
    def get_access_token(self, force_refresh: bool = False) -> str:
        ...
        Raises:
            WorkdayAuthenticationError: If token acquisition fails
        """
    ...
    def _acquire_token(self) -> str:
        ...
        try:
            ...
            if self.oauth_config.refresh_token:
                token_data["refresh_token"] = self.oauth_config.refresh_token
    ...
    def validate_token(self, token: str) -> bool:
        ...
        you would need to call Workday's token introspection endpoint.
...
_auth_handlers: Dict[str, WorkdayAuthHandler] = {}
_auth_lock = Lock()
...
def get_auth_handler(
    ...
    with _auth_lock:
        if cache_key not in _auth_handlers:
            _auth_handlers[cache_key] = WorkdayAuthHandler(
