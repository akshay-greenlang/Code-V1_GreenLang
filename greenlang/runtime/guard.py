...
class CapabilityViolation(Exception):
    ...
    def __init__(self, capability: str, operation: str, details: str = "", suggestion: str = ""):
...
class RuntimeGuard:
    ...
    def __init__(self, capabilities: Optional[Dict[str, Any]] = None):
        ...
        self.pack_data_dir = Path(os.environ.get('GL_PACK_DATA_DIR', '/tmp/gl_pack_data'))
        self.run_tmp = Path(os.environ.get('GL_RUN_TMP', '/tmp/gl_run'))
    ...
    def _audit_event(self, capability: str, operation: str, target: str, result: str, details: Optional[Dict] = None):
        """Record audit event"""
        event = {
        ...
        def guarded_socket(*args, **kwargs):
            ...
            def guarded_connect(address):
                ...
                if host in self.blocked_metadata_ips:
                    self._audit_event('net', 'socket.connect', host, 'denied', {'reason': 'metadata_endpoint'})
                    raise CapabilityViolation('net', f'Connecting to {host}',
                                            'Metadata endpoints are blocked')

                # Check against allowlist
                if allowed_domains and not self._match_domain(host, allowed_domains):
                    self._audit_event('net', 'socket.connect', host, 'denied', {'reason': 'not_in_allowlist'})
                    raise CapabilityViolation('net', f'Connecting to {host}',
    ...
    def _patch_filesystem(self):
        ...
        def guarded_open(file, mode='r', *args, **kwargs):
            ...
            self._audit_event('fs', f'open.{"write" if is_write else "read"}', str(path), 'allowed')
        ...
        def guarded_os_open(path, flags, *args, **kwargs):
            ...
            if is_write and not self._check_path_allowed(resolved_path, write_allowlist, 'write'):
                self._audit_event('fs', 'os.open.write', str(resolved_path), 'denied')
                raise CapabilityViolation('fs', f'Writing file: {resolved_path}',
                                        'Path not in write allowlist')

            if not is_write and not self._check_path_allowed(resolved_path, read_allowlist, 'read'):
                self._audit_event('fs', 'os.open.read', str(resolved_path), 'denied')
                raise CapabilityViolation('fs', f'Reading file: {resolved_path}',
                                        'Path not in read allowlist')
        ...
        def guarded_rename(src, dst):
            ...
            if not fs_allowed:
                self._audit_event('fs', 'os.rename', f'{src_path} -> {dst_path}', 'denied')
            ...
            if not self._check_path_allowed(src_path, write_allowlist, 'write'):
                ...
                raise CapabilityViolation('fs', f'Renaming source: {src_path}',
            ...
            if not self._check_path_allowed(dst_path, write_allowlist, 'write'):
                ...
                raise CapabilityViolation('fs', f'Renaming destination: {dst_path}',
    ...
    def _patch_subprocess(self):
        ...
        allowed_binaries = subprocess_config.get('allowlist', []) if subprocess_allowed else []
        ...
        def guarded_popen(args, *pargs, **kwargs):
            ...
            if str(cmd_path) not in allowed_binaries:
                ...
                raise CapabilityViolation('subprocess', f'Executing: {cmd_path}',
        ...
        def guarded_system(command):
            ...
            self._audit_event('subprocess', 'os.system', command, 'denied')
            raise CapabilityViolation('subprocess', 'os.system is not allowed',
...
def initialize_guard():
    ...
    logger.info(f"Runtime guard initialized with capabilities: {list(capabilities.keys())}")
