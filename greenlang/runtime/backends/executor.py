...
from .base import Pipeline, PipelineStep, ExecutionContext, ExecutionResult, ExecutionStatus
from .factory import BackendFactory, get_backend_manager
...
class PipelineExecutor:
    ...
    def execute(self, pipeline: Union[Pipeline, Dict, Path, str],
               context: Optional[ExecutionContext] = None,
               async_execution: bool = False) -> ExecutionResult:
        """
        Execute a pipeline
        ...
        try:
            if async_execution:
                ...
                return asyncio.create_task(
                    self._execute_async(pipeline, context)
                )
            else:
    ...
    def _execute_sync(self, pipeline: Pipeline, context: ExecutionContext) -> ExecutionResult:
    ...
    async def _execute_async(self, pipeline: Pipeline, context: ExecutionContext) -> ExecutionResult:
...
class PipelineBuilder:
    ...
    def add_step(self, name: str, command: List[str], 
                 args: List[str] = None,
...
def execute_pipeline(pipeline: Union[Pipeline, Dict, Path, str],
                    backend: str = "local",
