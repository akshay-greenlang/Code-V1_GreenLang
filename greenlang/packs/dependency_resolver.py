    ...
    def matches(self, version_str: str) -> bool:
        ...
        try:
            ...
            if self.version_spec.startswith("=="):
                ...
                return version.parse(version_str) >= version.parse(self.version_spec[2:].strip())
            return True
...
class DependencyResolver:
    ...
    def __init__(self, registry=None, strategy: ResolutionStrategy = ResolutionStrategy.LATEST):
        """
    ...
    def add_package_to_pool(self, package: PackageInfo) -> None:
        ...
        self.package_pool[package.name].sort(
            key=lambda p: version.parse(p.version),
            reverse=True
        )
                        ...
                        if isinstance(dep, str):
                            ...
                            try:
                                ...
                                if len(parts) == 2:
                                    ...
                                    deps.append(DependencySpec(name=dep, version_spec=""))
    ...
    def resolve(self, requirements: List[DependencySpec],
               locked_versions: Optional[Dict[str, str]] = None,
               include_optional: bool = False,
               include_groups: Optional[List[str]] = None) -> ResolutionResult:
        """
        ...
        while queue:
            ...
            if dep_spec.group and include_groups and dep_spec.group not in include_groups:
                continue
            ...
            if not compatible_version:
                # Record conflict
                self.conflicts.append(ResolutionConflict(
                    package=dep_spec.name,
                    requesters=[(parent, dep_spec.version_spec)] if parent else [],
                    available_versions=[p.version for p in self.package_pool.get(dep_spec.name, [])],
        ...
        if cycles:
            for cycle in cycles:
                self.warnings.append(f"Circular dependency detected: {' -> '.join(cycle)}")
    ...
    def _find_compatible_version(self, dep_spec: DependencySpec,
                                locked_version: Optional[str] = None) -> Optional[PackageInfo]:
    ...
    def _check_conflicts(self) -> None:
        ...
        for node in self.graph.nodes():
            if node in self.resolved:
                ...
                for parent in self.graph.predecessors(node):
                    ...
                    parent_info = self._get_package_info(parent, self.resolved.get(parent))
                    if parent_info:
        ...
        for package, version_constraints in constraints.items():
            ...
            for requester, constraint in version_constraints:
                if constraint and not DependencySpec(package, constraint).matches(resolved_version):
            ...
            if unsatisfied:
                self.conflicts.append(ResolutionConflict(
                    ...
                    available_versions=[p.version for p in self.package_pool.get(package, [])],
...
def resolve_dependencies(requirements: List[Union[str, DependencySpec]],
                        registry=None,
                        strategy: ResolutionStrategy = ResolutionStrategy.LATEST,
                        lock_file: Optional[Path] = None) -> ResolutionResult:
    """
    ...
    for req in requirements:
        if isinstance(req, str):
            ...
            try:
                ...
                dep_specs.append(DependencySpec(
                    name=parsed.name,
