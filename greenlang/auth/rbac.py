...
class Role:
    ...
    def get_all_permissions(self, rbac_manager: 'RBACManager' = None) -> List[Permission]:
        ...
        if rbac_manager and self.parent_roles:
            for parent_role_name in self.parent_roles:
                ...
                if parent_role:
                    all_permissions.extend(parent_role.get_all_permissions(rbac_manager))
...
class RBACManager:
    ...
    def __init__(self):
        ...
        self.resource_policies: Dict[str, List[Dict[str, Any]]] = {}  # Resource-specific policies
        ...
        for role_name in role_names:
            ...
            if role:
                ...
                for permission in all_permissions:
                    if permission.matches(resource, action, context):
                        logger.debug(f"User {user_id} granted {action} on {resource} via role {role_name}")
                        return True
        ...
        if resource in self.resource_policies:
            for policy in self.resource_policies[resource]:
                if self._evaluate_policy(policy, user_id, action, context):
                    logger.debug(f"User {user_id} granted {action} on {resource} via policy")
                    return True
    ...
    def _evaluate_policy(self, policy: Dict[str, Any], user_id: str,
...
class AccessControl:
    ...
    def require_permission(self, resource: str, action: str):
        ...
        def decorator(func):
            def wrapper(*args, **kwargs):
                ...
                # Check permission
                if not self.rbac_manager.check_permission(user_id, resource, action, context):
                    raise PermissionError(f"User {user_id} lacks permission: {resource}:{action}")
    ...
    def filter_resources(self, user_id: str, resources: List[Any],
                        resource_type: str, action: str = "read") -> List[Any]:
        """
