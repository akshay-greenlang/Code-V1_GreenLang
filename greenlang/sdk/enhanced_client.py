...
from greenlang.data.models import (
    ...
    HVACSystem, BuildingEnvelope, ApplianceLoad,
    EmissionResult, BuildingType, Country
)
...
class GreenLangClient:
    ...
    def calculate_emissions(self, 
        ...
        """
        # Get emission factor
        factor_result = self.orchestrator.execute_single_agent("grid_factor", {
    ...
    def analyze_building(self, building_data: Union[Dict, BuildingInput]) -> Dict[str, Any]:
        ...
        profile_result = self.orchestrator.execute_single_agent("building_profile", {
        ...
        for fuel_type, fuel_data in building_data.get("energy_consumption", {}).items():
            ...
            if emission_result["success"]:
                ...
                emissions_by_source[fuel_type] = emission_result["data"]["co2e_emissions_kg"]
        ...
        if "solar_pv_generation" in building_data.get("energy_consumption", {}):
            solar_kwh = building_data["energy_consumption"]["solar_pv_generation"]["value"]
        ...
        carbon_result = self.orchestrator.execute_single_agent("carbon", {
        ...
        intensity_result = self.orchestrator.execute_single_agent("intensity", {
        ...
        # 6. Recommendations
        rec_result = self.orchestrator.execute_single_agent("recommendation", {
            "emissions_by_source": emissions_by_source,
            "intensity": results["data"].get("intensity", {}).get("intensities", {}),
            "building_type": building_data["metadata"]["building_type"],
            "building_age": building_data["metadata"].get("building_age", 10),
            "performance_rating": results["data"].get("intensity", {}).get("performance_rating", "Average"),
            "load_breakdown": results["data"].get("profile", {}).get("load_breakdown", {}),
            "country": building_data["metadata"]["location"]["country"]
        })
    ...
    def get_recommendations(self,
                           ...
                           building_age: int = 10) -> Dict[str, Any]:
        """
        ...
        """
        result = self.orchestrator.execute_single_agent("recommendation", {
            "building_type": building_type,
    ...
    def get_emission_factor(self,
        ...
        """
        result = self.orchestrator.execute_single_agent("grid_factor", {
            "country": country or self.region,
            "fuel_type": fuel_type,
            "unit": unit
        })
    ...
    def calculate_intensity(self,
                           ...
                           floor_count: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        Returns:
            Intensity metrics
        """
        result = self.orchestrator.execute_single_agent("intensity", {
            "total_emissions_kg": total_emissions_kg,
            "area": area,
            "area_unit": "sqft",
            "occupancy": occupancy,
    ...
    def generate_report(self,
        ...
        Returns:
            Formatted report
        """
        result = self.orchestrator.execute_single_agent("report", {
        ...
        for i, building in enumerate(buildings):
            ...
            if result["success"]:
                ...
                if "emissions" in result["data"]:
                    total_emissions += result["data"]["emissions"].get("total_co2e_kg", 0)
    ...
    def export_analysis(self, analysis_results: Dict, filepath: str, format: str = "json"):
        """
        ...
        if format == "json":
            ...
            with pd.ExcelWriter(filepath) as writer:
                ...
                if "intensity" in analysis_results.get("data", {}):
                    intensity_df = pd.DataFrame([analysis_results["data"]["intensity"]["intensities"]])
                    intensity_df.to_excel(writer, sheet_name="Intensity", index=False)
                
                if "recommendations" in analysis_results.get("data", {}):
                    recs = analysis_results["data"]["recommendations"].get("recommendations", [])
                    if recs:
