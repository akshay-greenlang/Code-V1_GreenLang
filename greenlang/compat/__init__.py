...
class CompatibilityImporter:
    ...
    def get_item(self, item_name: str):
        ...
        _track_deprecated_import(f"{self.old_module}.{item_name}",
                               f"{self.new_module}.{item_name}")
...
try:
    ...
    def get_agent_class():
        _track_deprecated_import('greenlang.compat.Agent', 'core.greenlang.sdk.base.Agent')
        return Agent

    def get_pipeline_class():
        _track_deprecated_import('greenlang.compat.Pipeline', 'core.greenlang.sdk.base.Pipeline')
        return Pipeline

    def get_pack_registry():
        _track_deprecated_import('greenlang.compat.PackRegistry', 'core.greenlang.packs.registry.PackRegistry')
        return PackRegistry

    def get_pack_loader():
        _track_deprecated_import('greenlang.compat.PackLoader', 'core.greenlang.packs.loader.PackLoader')
        return PackLoader

except ImportError:
    # Fallback to legacy imports if core structure doesn't exist
    def get_agent_class():
        _track_deprecated_import('greenlang.compat.Agent', 'greenlang.agents.base.BaseAgent')
        from greenlang.agents.base import BaseAgent
        return BaseAgent

    def get_pipeline_class():
        _track_deprecated_import('greenlang.compat.Pipeline', 'greenlang.core.workflow.Workflow')
        from greenlang.core.workflow import Workflow
        return Workflow

    def get_pack_registry():
        _track_deprecated_import('greenlang.compat.PackRegistry', 'greenlang.registry.PackRegistry')
        try:
    ...
    def get_pack_loader():
        _track_deprecated_import('greenlang.compat.PackLoader', 'greenlang.packs.loader.PackLoader')
        try:
...
def get_orchestrator_class():
    ...
    _track_deprecated_import('greenlang.compat.Orchestrator', 'greenlang.core.orchestrator.Orchestrator')
    try:
...
def get_workflow_class():
    ...
    _track_deprecated_import('greenlang.compat.Workflow', 'greenlang.core.workflow.Workflow')
    try:
        from greenlang.core.workflow import Workflow
        return Workflow
    except ImportError:
