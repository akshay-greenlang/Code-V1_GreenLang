...
try:
    ...
    try:
        ...
        _warn_deprecated_test_import('greenlang.compat.testing', 'greenlang.testing.numerics')
    except ImportError:
        # Provide basic implementations if nothing is available
        def assert_close(actual: float, expected: float, rel_tol: float = 1e-09, abs_tol: float = 0.0) -> None:
        ...
        def assert_within_tolerance(actual: float, expected: float, tolerance: float = 0.01) -> None:
            ...
            if abs(actual - expected) > tolerance:
                raise AssertionError(f"Value {actual} not within tolerance {tolerance} of {expected}")
...
def create_test_data(data_type: str = "fuel", **kwargs) -> dict:
    ...
    _warn_deprecated_test_import('greenlang.compat.testing.create_test_data', 'tests.fixtures.data_factory')
...
class TestFixtures:
    ...
    def get_sample_pipeline() -> dict:
        ...
        _warn_deprecated_test_import('greenlang.compat.testing.TestFixtures', 'tests.fixtures.pipelines')
        return {
            "name": "test_pipeline",
            "version": "1.0",
            "steps": [
    ...
    def get_sample_manifest() -> dict:
        ...
        _warn_deprecated_test_import('greenlang.compat.testing.TestFixtures', 'tests.fixtures.manifests')
        return {
            "name": "test-pack",
            "version": "1.0.0",
