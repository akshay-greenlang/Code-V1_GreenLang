...
class ProvenanceContext:
    ...
    def add_artifact(self, name: str, path: Path, artifact_type: str = "file", metadata: Dict = None):
...
def generate_provenance_report(artifact_path: Path) -> Dict[str, Any]:
    ...
    if sig_path.exists():
        ...
        report["signature"] = {
            ...
            "algorithm": sig.get("spec", {}).get("signature", {}).get("algorithm", "unknown"),
    ...
    if not comparison["identical"]:
        ...
        for diff in comparison["differences"]:
            if diff["field"] == "pipeline_hash":
                report["recommendations"].append("Pipeline code has changed between runs")
                ...
                report["recommendations"].append("Outputs differ - check for non-deterministic operations")
    
    return report


def export_provenance_bundle(artifact_path: Path, output_path: Optional[Path] = None) -> Path:
    """
...
def import_provenance_bundle(bundle_path: Path, extract_to: Optional[Path] = None) -> Dict[str, Path]:
    """
    ...
    artifact_files = [f for f in extracted.keys() if not f.endswith(('.json', '.sig', '.pem'))]
    if artifact_files:
