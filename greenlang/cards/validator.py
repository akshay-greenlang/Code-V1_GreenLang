...
class CardValidator:
    ...
    REQUIRED_SECTIONS = {
        "pack": [
            "Overview", "Usage", "Inputs", "Outputs", 
            "License", "Dependencies"
        ],
        "dataset": [
            "Dataset Summary", "Dataset Description", "Dataset Structure",
            "Dataset Creation", "Licensing Information"
        ],
        "model": [
            "Model Details", "Uses", "Training Details",
            "Evaluation", "Environmental Impact", "Citation"
        ],
        "pipeline": [
            "Pipeline Overview", "Inputs and Outputs", "Dependencies",
    ...
    def validate_card(self, 
                     content: str, 
                     card_type: str = "pack") -> CardValidationResult:
        """
            ...
            for section in sections:
                if req_section.lower() in section.lower():
                    ...
                    if len(section_content.strip()) < self.min_section_length:
                        warnings.append(f"Section '{section}' is too short (< {self.min_section_length} chars)")
                    break
        ...
        if card_type in ["model", "pipeline"]:
            if "carbon" not in body.lower() and "emission" not in body.lower():
                suggestions.append("Add environmental impact information (carbon emissions)")
    ...
    def _calculate_score(self, errors, warnings, suggestions, 
                         missing_sections, sections, body) -> float:
...
def validate_card_file(path: Path, card_type: Optional[str] = None) -> CardValidationResult:
    """
