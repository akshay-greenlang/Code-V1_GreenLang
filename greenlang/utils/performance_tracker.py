...
class PerformanceTracker:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        # Memory statistics
        memory_deltas = [m.memory_delta for m in self.metrics if m.memory_delta is not None]
        if memory_deltas:
            summary["memory"] = {
    ...
    def get_memory_intensive_operations(self, 
                                       threshold: Optional[float] = None) -> List[PerformanceMetric]:
        ...
        if format == "json":
            ...
            with open(filename, 'w', newline='') as f:
                writer = csv.DictWriter(
                    f, 
                    fieldnames=["name", "duration", "memory_delta", "timestamp"]
                )
                writer.writeheader()
