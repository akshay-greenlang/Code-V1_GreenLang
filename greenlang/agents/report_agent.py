...
class ReportAgent(BaseAgent):
    ...
    def __init__(self, config: AgentConfig = None):
        if config is None:
            config = AgentConfig(
                name="ReportAgent",
                description="Generates carbon footprint reports"
            )
    ...
    def execute(self, input_data: Dict[str, Any]) -> AgentResult:
        ...
        if report_format == "markdown":
            report = self._generate_markdown_report(carbon_data, building_info, period_info)
    ...
    def _generate_text_report(self, carbon_data: Dict, building_info: Dict, period_info: Dict) -> str:
        report = []
        report.append("=" * 60)
        ...
        if building_info:
            ...
            if "area" in building_info:
                report.append(f"Building Area: {building_info['area']:,.0f} {building_info.get('area_unit', 'sqft')}")
        ...
        if period_info:
            ...
            if "start_date" in period_info and "end_date" in period_info:
                report.append(f"Period: {period_info['start_date']} to {period_info['end_date']}")
            elif "duration" in period_info:
                report.append(f"Duration: {period_info['duration']} {period_info.get('duration_unit', 'month')}(s)")
            report.append("")
        ...
        report.append(f"                 {carbon_data.get('total_co2e_kg', 0):.2f} kg CO2e")
        report.append("")
        
        if "carbon_intensity" in carbon_data:
            intensity = carbon_data["carbon_intensity"]
            if "per_sqft" in intensity:
                report.append(f"Carbon Intensity: {intensity['per_sqft']:.2f} kg CO2e/sqft")
            if "per_person" in intensity:
                report.append(f"                  {intensity['per_person']:.2f} kg CO2e/person")
            report.append("")
    ...
    def _generate_markdown_report(self, carbon_data: Dict, building_info: Dict, period_info: Dict) -> str:
        report = []
        report.append("# Carbon Footprint Report")
        report.append("")
        ...
        if "carbon_intensity" in carbon_data:
            ...
            intensity = carbon_data["carbon_intensity"]
            if "per_sqft" in intensity:
                report.append(f"- **Per Square Foot:** {intensity['per_sqft']:.2f} kg CO2e/sqft")
            if "per_person" in intensity:
                report.append(f"- **Per Person:** {intensity['per_person']:.2f} kg CO2e/person")
            report.append("")
    ...
    def _generate_json_report(self, carbon_data: Dict, building_info: Dict, period_info: Dict) -> Dict:
        return {
