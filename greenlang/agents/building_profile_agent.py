...
from greenlang.data.models import BuildingType, ClimateZone
import logging
...
class BuildingProfileAgent(BaseAgent):
    ...
    def execute(self, input_data: Dict[str, Any]) -> AgentResult:
        ...
        try:
            ...
            profile = self.building_profiles.get(
                building_type, 
                self.building_profiles["commercial_office"]
            )
            ...
            adjusted_eui = base_eui * efficiency_modifier * climate_adjustments.get("hvac_modifier", 1.0)
            
            # Provide load breakdown
            load_breakdown = {}
            total_factor = 0
            for load_type in ["hvac_load_factor", "lighting_load_factor", "plug_load_factor", 
                            "equipment_load_factor", "it_load_factor", "water_heating_factor"]:
                if load_type in profile:
                    load_breakdown[load_type.replace("_factor", "")] = profile[load_type]
            ...
            result_data = {
                ...
                "benchmark_standards": self._get_benchmark_standards(building_type, country)
            }
    ...
    def _get_climate_adjustments(self, climate_zone: Optional[str]) -> Dict:
        ...
        climate_factors = {
            ...
            "continental": {"hvac_modifier": 1.15, "description": "Continental climate"},
            "polar": {"hvac_modifier": 1.4, "description": "Polar climate"}
        }
        
        return climate_factors.get(climate_zone, {"hvac_modifier": 1.0, "description": "Unknown zone"})
    ...
    def _get_benchmark_standards(self, building_type: str, country: str) -> Dict:
        ...
        standards = {
            ...
            "IN": {
                ...
                "certification_levels": ["5 Star", "4 Star", "3 Star", "2 Star", "1 Star"],
