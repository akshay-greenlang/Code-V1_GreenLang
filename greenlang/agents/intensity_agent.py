...
class IntensityAgent(BaseAgent):
    ...
    def execute(self, input_data: Dict[str, Any]) -> AgentResult:
        ...
        try:
            ...
            if area_sqft > 0:
                ...
                intensities["per_sqft_month"] = round(annual_emissions_kg / area_sqft / 12, 4)
            
            # Occupancy-based intensities
            if occupancy > 0:
                intensities["per_person_year"] = round(annual_emissions_kg / occupancy, 1)
                intensities["per_person_day"] = round(annual_emissions_kg / occupancy / operating_days, 3)
            
            # Floor-based intensity
            if floor_count > 0:
                intensities["per_floor_year"] = round(annual_emissions_kg / floor_count, 1)
            
            # Operating hours intensity
            total_operating_hours = operating_hours * operating_days
            if total_operating_hours > 0:
                intensities["per_operating_hour"] = round(annual_emissions_kg / total_operating_hours, 2)
            
            # Economic intensity
            if revenue > 0:
                intensities["per_revenue_dollar"] = round(annual_emissions_kg / revenue, 6)
                intensities["per_million_revenue"] = round(annual_emissions_kg / (revenue / 1000000), 1)
            
            # Production intensity (for industrial)
            if production_units > 0:
                intensities["per_production_unit"] = round(annual_emissions_kg / production_units, 3)
            ...
            if "total_energy_kwh" in input_data:
                ...
                if area_sqft > 0:
                    intensities["energy_use_intensity_kwh_sqft"] = round(total_energy_kwh / area_sqft, 1)
                if total_energy_kwh > 0:
                    intensities["carbon_per_kwh"] = round(total_emissions_kg / total_energy_kwh, 4)
            
            # Determine performance rating based on intensity
            rating = self._determine_rating(intensities.get("per_sqft_year", 0), 
    ...
    def _determine_rating(self, intensity: float, building_type: str) -> str:
        ...
        building_thresholds = thresholds.get(building_type, thresholds["commercial_office"])
    ...
    def _compare_to_benchmark(self, intensity: float, building_type: str, country: str) -> Dict:
        ...
        return {
            ...
            "performance": "Better than benchmark" if difference < 0 else "Worse than benchmark",
