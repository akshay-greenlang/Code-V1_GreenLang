...
if FASTAPI_AVAILABLE:
    ...
    async def add_policy(body: AddPolicyRequest, request: Request) -> Dict[str, Any]:
        ...
        from greenlang.agents.foundation.access_guard.models import Policy, PolicyRule
        service = _get_service(request)
        try:
    ...
    async def update_policy(
        ...
        from greenlang.agents.foundation.access_guard.models import PolicyRule
        service = _get_service(request)
        try:
            rules = None
    ...
    async def classify_resource(
        ...
        from greenlang.agents.foundation.access_guard.models import DataClassification, Resource
        service = _get_service(request)
        try:
            resource = Resource(
    ...
    async def simulate_policies(
        ...
        from greenlang.agents.foundation.access_guard.models import AccessRequest, Principal, Resource
        service = _get_service(request)
        try:
            test_requests = []
