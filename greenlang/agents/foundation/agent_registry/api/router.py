...
if FASTAPI_AVAILABLE:
    ...
    async def register_agent(body: RegisterAgentRequest, request: Request) -> Dict[str, Any]:
        ...
        from greenlang.agents.foundation.agent_registry.models import AgentMetadataEntry
        service = _get_service(request)
        try:
    ...
    async def set_agent_health(
        ...
        from greenlang.agents.foundation.agent_registry.models import AgentHealthStatus
        service = _get_service(request)
        try:
