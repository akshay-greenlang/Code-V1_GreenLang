# -*- coding: utf-8 -*-
"""
API Dependencies - AGENT-EUDR-014 QR Code Generator
...
Dependencies:
    ...
    - get_qrg_service: Returns the QR Code Generator service singleton.
...
Rate Limiter Tiers (5):
    - qr_generate: 50 requests/minute (QR code generation)
    - label_generate: 30 requests/minute (label rendering)
    - verify: 200 requests/minute (verification lookups)
    - bulk: 10 requests/minute (bulk generation jobs)
    - standard: 100 requests/minute (general read operations)

Permission Prefix: eudr-qrg:*
...
class AuthUser(BaseModel):
    ...
    Attributes:
        ...
        permissions: Granted fine-grained permissions (e.g. ``eudr-qrg:qr:generate``).
    """
...
def require_permission(permission: str) -> Callable:
    ...
    Uses wildcard matching: ``eudr-qrg:*`` grants all
    ``eudr-qrg:<action>`` permissions.

    Args:
        permission: Required permission string, e.g.
            ``eudr-qrg:qr:generate``.
    ...
    Example:
        ...
        ...     user: AuthUser = Depends(require_permission("eudr-qrg:qr:generate"))
        ... ):
        ...     ...
    """
    ...
    async def _check_permission(
        ...
        # Check wildcard patterns (eudr-qrg:qr:* matches eudr-qrg:qr:generate)
        parts = permission.split(":")
        for i in range(len(parts)):
            wildcard = ":".join(parts[: i + 1]) + ":*"
            if wildcard in user.permissions:
                return user
        # Check top-level wildcard (eudr-qrg:*)
...
_rate_limit_qr_generate = RateLimiter(max_requests=50, window_seconds=60)
_rate_limit_label_generate = RateLimiter(max_requests=30, window_seconds=60)
_rate_limit_verify = RateLimiter(max_requests=200, window_seconds=60)
_rate_limit_bulk = RateLimiter(max_requests=10, window_seconds=60)
_rate_limit_standard = RateLimiter(max_requests=100, window_seconds=60)
_rate_limit_counterfeit = RateLimiter(max_requests=50, window_seconds=60)
_rate_limit_lifecycle = RateLimiter(max_requests=100, window_seconds=60)
_rate_limit_batch_code = RateLimiter(max_requests=50, window_seconds=60)


# Wrapper functions for dependency injection (overridable in tests)
async def rate_limit_qr_generate(
    request: Request,
    user: AuthUser = Depends(get_current_user),
) -> None:
    """QR generate rate limit: 50 requests/minute for QR code generation."""
    await _rate_limit_qr_generate(request, user)


async def rate_limit_label_generate(
    request: Request,
    user: AuthUser = Depends(get_current_user),
) -> None:
    """Label generate rate limit: 30 requests/minute for label rendering."""
    await _rate_limit_label_generate(request, user)
...
async def rate_limit_verify(
    ...
    """Verify rate limit: 200 requests/minute for verification operations."""
    await _rate_limit_verify(request, user)
...
async def rate_limit_bulk(
    ...
    """Bulk rate limit: 10 requests/minute for bulk generation jobs."""
    await _rate_limit_bulk(request, user)
...
async def rate_limit_counterfeit(
    request: Request,
    user: AuthUser = Depends(get_current_user),
) -> None:
    """Counterfeit rate limit: 50 requests/minute for anti-counterfeiting."""
    await _rate_limit_counterfeit(request, user)


async def rate_limit_lifecycle(
    request: Request,
    user: AuthUser = Depends(get_current_user),
) -> None:
    """Lifecycle rate limit: 100 requests/minute for lifecycle operations."""
    await _rate_limit_lifecycle(request, user)


async def rate_limit_batch_code(
    request: Request,
    user: AuthUser = Depends(get_current_user),
) -> None:
    """Batch code rate limit: 50 requests/minute for batch code operations."""
    await _rate_limit_batch_code(request, user)
...
def validate_code_id(
    code_id: str = Path(..., description="QR code identifier"),
) -> str:
    """Validate code_id path parameter."""
    return validate_uuid_path(code_id, "code_id")


def validate_payload_id(
    payload_id: str = Path(..., description="Payload identifier"),
) -> str:
    """Validate payload_id path parameter."""
    return validate_uuid_path(payload_id, "payload_id")


def validate_label_id(
    label_id: str = Path(..., description="Label identifier"),
) -> str:
    """Validate label_id path parameter."""
    return validate_uuid_path(label_id, "label_id")


def validate_batch_code(
    code: str = Path(..., description="Batch code value"),
) -> str:
    """Validate batch code path parameter."""
    return validate_uuid_path(code, "code")


def validate_job_id(
    job_id: str = Path(..., description="Bulk job identifier"),
) -> str:
    """Validate job_id path parameter."""
    return validate_uuid_path(job_id, "job_id")


def validate_format_path(
    format: str = Path(..., description="Image format (png, svg, pdf, zpl, eps)"),
) -> str:
    """Validate output format path parameter.

    Args:
        format: Image output format.

    Returns:
        Lowercased, validated format string.

    Raises:
        HTTPException: 400 if format is not supported.
    """
    normalized = format.strip().lower()
    valid_formats = {"png", "svg", "pdf", "zpl", "eps"}
    if normalized not in valid_formats:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Unsupported format '{format}'. Must be one of: {sorted(valid_formats)}",
        )
    return normalized
...
class _QRGServiceStub:
    """Stub service for QR Code Generator operations.
    ...
    def __init__(self) -> None:
        ...
        logger.info("QR Code Generator service stub initialized")
...
_qrg_service_instance: Optional[Any] = None


def get_qrg_service() -> Any:
    """Return the QR Code Generator service singleton.
    ...
    Returns:
        QR Code Generator service instance (real or stub).
    """
    global _qrg_service_instance

    if _qrg_service_instance is not None:
        return _qrg_service_instance
    ...
    try:
        ...
        config = get_config()
        _qrg_service_instance = {
            "config": config,
            "initialized": True,
        }
        logger.info("QR Code Generator service engines initialized")
    except Exception as exc:
        logger.warning(
            "Could not initialize QRG service engines, using stub: %s",
            exc,
        )
        _qrg_service_instance = _QRGServiceStub()

    return _qrg_service_instance


def reset_qrg_service() -> None:
    """Reset the service singleton. Used in testing."""
    global _qrg_service_instance
    _qrg_service_instance = None
