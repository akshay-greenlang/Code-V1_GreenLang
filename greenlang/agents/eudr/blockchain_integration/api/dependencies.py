# -*- coding: utf-8 -*-
"""
API Dependencies - AGENT-EUDR-013 Blockchain Integration
...
Dependencies:
    ...
    - get_blockchain_service: Returns the BlockchainIntegration service singleton.
...
Rate Limiter Tiers (5):
    - anchor: 50 requests/minute (anchoring operations)
    - contract: 10 requests/minute (contract deployment/calls)
    - verify: 100 requests/minute (verification lookups)
    - event: 30 requests/minute (event subscriptions/queries)
    - evidence: 20 requests/minute (evidence package generation)

Permission Prefix: eudr-bci:*
...
class AuthUser(BaseModel):
    ...
    Attributes:
        ...
        permissions: Granted fine-grained permissions (e.g. ``eudr-bci:anchors:create``).
    """
...
def require_permission(permission: str) -> Callable:
    ...
    Uses wildcard matching: ``eudr-bci:*`` grants all
    ``eudr-bci:<action>`` permissions.
    ...
    Example:
        ...
        ...     user: AuthUser = Depends(require_permission("eudr-bci:anchors:create"))
        ... ):
        ...     ...
    """
    ...
    async def _check_permission(
        ...
        # Check wildcard patterns (eudr-bci:anchors:* matches eudr-bci:anchors:create)
        parts = permission.split(":")
        for i in range(len(parts)):
            wildcard = ":".join(parts[: i + 1]) + ":*"
            if wildcard in user.permissions:
                return user
        # Check top-level wildcard (eudr-bci:*)
...
_rate_limit_anchor = RateLimiter(max_requests=50, window_seconds=60)
_rate_limit_contract = RateLimiter(max_requests=10, window_seconds=60)
_rate_limit_verify = RateLimiter(max_requests=100, window_seconds=60)
_rate_limit_event = RateLimiter(max_requests=30, window_seconds=60)
...
async def rate_limit_anchor(
    request: Request,
    user: AuthUser = Depends(get_current_user),
) -> None:
    """Anchor rate limit: 50 requests/minute for anchoring operations."""
    await _rate_limit_anchor(request, user)


async def rate_limit_contract(
    request: Request,
    user: AuthUser = Depends(get_current_user),
) -> None:
    """Contract rate limit: 10 requests/minute for contract operations."""
    await _rate_limit_contract(request, user)
...
async def rate_limit_verify(
    ...
    """Verify rate limit: 100 requests/minute for verification operations."""
...
async def rate_limit_event(
    ...
    """Event rate limit: 30 requests/minute for event operations."""
    await _rate_limit_event(request, user)
...
async def rate_limit_evidence(
    ...
    """Evidence rate limit: 20 requests/minute for evidence operations."""
...
async def rate_limit_batch(
    ...
    """Batch rate limit: 20 requests/minute for batch operations."""
...
def validate_anchor_id(
    anchor_id: str = Path(..., description="Anchor identifier"),
) -> str:
    """Validate anchor_id path parameter."""
    return validate_uuid_path(anchor_id, "anchor_id")


def validate_contract_id(
    contract_id: str = Path(..., description="Contract identifier"),
) -> str:
    """Validate contract_id path parameter."""
    return validate_uuid_path(contract_id, "contract_id")


def validate_chain_id(
    chain_id: str = Path(..., description="Chain connection identifier"),
) -> str:
    """Validate chain_id path parameter."""
    return validate_uuid_path(chain_id, "chain_id")


def validate_tree_id(
    tree_id: str = Path(..., description="Merkle tree identifier"),
) -> str:
    """Validate tree_id path parameter."""
    return validate_uuid_path(tree_id, "tree_id")


def validate_grant_id(
    grant_id: str = Path(..., description="Access grant identifier"),
) -> str:
    """Validate grant_id path parameter."""
    return validate_uuid_path(grant_id, "grant_id")


def validate_package_id(
    package_id: str = Path(..., description="Evidence package identifier"),
) -> str:
...
def validate_tx_hash(
    ...
    """Validate tx_hash path parameter.

    Args:
        tx_hash: Transaction hash from URL path.
    ...
    """
    validated = validate_uuid_path(tx_hash, "tx_hash")
...
def validate_record_id(
    ...
    """Validate record_id path parameter."""
    return validate_uuid_path(record_id, "record_id")
...
def validate_subscription_id(
    ...
    """Validate subscription_id path parameter."""
    return validate_uuid_path(subscription_id, "subscription_id")
...
class _BCIServiceStub:
    """Stub service for Blockchain Integration operations.
    ...
    def __init__(self) -> None:
        ...
        logger.info("Blockchain Integration service stub initialized")
...
_bci_service_instance: Optional[Any] = None


def get_blockchain_service() -> Any:
    """Return the Blockchain Integration service singleton.
    ...
    Returns:
        Blockchain Integration service instance (real or stub).
    """
    global _bci_service_instance

    if _bci_service_instance is not None:
    ...
    try:
        ...
        config = get_config()
        _bci_service_instance = {
            "config": config,
            "initialized": True,
        }
        logger.info("Blockchain Integration service engines initialized")
    except Exception as exc:
        logger.warning(
            "Could not initialize BCI service engines, using stub: %s",
            exc,
        )
        _bci_service_instance = _BCIServiceStub()
