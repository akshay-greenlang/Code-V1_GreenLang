# -*- coding: utf-8 -*-
"""
API Dependencies - AGENT-EUDR-009 Chain of Custody
...
Dependencies:
    ...
    - get_coc_service: Returns the ChainOfCustody service singleton (lazy init).
...
def require_permission(permission: str) -> Callable:
    ...
    Uses wildcard matching: ``eudr-coc:*`` grants all
    ``eudr-coc:<action>`` permissions.

    Args:
        permission: Required permission string, e.g.
            ``eudr-coc:events:create``.
    ...
    Example:
        ...
        ...     user: AuthUser = Depends(require_permission("eudr-coc:events:create"))
        ... ):
        ...     ...
    """
...
def validate_event_id(
    event_id: str = Path(..., description="Custody event identifier"),
) -> str:
...
def validate_batch_id(
    batch_id: str = Path(..., description="Batch/lot identifier"),
) -> str:
    """Validate batch_id path parameter."""
    return validate_uuid_path(batch_id, "batch_id")
...
def validate_transform_id(
    transform_id: str = Path(..., description="Transformation identifier"),
) -> str:
    """Validate transform_id path parameter."""
    return validate_uuid_path(transform_id, "transform_id")
...
def validate_job_id(
    batch_id: str = Path(..., description="Batch job identifier"),
) -> str:
...
class _CoCServiceStub:
    """Stub service for Chain of Custody operations.
    ...
    def __init__(self) -> None:
        ...
        logger.info("Chain of Custody service stub initialized")
...
_coc_service_instance: Optional[Any] = None


def get_coc_service() -> Any:
    """Return the Chain of Custody service singleton.
    ...
    Returns:
        Chain of Custody service instance (real or stub).
    """
    global _coc_service_instance

    if _coc_service_instance is not None:
        return _coc_service_instance
    ...
    try:
        ...
        _coc_service_instance = {
            "event_tracker": CustodyEventTracker(),
            "batch_manager": BatchLifecycleManager(),
            "model_enforcer": CoCModelEnforcer(),
            "balance_engine": MassBalanceEngine(),
        }
        logger.info("Chain of Custody service engines initialized")
    except Exception as exc:
        logger.warning(
            "Could not initialize CoC service engines, using stub: %s",
            exc,
        )
        _coc_service_instance = _CoCServiceStub()

    return _coc_service_instance


def reset_coc_service() -> None:
    """Reset the service singleton. Used in testing."""
    global _coc_service_instance
    _coc_service_instance = None
