# -*- coding: utf-8 -*-
"""
API Dependencies - AGENT-EUDR-001 Supply Chain Mapper

FastAPI dependency injection providers for authentication, authorization,
rate limiting, and service access. All route handlers inject these
dependencies to enforce JWT auth (SEC-001), RBAC (SEC-002), and
...
Dependencies:
    ...
    - get_graph_service: Returns the SupplyChainGraphEngine singleton.
...
async def get_current_user(
    ...
    # Fallback: validate token/api_key manually
    if token:
        try:
...
def require_permission(permission: str) -> Callable:
    ...
    Uses wildcard matching: ``eudr-supply-chain:*`` grants all
    ``eudr-supply-chain:<action>`` permissions.

    Args:
        permission: Required permission string, e.g.
            ``eudr-supply-chain:graphs:read``.
    ...
    Example:
        ...
        ...     user: AuthUser = Depends(require_permission("eudr-supply-chain:graphs:read"))
        ... ):
        ...     ...
    """
