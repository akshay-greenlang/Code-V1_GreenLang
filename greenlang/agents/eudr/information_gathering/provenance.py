# -*- coding: utf-8 -*-
"""
Provenance Tracker - AGENT-EUDR-027: Information Gathering Agent

SHA-256 provenance hash chain for deterministic, auditable data lineage
tracking across all information gathering operations. Every collected
record, normalization, and assembly step is hash-chained for integrity
...
class ProvenanceTracker:
    ...
    and builds verifiable hash chains across processing steps.

    Example:
        >>> tracker = ProvenanceTracker()
        >>> h1 = tracker.compute_hash({"key": "value"})
        >>> assert len(h1) == 64
        >>> entry = tracker.create_entry("collect", "eu_traces", GENESIS_HASH, h1)
    ...
    def create_entry(
        ...
        Args:
            step: Processing step name (e.g., "collect", "normalize", "assemble").
