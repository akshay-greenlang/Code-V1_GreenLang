# -*- coding: utf-8 -*-
"""
API Dependencies - AGENT-EUDR-011 Mass Balance Calculator
...
Dependencies:
    ...
    - get_mbc_service: Returns the MassBalanceCalculator service singleton (lazy init).
...
Rate Limiter Tiers:
    - standard: 200 requests/minute (reads, queries, lookups)
    - write: 50 requests/minute (ledger entries, loss recording)
    - batch: 10 requests/minute (bulk imports, reconciliation)
    - report: 20 requests/minute (report generation, consolidation)
    - export: 10 requests/minute (report downloads)

Permission Prefix: eudr-mbc:*
...
class AuthUser(BaseModel):
    ...
    Attributes:
        ...
        permissions: Granted fine-grained permissions (e.g. ``eudr-mbc:ledgers:create``).
    """
...
def require_permission(permission: str) -> Callable:
    ...
    Uses wildcard matching: ``eudr-mbc:*`` grants all
    ``eudr-mbc:<action>`` permissions.

    Args:
        permission: Required permission string, e.g.
            ``eudr-mbc:ledgers:create``.
    ...
    Example:
        ...
        ...     user: AuthUser = Depends(require_permission("eudr-mbc:ledgers:create"))
        ... ):
        ...     ...
    """
    ...
    async def _check_permission(
        ...
        # Check wildcard patterns (eudr-mbc:ledgers:* matches eudr-mbc:ledgers:create)
        parts = permission.split(":")
        for i in range(len(parts)):
            wildcard = ":".join(parts[: i + 1]) + ":*"
            if wildcard in user.permissions:
                return user
        # Check top-level wildcard (eudr-mbc:*)
...
async def rate_limit_standard(
    ...
    """Standard rate limit: 200 requests/minute for read operations."""
...
def validate_ledger_id(
    ledger_id: str = Path(..., description="Mass balance ledger identifier"),
) -> str:
    """Validate ledger_id path parameter."""
    return validate_uuid_path(ledger_id, "ledger_id")


def validate_period_id(
    period_id: str = Path(..., description="Credit period identifier"),
) -> str:
    """Validate period_id path parameter."""
    return validate_uuid_path(period_id, "period_id")
...
def validate_reconciliation_id(
    reconciliation_id: str = Path(
        ..., description="Reconciliation identifier"
    ),
) -> str:
    """Validate reconciliation_id path parameter."""
    return validate_uuid_path(reconciliation_id, "reconciliation_id")


def validate_report_id(
    report_id: str = Path(..., description="Consolidation report identifier"),
) -> str:
...
def validate_commodity_path(
    commodity: str = Path(..., description="EUDR commodity"),
) -> str:
    """Validate commodity path parameter.

    Args:
        commodity: Commodity name from URL path.

    Returns:
        Lowercased, stripped commodity string.

    Raises:
        HTTPException: 400 if commodity is empty.
    """
    validated = validate_uuid_path(commodity, "commodity")
...
class _MBCServiceStub:
    """Stub service for Mass Balance Calculator operations.
    ...
    def __init__(self) -> None:
        ...
        logger.info("Mass Balance Calculator service stub initialized")
...
_mbc_service_instance: Optional[Any] = None


def get_mbc_service() -> Any:
    """Return the Mass Balance Calculator service singleton.
    ...
    Returns:
        Mass Balance Calculator service instance (real or stub).
    """
    global _mbc_service_instance

    if _mbc_service_instance is not None:
        return _mbc_service_instance
    ...
    try:
        ...
        _mbc_service_instance = {
            "ledger_manager": LedgerManager(),
        }
        logger.info("Mass Balance Calculator service engines initialized")
    except Exception as exc:
        logger.warning(
            "Could not initialize MBC service engines, using stub: %s",
            exc,
        )
        _mbc_service_instance = _MBCServiceStub()

    return _mbc_service_instance


def reset_mbc_service() -> None:
    """Reset the service singleton. Used in testing."""
    global _mbc_service_instance
    _mbc_service_instance = None
