...
class SolarResourceAgent(BaseAgent):
    ...
    def execute(self, input_data: Dict[str, Any]) -> AgentResult:
        ...
        try:
            ...
            timestamps = pd.to_datetime(pd.date_range(start="2023-01-01", end="2023-12-31 23:00", freq="h"))
            ...
            df = pd.DataFrame({
                "dni_w_per_m2": dni[:8760],
                "temp_c": temp[:8760]
            }, index=timestamps)

            return AgentResult(
                success=True,
                data={"solar_resource_df": df.to_json(orient="split")}
            )
