...
class GridFactorAgent(Agent[GridFactorInput, GridFactorOutput]):
    ...
    def __init__(self) -> None:
        ...
        self.factors_path = Path(__file__).parent.parent / "data" / "global_emission_factors.json"
        self.emission_factors: Dict[str, Dict[str, Dict[str, float]]] = self._load_emission_factors()
    ...
    def validate(self, payload: GridFactorInput) -> bool:
        ...
        if not payload.get("country") or not payload.get("fuel_type") or not payload.get("unit"):
            return False
        return True
    ...
    def get_available_fuel_types(self, country: str) -> List[str]:
        ...
        if country in self.emission_factors:
            return [k for k in self.emission_factors[country].keys() 
