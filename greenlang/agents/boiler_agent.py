...
class BoilerAgent(Agent[BoilerInput, BoilerOutput]):
    ...
    def load_efficiency_config(cls) -> Dict:
        ...
        config_path = Path(__file__).parent.parent / "configs" / "boiler_efficiencies.json"
    ...
    def validate(self, payload: BoilerInput) -> bool:
        ...
        """
        self.logger.debug(f"Validating payload for boiler type: {payload.get('boiler_type')}")
    ...
    def _get_cached_emission_factor(self, fuel_type: str, unit: str, 
                                   region: str, year: int) -> Optional[Dict]:
    ...
    def run(self, payload: BoilerInput) -> AgentResult[BoilerOutput]:
        ...
        with self.performance_tracker.track("boiler_calculation"):
            ...
            try:
                ...
                self.logger.info(f"Boiler calculation completed in {execution_time:.3f}s")
                ...
                self.logger.error(f"Error in boiler calculation: {str(e)}", exc_info=True)
                error_info: ErrorInfo = {
    ...
    def batch_process(self, boilers: List[BoilerInput]) -> List[AgentResult[BoilerOutput]]:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = {executor.submit(self.run, boiler): i 
    ...
    async def async_batch_process(self, boilers: List[BoilerInput]) -> List[AgentResult[BoilerOutput]]:
    ...
    def _calculate_fuel_from_thermal(self, thermal_output: Dict[str, Any], 
        ...
        if fuel_type == "natural_gas":
            ...
            return {"value": fuel_mmbtu * 7.25, "unit": "gallons"}  # MMBtu to gallons (diesel)
    ...
    def export_results(self, results: List[BoilerOutput], format: str = "json") -> str:
        ...
        if format == "json":
            ...
            with open(output_path, 'w') as f:
                json.dump([r.dict() if hasattr(r, 'dict') else r for r in results], f, indent=2)
            ...
            if results:
                keys = results[0].keys() if isinstance(results[0], dict) else results[0].__dict__.keys()
                with open(output_path, 'w', newline='') as f:
                    writer = csv.DictWriter(f, fieldnames=keys)
                    writer.writeheader()
                    for result in results:
                        writer.writerow(result if isinstance(result, dict) else result.__dict__)
            ...
            try:
                ...
                self.logger.error("pandas not installed for Excel export")
