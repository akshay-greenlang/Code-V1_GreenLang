...
class FuelAgent(Agent[FuelInput, FuelOutput]):
    ...
    def validate(self, payload: FuelInput) -> bool:
        ...
        """
        self.logger.debug(f"Validating payload for fuel type: {payload.get('fuel_type')}")
        ...
        renewable_fuels = self.fuel_config.get("fuel_properties", {}).get(
            fuel_type, {}
        ).get("renewable_potential") == "renewable"
        
        if renewable_fuels and amount > 0:
            self.logger.warning(f"Renewable fuel {fuel_type} should have negative amount for generation")
        elif not renewable_fuels and amount < 0:
            self.logger.error(f"Non-renewable fuel {fuel_type} cannot have negative amount")
            return False
    ...
    def _get_cached_emission_factor(self, fuel_type: str, unit: str, 
    ...
    def run(self, payload: FuelInput) -> AgentResult[FuelOutput]:
        ...
        with self.performance_tracker.track("fuel_calculation"):
            ...
            self.logger.info(f"Calculating emissions for {amount} {unit} of {fuel_type}")
            ...
            try:
                ...
                if renewable_percentage > 0 and fuel_type == "electricity":
                    ...
                    self.logger.info(f"Applied {renewable_percentage}% renewable offset")
                ...
                # Get fuel properties
                fuel_props = self.fuel_config.get("fuel_properties", {}).get(fuel_type, {})
                ...
                self.logger.info(f"Calculated {co2e_emissions_kg:.2f} kg CO2e emissions")
    ...
    def batch_process(self, fuels: List[FuelInput]) -> List[AgentResult[FuelOutput]]:
        ...
        with self.performance_tracker.track("batch_processing"):
            ...
            with ThreadPoolExecutor(max_workers=5) as executor:
                future_to_fuel = {executor.submit(self.run, fuel): fuel for fuel in fuels}
            ...
            total_emissions = sum(
                r["data"]["co2e_emissions_kg"] 
                for r in results if r["success"]
            )
            
            self.logger.info(f"Batch processing complete. Total emissions: {total_emissions:.2f} kg CO2e")
            
            return results
    
    async def async_batch_process(self, fuels: List[FuelInput]) -> List[AgentResult[FuelOutput]]:
        ...
        self.logger.info(f"Async processing complete. Total emissions: {total_emissions:.2f} kg CO2e")
        
        return results
    
    def _calculate_energy_content(self, amount: float, unit: str, fuel_type: str) -> float:
    ...
    def _generate_fuel_recommendations(self, fuel_type: str, amount: float, 
                                      unit: str, emissions_kg: float, 
                                      country: str) -> List[Dict[str, str]]:
    ...
    def export_results(self, results: List[AgentResult], format: str = "json") -> str:
        ...
        if format == "json":
            ...
            try:
                ...
                self.logger.error("pandas/openpyxl required for Excel export")
                raise
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        summary["fuel_metrics"] = {
            ...
            "average_execution_time_ms": sum(self._execution_times) / max(len(self._execution_times), 1) * 1000,
            "total_calculations": len(self._historical_data),
            "unique_fuel_types": len(set(d["fuel_type"] for d in self._historical_data))
        }
