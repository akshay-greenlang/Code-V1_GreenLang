...
Usage:
    ...
    >>> from greenlang.agents.mrv.flaring.setup import configure_flaring
    >>> app = FastAPI()
    >>> configure_flaring(app)

    >>> from greenlang.agents.mrv.flaring.setup import get_service
    >>> svc = get_service()
...
try:
    from greenlang.agents.mrv.flaring.config import (
        FlaringConfig,
        get_config,
    )
except ImportError:
...
try:
    from greenlang.agents.mrv.flaring.flare_system_database import FlareSystemDatabaseEngine
except ImportError:
    FlareSystemDatabaseEngine = None  # type: ignore[assignment, misc]

try:
    from greenlang.agents.mrv.flaring.emission_calculator import (
...
try:
    from greenlang.agents.mrv.flaring.combustion_efficiency import (
...
try:
    from greenlang.agents.mrv.flaring.flaring_event_tracker import (
...
try:
    from greenlang.agents.mrv.flaring.uncertainty_quantifier import (
...
try:
    from greenlang.agents.mrv.flaring.compliance_checker import (
...
try:
    from greenlang.agents.mrv.flaring.flaring_pipeline import (
...
try:
    from greenlang.agents.mrv.flaring.provenance import ProvenanceTracker
except ImportError:
    ProvenanceTracker = None  # type: ignore[assignment, misc]

try:
    from greenlang.agents.mrv.flaring.metrics import (
...
class FlaringService:
    ...
    def register_composition(
        ...
        hhv = 0.0
        from greenlang.agents.mrv.flaring.flaring_pipeline import GAS_COMPONENT_HHVS
    ...
    def check_compliance(
        ...
        from greenlang.agents.mrv.flaring.flaring_pipeline import REGULATORY_FRAMEWORKS
...
def get_router() -> Any:
    ...
    try:
        from greenlang.agents.mrv.flaring.api.router import create_router
