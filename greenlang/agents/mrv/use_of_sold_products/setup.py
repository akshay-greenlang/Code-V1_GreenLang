...
Example:
    >>> from greenlang.agents.mrv.use_of_sold_products.setup import get_service
    >>> service = get_service()
...
Integration:
    >>> from greenlang.agents.mrv.use_of_sold_products.setup import get_router
    >>> app.include_router(get_router(), prefix="/api/v1/use-of-sold-products")

Module: greenlang.agents.mrv.use_of_sold_products.setup
Agent: AGENT-MRV-024
Version: 1.0.0
"""
...
class UseOfSoldProductsService:
    ...
    def __init__(self) -> None:
        ...
        self._database_engine = self._init_engine(
            "greenlang.agents.mrv.use_of_sold_products.product_use_database",
            "ProductUseDatabaseEngine",
        )
        self._direct_engine = self._init_engine(
            "greenlang.agents.mrv.use_of_sold_products.direct_emissions_calculator",
            "DirectEmissionsCalculatorEngine",
        )
        self._indirect_engine = self._init_engine(
            "greenlang.agents.mrv.use_of_sold_products.indirect_emissions_calculator",
            "IndirectEmissionsCalculatorEngine",
        )
        self._fuels_engine = self._init_engine(
            "greenlang.agents.mrv.use_of_sold_products.fuels_feedstocks_calculator",
            "FuelsAndFeedstocksCalculatorEngine",
        )
        self._lifetime_engine = self._init_engine(
            "greenlang.agents.mrv.use_of_sold_products.lifetime_modeling",
            "LifetimeModelingEngine",
        )
        self._compliance_engine = self._init_engine(
            "greenlang.agents.mrv.use_of_sold_products.compliance_checker",
            "ComplianceCheckerEngine",
        )
        self._pipeline_engine = self._init_engine(
            "greenlang.agents.mrv.use_of_sold_products.use_of_sold_products_pipeline",
...
def get_router():
    ...
    """
    from greenlang.agents.mrv.use_of_sold_products.api.router import router
    return router
