# -*- coding: utf-8 -*-
"""
Provenance Tracking for Refrigerants & F-Gas Agent - AGENT-MRV-002

Provides SHA-256 based audit trail tracking for all refrigerants and
fluorinated gas agent operations. Maintains an in-memory chain-hashed
operation log for tamper-evident provenance across refrigerant management,
equipment tracking, service events, leak rate estimation, emission
calculations, blend decomposition, compliance checking, batch processing,
...
Zero-Hallucination Guarantees:
    ...
    - Complete provenance for every refrigerant and F-gas operation

Entity Types:
    - refrigerant: Registered refrigerant type definitions and GWP properties
    - equipment: Refrigerant-containing equipment profiles
    - service_event: Equipment service events (recharge, repair, recovery)
    - leak_rate: Leak rate selections and estimations by equipment/lifecycle
    - calculation: Individual refrigerant emission calculation results
    - blend: Blend decomposition operations (multi-component refrigerants)
    - compliance: Regulatory compliance checks and quota tracking
    - audit: Audit trail entries for calculation step traceability
    - pipeline: End-to-end pipeline orchestration runs

Actions (14):
    Entity management: register, lookup, service
    Calculations: estimate, calculate, decompose, aggregate
    Validation: validate, check_compliance
    Batch operations: batch, export
    Pipeline: pipeline_start, pipeline_end, pipeline_fail

Example:
    >>> from greenlang.agents.mrv.refrigerants_fgas.provenance import ProvenanceTracker
...
class ProvenanceEntry:
    """A single tamper-evident provenance record for a refrigerant and F-gas event.

    Attributes:
        entity_type: Type of entity being tracked (refrigerant,
            equipment, service_event, leak_rate, calculation,
            blend, compliance, audit, pipeline).
        entity_id: Unique identifier for the entity instance.
        action: Action performed (register, lookup, service, estimate,
            calculate, decompose, aggregate, validate, check_compliance,
            batch, export, pipeline_start, pipeline_end, pipeline_fail).
...
class ProvenanceTracker:
    """Tracks provenance for refrigerant and F-gas operations with SHA-256 chain hashing.
    ...
    Supported entity types:
        - ``refrigerant``: Registered refrigerant type definitions and GWP
          property records. Covers HFCs, HFC blends, HFOs, PFCs, SF6,
          NF3, HCFCs, CFCs, and natural refrigerants with per-IPCC-AR
          GWP values and physical properties (molecular weight, boiling
          point, atmospheric lifetime, ODP).
        - ``equipment``: Refrigerant-containing equipment profiles including
          commercial refrigeration, HVAC systems, chillers, heat pumps,
          transport refrigeration, switchgear, and semiconductor equipment
          with charge amounts, status, and location tracking.
        - ``service_event``: Equipment service events recording refrigerant
          additions (installation, recharge), removals (recovery,
          decommissioning), and inspections (leak check, repair) for
          lifecycle emission accounting.
        - ``leak_rate``: Leak rate estimation records combining base rates,
          age factors, climate factors, and LDAR adjustments to produce
          effective annual leak rates by equipment type and lifecycle stage.
        - ``calculation``: Individual refrigerant emission calculation results
          with per-gas breakdown, CO2e totals, and calculation methodology
          metadata (equipment-based, mass-balance, screening).
        - ``blend``: Blend decomposition operations that resolve multi-
          component refrigerant mixtures (R-404A, R-410A, R-407C, etc.)
          into constituent gases with weight-fraction GWP contributions.
        - ``compliance``: Regulatory compliance check results and quota
          tracking against EU F-Gas, Kigali Amendment, EPA Subpart DD/OO/L,
          GHG Protocol, ISO 14064, and CSRD frameworks.
        - ``audit``: Audit trail entries recording each discrete step in
          the emission calculation process for full reproducibility.
        - ``pipeline``: End-to-end pipeline orchestration runs coordinating
          data intake, calculation, aggregation, compliance, and reporting.
    ...
    def __init__(
        ...
        Args:
            genesis_hash: String used to compute the immutable genesis hash.
                Defaults to
                ``"GL-MRV-X-002-REFRIGERANTS-FGAS-GENESIS"``.
        """
    ...
    def record(
        ...
        """Record a provenance entry for a refrigerant and F-gas operation.
        ...
        Args:
            entity_type: Type of entity (refrigerant, equipment,
                service_event, leak_rate, calculation, blend, compliance,
                audit, pipeline).
            action: Action performed (register, lookup, service, estimate,
                calculate, decompose, aggregate, validate, check_compliance,
                batch, export, pipeline_start, pipeline_end, pipeline_fail).
    ...
    def get_entries(
        ...
        Args:
            entity_type: Optional entity type to filter by (refrigerant,
                equipment, service_event, leak_rate, calculation, blend,
    ...
    if _singleton_tracker is None:
        with _singleton_lock:
            if _singleton_tracker is None:
                ...
                logger.info(
                    "Refrigerants & F-Gas singleton ProvenanceTracker created"
                )
...
def set_provenance_tracker(tracker: ProvenanceTracker) -> None:
    ...
    logger.info("Refrigerants & F-Gas ProvenanceTracker singleton replaced")
...
def reset_provenance_tracker() -> None:
    ...
    logger.info(
        "Refrigerants & F-Gas ProvenanceTracker singleton reset to None"
    )
