...
async def calculate_asset_specific(
    ...
    try:
        ...
        logger.error("Validation error in calculate_asset_specific: %s", e)
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e),
        )
...
async def get_calculation_detail(
    ...
    """
    try:
        logger.info("Getting DLA calculation detail: %s", calculation_id)
...
async def delete_calculation(
    ...
    """
    try:
        logger.info("Deleting DLA calculation: %s", calculation_id)
...
async def get_emission_factors(
    ...
    """
    try:
        logger.info("Getting DLA emission factors: asset_type=%s", asset_type)
