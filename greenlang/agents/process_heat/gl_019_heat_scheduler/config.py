# -*- coding: utf-8 -*-
"""
GL-019 HEATSCHEDULER Agent - Configuration Module

Configuration schemas for heat scheduling including tariff structures,
thermal storage, demand charge optimization, and load forecasting settings.
...
class StorageType(str, Enum):
    """Thermal storage system types."""

    HOT_WATER_TANK = "hot_water_tank"
    CHILLED_WATER_TANK = "chilled_water_tank"
...
class TariffConfiguration(BaseModel):
    ...
    def validate_rate_structure(self) -> "TariffConfiguration":
        ...
        if self.shoulder_rate_per_kwh is not None:
            if not (
                self.off_peak_rate_per_kwh
                <= self.shoulder_rate_per_kwh
                <= self.peak_rate_per_kwh
            ):
                logger.warning(
...
class EquipmentConfiguration(BaseModel):
    ...
    # Capacity
    capacity_kw: float = Field(..., gt=0, description="Rated power capacity (kW)")
    ...
    def validate_max_power(cls, v: Optional[float], info) -> Optional[float]:
        ...
        if v is not None and "capacity_kw" in info.data:
            if v < info.data["capacity_kw"]:
                logger.warning(
                    f"Max power ({v} kW) is less than capacity "
                    f"({info.data['capacity_kw']} kW)"
                )
        return v
...
class ThermalStorageConfiguration(BaseModel):
    """
    Thermal storage system configuration.

    Defines thermal storage parameters for hot water tanks, PCM systems,
    and other thermal storage technologies.
    """

    storage_id: str = Field(..., description="Unique storage identifier")
    storage_type: StorageType = Field(..., description="Storage technology type")
    storage_name: Optional[str] = Field(None, description="Storage display name")
    enabled: bool = Field(default=True, description="Storage system enabled")

    # Capacity
    capacity_kwh: float = Field(
        ..., gt=0, description="Thermal storage capacity (kWh)"
    )
    max_charge_rate_kw: float = Field(
        ..., gt=0, description="Maximum charging rate (kW)"
    )
    max_discharge_rate_kw: float = Field(
        ..., gt=0, description="Maximum discharge rate (kW)"
    )
    ...
    round_trip_efficiency: float = Field(
        ...
        ge=0.5,
        le=1.0,
        description="Round-trip charging/discharging efficiency",
    )
    standby_loss_pct_per_hour: float = Field(
        default=0.5,
        ge=0,
        le=10,
        description="Standby thermal loss (% per hour)",
    )
    ...
    # Temperature (for hot water/PCM)
    design_temperature_c: float = Field(
        default=80.0, description="Design storage temperature (C)"
    )
    min_temperature_c: float = Field(
        default=60.0, description="Minimum operating temperature (C)"
    )
    max_temperature_c: float = Field(
        default=95.0, description="Maximum operating temperature (C)"
    )

    # PCM specific
    pcm_melt_temperature_c: Optional[float] = Field(
        None, description="PCM melting temperature (C)"
    )
    pcm_latent_heat_kj_kg: Optional[float] = Field(
        None, description="PCM latent heat (kJ/kg)"
    )
    ...
    current_temperature_c: float = Field(
        default=75.0, description="Current temperature (C)"
    )

    # Control parameters
    charge_priority: int = Field(
        default=5,
        ge=1,
        le=10,
        description="Charging priority (1=lowest, 10=highest)",
    )
    discharge_priority: int = Field(
        default=5,
        ge=1,
        le=10,
        description="Discharge priority (1=lowest, 10=highest)",
    )

    @model_validator(mode="after")
    def validate_soc_limits(self) -> "ThermalStorageConfiguration":
        """Validate state of charge limits."""
...
class LoadForecastingConfiguration(BaseModel):
    """
    Load forecasting configuration for ML-based predictions.

    Defines model parameters, horizons, and feature engineering settings
    for 24-48 hour load prediction.
    """

    enabled: bool = Field(default=True, description="Load forecasting enabled")
    model_type: ForecastModel = Field(
        default=ForecastModel.ENSEMBLE,
    ...
    granularity_minutes: int = Field(
        ...
        ge=5,
        le=60,
        description="Forecast granularity (minutes)",
    )
    ...
    # Feature engineering
    use_weather_features: bool = Field(
        default=True, description="Include weather features"
    )
    use_calendar_features: bool = Field(
        default=True, description="Include calendar features"
    )
    use_production_features: bool = Field(
        default=True, description="Include production schedule features"
    )
    use_lagged_features: bool = Field(
        default=True, description="Include lagged load features"
    )
    ...
    # Error thresholds
    mape_warning_threshold: float = Field(
        default=10.0,
        ge=1,
        le=50,
        description="MAPE warning threshold (%)",
    )
    mape_critical_threshold: float = Field(
        default=20.0,
        ge=5,
        le=100,
        description="MAPE critical threshold (%)",
    )
...
class DemandChargeConfiguration(BaseModel):
    """
    Demand charge optimization configuration.

    Defines parameters for peak demand reduction, load shifting,
    and real-time pricing response.
    """

    enabled: bool = Field(default=True, description="Demand charge optimization enabled")

    # Peak demand limits
    peak_demand_limit_kw: float = Field(
        ..., gt=0, description="Target peak demand limit (kW)"
    )
    soft_peak_limit_kw: Optional[float] = Field(
        None, gt=0, description="Soft peak limit for optimization"
    )
    absolute_max_demand_kw: Optional[float] = Field(
        None, gt=0, description="Absolute maximum demand (kW)"
    )

    # Demand window
    demand_interval_minutes: int = Field(
        default=15,
        ge=5,
        le=60,
        description="Demand measurement interval (minutes)",
    )
    rolling_demand_average: bool = Field(
        default=True, description="Use rolling average for demand"
    )
    ...
    max_shift_hours: int = Field(
        ...
        ge=0,
        le=12,
        description="Maximum load shift (hours)",
    )
    min_shift_savings_threshold_usd: float = Field(
        default=5.0,
        ge=0,
        description="Minimum savings threshold for shifting ($)",
    )
    ...
    demand_response_threshold_kw: float = Field(
        default=1000.0,
        ge=0,
        description="Demand response threshold (kW)",
    )
    max_demand_curtailment_pct: float = Field(
        default=30.0,
        ge=0,
        le=100,
        description="Maximum demand curtailment (%)",
    )
    dr_notification_lead_time_minutes: int = Field(
        default=30,
        ge=0,
        le=240,
        description="DR event notification lead time (minutes)",
    )

    # Ratchet management
    consider_ratchet: bool = Field(
        default=True, description="Consider demand ratchet in optimization"
    )
    ratchet_percentage: float = Field(
        default=80.0,
        ge=0,
    ...
    annual_ratchet_peak_kw: float = Field(
        default=0.0,
        ge=0,
        description="Current annual ratchet peak (kW)",
    )
...
class WeatherConfiguration(BaseModel):
    """
    Weather forecast integration configuration.

    Defines weather data sources, update intervals, and feature
    extraction settings for load forecasting.
    """

    enabled: bool = Field(default=True, description="Weather integration enabled")
    ...
    api_endpoint: Optional[str] = Field(
        None, description="Custom API endpoint"
    )
    ...
    update_interval_minutes: int = Field(
        ...
        ge=15,
        le=360,
        description="Weather data update interval (minutes)",
    )
    forecast_horizon_hours: int = Field(
        default=72,
        ge=24,
        le=168,
        description="Weather forecast horizon (hours)",
    )

    # Features to extract
    include_temperature: bool = Field(
        default=True, description="Include temperature data"
    )
    include_humidity: bool = Field(
        default=True, description="Include humidity data"
    )
    include_solar_radiation: bool = Field(
        default=True, description="Include solar radiation"
    )
    include_wind: bool = Field(
        default=False, description="Include wind data"
    )
    ...
    heating_base_temp_c: float = Field(
        default=18.0,
        description="Heating degree day base temperature (C)",
    )
    cooling_base_temp_c: float = Field(
        default=24.0,
        description="Cooling degree day base temperature (C)",
    )
...
class ControlSystemIntegration(BaseModel):
    ...
    enabled: bool = Field(
        default=True, description="Control system integration enabled"
    )
    ...
    power_setpoint_tag: str = Field(default="", description="Power setpoint tag")
...
class HeatSchedulerConfig(BaseModel):
    """
    Complete GL-019 Heat Scheduler Agent configuration.

    Top-level configuration for the Process Heating Scheduler Agent
    including all sub-configurations for tariffs, equipment, thermal
    storage, load forecasting, and demand charge optimization.
    """
    ...
    tariffs: List[TariffConfiguration] = Field(
        default_factory=list, description="List of energy tariff configurations"
    )

    # Equipment configurations
    equipment: List[EquipmentConfiguration] = Field(
        default_factory=list, description="List of heating equipment configurations"
    )

    # Thermal storage
    thermal_storage: List[ThermalStorageConfiguration] = Field(
        default_factory=list, description="Thermal storage system configurations"
    )

    # Load forecasting
    load_forecasting: LoadForecastingConfiguration = Field(
        default_factory=LoadForecastingConfiguration,
        description="Load forecasting configuration",
    )

    # Demand charge optimization
    demand_charge: DemandChargeConfiguration = Field(
        default_factory=lambda: DemandChargeConfiguration(peak_demand_limit_kw=5000.0),
        description="Demand charge optimization configuration",
    )

    # Weather integration
    weather: WeatherConfiguration = Field(
        default_factory=WeatherConfiguration,
        description="Weather forecast integration configuration",
    )
    ...
    def get_storage(self, storage_id: str) -> Optional[ThermalStorageConfiguration]:
        """Get thermal storage configuration by ID."""
        for storage in self.thermal_storage:
