...
Usage:
    ...
    >>> from greenlang.agents.data.excel_normalizer.setup import configure_excel_normalizer
    >>> app = FastAPI()
    >>> import asyncio
...
async def configure_excel_normalizer(
    ...
    try:
        from greenlang.agents.data.excel_normalizer.api.router import (
...
def get_router(service: Optional[ExcelNormalizerService] = None) -> Any:
    ...
    """
    try:
        from greenlang.agents.data.excel_normalizer.api.router import router
        return router
    except ImportError:
        return None
