...
Usage:
    ...
    >>> from greenlang.agents.data.duplicate_detector.setup import configure_duplicate_detector
    >>> app = FastAPI()
    >>> import asyncio
...
class DuplicateDetectorService:
    ...
    def _init_engines(self) -> None:
        ...
        try:
            from greenlang.agents.data.duplicate_detector.blocking_engine import BlockingEngine
        ...
        try:
            from greenlang.agents.data.duplicate_detector.match_classifier import (
                MatchClassifierEngine,
            )
        ...
        try:
            from greenlang.agents.data.duplicate_detector.cluster_resolver import (
                ClusterResolverEngine,
            )
        ...
        try:
            from greenlang.agents.data.duplicate_detector.merge_engine import MergeEngine
            self._merge_engine = MergeEngine(self.config)
        except ImportError:
            logger.warning("MergeEngine not available; using stub")

        try:
            from greenlang.agents.data.duplicate_detector.dedup_pipeline import (
                DeduplicationPipelineEngine,
            )
...
async def configure_duplicate_detector(
    ...
    try:
        from greenlang.agents.data.duplicate_detector.api.router import router as dd_router
