...
extended outlier-detector API (thread-safe, ``add_to_chain``,
...
class ProvenanceTracker(_BaseProvenanceTracker):
    """Thread-safe provenance tracker for outlier detection operations.
    ...
    def __init__(self) -> None:
        """Initialize with outlier-detector genesis hash."""
        super().__init__(agent_name="outlier-detector")
    ...
    def add_to_chain(
        ...
        with self._lock:
            chain_hash = self._compute_od_chain_hash(
    ...
    def _compute_od_chain_hash(
        ...
        """Compute the next chain hash (5-arg variant for outlier detector).
