...
Usage:
    ...
    >>> from greenlang.agents.data.pdf_extractor.setup import configure_pdf_extractor
    >>> app = FastAPI()
    >>> import asyncio
...
async def configure_pdf_extractor(
    ...
    try:
        from greenlang.agents.data.pdf_extractor.api.router import router as pdf_router
...
def get_router(service: Optional[PDFExtractorService] = None) -> Any:
    ...
    """
    try:
        from greenlang.agents.data.pdf_extractor.api.router import router
        return router
    except ImportError:
        return None
