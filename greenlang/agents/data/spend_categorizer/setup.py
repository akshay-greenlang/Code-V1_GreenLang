...
Usage:
    ...
    >>> from greenlang.agents.data.spend_categorizer.setup import configure_spend_categorizer
    >>> app = FastAPI()
    >>> import asyncio
...
class SpendCategorizerService:
    ...
    def _init_engines(self) -> None:
        ...
        """
        try:
            from greenlang.agents.data.spend_categorizer.record_ingestion import RecordIngestionEngine
        ...
        try:
            from greenlang.agents.data.spend_categorizer.taxonomy_classifier import TaxonomyClassifier
            self._taxonomy_classifier = TaxonomyClassifier(self.config)
        except ImportError:
            logger.warning("TaxonomyClassifier not available; using stub")

        try:
            from greenlang.agents.data.spend_categorizer.scope3_mapper import Scope3Mapper
            self._scope3_mapper = Scope3Mapper(self.config)
        except ImportError:
            logger.warning("Scope3Mapper not available; using stub")

        try:
            from greenlang.agents.data.spend_categorizer.emission_calculator import EmissionCalculator
            self._emission_calculator = EmissionCalculator(self.config)
        except ImportError:
            logger.warning("EmissionCalculator not available; using stub")

        try:
            from greenlang.agents.data.spend_categorizer.rule_engine import RuleEngine
        ...
        try:
            from greenlang.agents.data.spend_categorizer.report_generator import ReportGeneratorEngine
...
async def configure_spend_categorizer(
    ...
    try:
        from greenlang.agents.data.spend_categorizer.api.router import router as cat_router
