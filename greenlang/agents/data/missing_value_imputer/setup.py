...
Usage:
    ...
    >>> from greenlang.agents.data.missing_value_imputer.setup import configure_missing_value_imputer
    >>> app = FastAPI()
    >>> import asyncio
...
class MissingValueImputerService:
    ...
    def _init_engines(self) -> None:
        ...
        try:
            from greenlang.agents.data.missing_value_imputer.statistical_imputer import (
                StatisticalImputerEngine,
            )
...
async def configure_missing_value_imputer(
    ...
    try:
        from greenlang.agents.data.missing_value_imputer.api.router import router as mvi_router
...
def get_router(service: Optional[MissingValueImputerService] = None) -> Any:
    ...
    """
    try:
        from greenlang.agents.data.missing_value_imputer.api.router import router
        return router
    except ImportError:
        return None
