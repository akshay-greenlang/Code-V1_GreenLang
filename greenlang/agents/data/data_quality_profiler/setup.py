...
Usage:
    ...
    >>> from greenlang.agents.data.data_quality_profiler.setup import configure_data_quality_profiler
    >>> app = FastAPI()
    >>> import asyncio
...
class DataQualityProfilerService:
    ...
    def _init_engines(self) -> None:
        ...
        """
        try:
            from greenlang.agents.data.data_quality_profiler.dataset_profiler import DatasetProfilerEngine
        ...
        try:
            from greenlang.agents.data.data_quality_profiler.quality_assessor import QualityAssessorEngine
        ...
        try:
            from greenlang.agents.data.data_quality_profiler.anomaly_detector import AnomalyDetectorEngine
        ...
        try:
            from greenlang.agents.data.data_quality_profiler.freshness_checker import FreshnessCheckerEngine
        ...
        try:
            from greenlang.agents.data.data_quality_profiler.dq_rule_engine import DQRuleEngine
            self._rule_engine = DQRuleEngine(self.config)
        except ImportError:
            logger.warning("DQRuleEngine not available; using stub")

        try:
            from greenlang.agents.data.data_quality_profiler.quality_gate import QualityGateEngine
            self._quality_gate_engine = QualityGateEngine(self.config)
        except ImportError:
            logger.warning("QualityGateEngine not available; using stub")

        try:
            from greenlang.agents.data.data_quality_profiler.dq_report_generator import DQReportGeneratorEngine
...
async def configure_data_quality_profiler(
    ...
    try:
        from greenlang.agents.data.data_quality_profiler.api.router import router as dq_router
