...
Usage:
    ...
    >>> from greenlang.agents.data.erp_connector.setup import configure_erp_connector
    >>> app = FastAPI()
    >>> import asyncio
...
async def configure_erp_connector(
    ...
    try:
        from greenlang.agents.data.erp_connector.api.router import router as erp_router
...
def get_router(service: Optional[ERPConnectorService] = None) -> Any:
    ...
    """
    try:
        from greenlang.agents.data.erp_connector.api.router import router
        return router
    except ImportError:
        return None
