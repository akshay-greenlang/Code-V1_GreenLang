...
class GeocoderEngine:
    ...
    def forward(
        ...
        if not results:
            try:
                from greenlang.agents.data.gis_connector.boundary_resolver import COUNTRY_BBOX
        ...
        # Record metrics
        try:
            from greenlang.agents.data.gis_connector.metrics import record_geocoding_request
