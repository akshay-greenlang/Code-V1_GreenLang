...
from greenlang.hub import (
    HubClient, HubAuth, PackIndex, SearchFilters,
    SortOrder, create_manifest, load_manifest
)
...
@click.option('--sort', type=click.Choice(['relevance', 'downloads', 'stars', 'updated']),
              default='relevance', help='Sort order')
def search(query: str, category: str, tag: tuple, author: str, 
          verified: bool, limit: int, sort: str):
    ...
    try:
        if not query and not category and not tag and not author:
            console.print("[yellow]Showing featured packs (use search terms for specific results)[/yellow]\n")
        ...
        for pack in packs:
            ...
            table.add_row(
                name_display,
                pack.version,
                pack.description[:50] + "..." if len(pack.description) > 50 else pack.description,
        ...
        if installed:
            ...
            for pack_dir in packs_dir.iterdir():
                if pack_dir.is_dir():
                    try:
                        manifest = load_manifest(pack_dir)
                        console.print(f"  • {manifest.name}@{manifest.version} - {manifest.description}")
                    except:
