...
@pass_cli_context
def run(ctx: CLIContext, workflow_file: str, input: Optional[str], 
        output: Optional[str], log_dir: Optional[str]) -> None:
    ...
    # Log start event
    ctx.jsonl_logger.log_event("start", {
        "run_id": run_id,
    ...
    try:
        ...
        if ctx.dry_run:
            ...
            # Log completion
            ctx.jsonl_logger.log_event("complete", {
                "run_id": run_id,
        ...
        ctx.jsonl_logger.log_event("error", {
            "run_id": run_id,
            "error": str(e)
        })
...
@pass_cli_context
def report(ctx: CLIContext, input_file: str, output: Optional[str], format: str) -> None:
    ...
    if ctx.dry_run:
        console.print(f"[yellow]DRY-RUN: Would generate {format.upper()} report to {output_path}[/yellow]")
        return
