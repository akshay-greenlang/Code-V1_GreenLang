...
from greenlang.auth import (
    TenantManager, Tenant, TenantQuota, TenantIsolation,
...
@click.option('--max-compute-hours', type=int, default=1000, help='Maximum compute hours')
@click.option('--output', '-o', type=click.Choice(['json', 'yaml']), help='Output format')
def create(name: str, domain: Optional[str], admin_email: str, 
          admin_name: Optional[str], isolation: str,
          max_users: int, max_pipelines: int, 
          max_storage_gb: int, max_compute_hours: int,
          output: Optional[str]):
    """Create a new tenant"""
    try:
...
@click.option('--output', '-o', type=click.Choice(['json', 'yaml', 'table']), 
    ...
    try:
        ...
        if output == 'json':
            ...
            if tenant.quota:
                ...
                table.add_row("Used Storage (GB)", f"{tenant.quota.used_storage_gb:.2f}")
                table.add_row("Max Compute Hours", str(tenant.quota.max_compute_hours))
                table.add_row("Used Compute Hours", f"{tenant.quota.used_compute_hours:.2f}")
...
@tenant.command()
@click.option('--status', type=click.Choice(['active', 'suspended', 'all']), 
...
def delete(tenant_id: str, force: bool):
    ...
    try:
        ...
        # Confirm deletion
        if not force:
            if not click.confirm(f"Are you sure you want to delete tenant '{tenant.name}'?"):
...
def quota(tenant_id: str, output: str):
    ...
    try:
        ...
        if output == 'json':
            ...
            user_pct = (tenant.quota.used_users / tenant.quota.max_users * 100) if tenant.quota.max_users > 0 else 0
            pipeline_pct = (tenant.quota.used_pipelines / tenant.quota.max_pipelines * 100) if tenant.quota.max_pipelines > 0 else 0
            storage_pct = (tenant.quota.used_storage_gb / tenant.quota.max_storage_gb * 100) if tenant.quota.max_storage_gb > 0 else 0
            compute_pct = (tenant.quota.used_compute_hours / tenant.quota.max_compute_hours * 100) if tenant.quota.max_compute_hours > 0 else 0
            
            table.add_row("Users", str(tenant.quota.used_users), 
                         str(tenant.quota.max_users), f"{user_pct:.1f}%")
            table.add_row("Pipelines", str(tenant.quota.used_pipelines), 
                         str(tenant.quota.max_pipelines), f"{pipeline_pct:.1f}%")
            table.add_row("Storage (GB)", f"{tenant.quota.used_storage_gb:.2f}", 
                         str(tenant.quota.max_storage_gb), f"{storage_pct:.1f}%")
            table.add_row("Compute Hours", f"{tenant.quota.used_compute_hours:.2f}", 
                         str(tenant.quota.max_compute_hours), f"{compute_pct:.1f}%")
            ...
            warnings = []
            if user_pct > 80:
                warnings.append(f"[yellow]Warning: User quota is at {user_pct:.1f}%[/yellow]")
            if storage_pct > 80:
                warnings.append(f"[yellow]Warning: Storage quota is at {storage_pct:.1f}%[/yellow]")
            if compute_pct > 80:
                warnings.append(f"[yellow]Warning: Compute hours quota is at {compute_pct:.1f}%[/yellow]")
...
def assign_role(tenant_id: str, user_id: str, role_name: str):
    ...
    try:
        ...
        if rbac.assign_role(user_id, role_name):
            console.print(f"[green]Role '{role_name}' assigned to user '{user_id}'[/green]")
...
def revoke_role(tenant_id: str, user_id: str, role_name: str):
    ...
    try:
        ...
        if rbac.revoke_role(user_id, role_name):
            console.print(f"[yellow]Role '{role_name}' revoked from user '{user_id}'[/yellow]")
...
def create_apikey(tenant_id: str, name: str, user_id: str, 
                 expires_days: int, scopes: tuple):
...
@click.option('--output', '-o', type=click.Choice(['json', 'yaml', 'table']), 
              default='table', help='Output format')
def audit_logs(tenant_id: str, user: Optional[str], event_type: Optional[str],
        ...
        if output == 'json':
            ...
            for event in events:
                table.add_row(
                    ...
                    f"{event.resource_type}:{event.resource_id}" if event.resource_type else "N/A",
                    event.result or "N/A"
                )
