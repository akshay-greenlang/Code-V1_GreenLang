"""
Base Connector Module for GL-014 EXCHANGER-PRO (Heat Exchanger Optimization Agent).

Provides abstract base class with common functionality for all heat exchanger
...
class ConnectionState(str, Enum):
    """Connection state enumeration for heat exchanger system connectors."""
...
class ConnectorType(str, Enum):
    """Types of connectors supported by GL-014 EXCHANGER-PRO."""

    PROCESS_HISTORIAN = "process_historian"  # OSIsoft PI, Honeywell PHD, AspenTech IP.21
    DCS_SCADA = "dcs_scada"  # DeltaV, Experion, CENTUM
    CMMS = "cmms"  # SAP PM, IBM Maximo, Oracle EAM
    AGENT_COORDINATOR = "agent_coordinator"  # GL-001, GL-006, GL-013
...
class BaseConnectorConfig(BaseModel):
    """Base configuration for all heat exchanger system connectors."""
...
class ConnectionInfo(BaseModel):
    ...
    protocol: Optional[str] = Field(
        default=None,
        description="Protocol used (HTTP, HTTPS, OPC-UA, etc.)"
    )
...
class BaseConnector(ABC):
    """
    Abstract base class for GL-014 EXCHANGER-PRO connectors.
