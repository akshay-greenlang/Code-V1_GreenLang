...
Example:
    >>> from greenlang.processing_sold_products.setup import get_service
    >>> service = get_service()
...
Integration:
    >>> from greenlang.processing_sold_products.setup import get_router
...
class ProcessingSoldProductsService:
    ...
    def __init__(self) -> None:
        ...
        self._database_engine = self._init_engine(
            "greenlang.processing_sold_products.processing_database",
            "ProcessingDatabaseEngine",
            "database",
        )
        self._site_specific_engine = self._init_engine(
            "greenlang.processing_sold_products.site_specific_calculator",
            "SiteSpecificCalculatorEngine",
            "site_specific",
        )
        self._average_data_engine = self._init_engine(
            "greenlang.processing_sold_products.average_data_calculator",
            "AverageDataCalculatorEngine",
            "average_data",
        )
        self._spend_based_engine = self._init_engine(
            "greenlang.processing_sold_products.spend_based_calculator",
            "SpendBasedCalculatorEngine",
            "spend_based",
        )
        self._hybrid_engine = self._init_engine(
            "greenlang.processing_sold_products.hybrid_aggregator",
            "HybridAggregatorEngine",
            "hybrid",
        )
        self._compliance_engine = self._init_engine(
            "greenlang.processing_sold_products.compliance_checker",
            "ComplianceCheckerEngine",
            "compliance",
        )
        self._pipeline_engine = self._init_engine(
            "greenlang.processing_sold_products.processing_pipeline",
    ...
    def get_processing_ef(self, category: str) -> Decimal:
        ...
        # Hard fallback
        from greenlang.processing_sold_products.hybrid_aggregator import _get_fallback_ef
    ...
    def get_all_categories(self) -> List[str]:
        ...
        """
        try:
            from greenlang.processing_sold_products.models import IntermediateProductCategory
    ...
    def get_all_processing_types(self) -> List[str]:
        ...
        """
        try:
            from greenlang.processing_sold_products.models import ProcessingType
    ...
    def get_processing_chains(self) -> List[str]:
        ...
        """
        try:
            from greenlang.processing_sold_products.models import ProcessingChainType
...
def get_router():
    ...
    """
    from greenlang.processing_sold_products.api.router import router
    return router
