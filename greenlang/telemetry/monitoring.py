        ...
        for rule in self.rules.values():
            ...
            try:
                ...
                if value is not None and rule.evaluate(value):
                    ...
                    elif (datetime.utcnow() - self._pending_alerts[rule.name]).total_seconds() >= rule.duration:
                        # Fire alert
                        self.fire_alert(
                            rule,
                            f"{rule.expression} {rule.condition} (value: {value})"
                        )
    ...
    def to_grafana_json(self) -> Dict[str, Any]:
        ...
        return {
            "dashboard": {
                ...
                "templating": {
                    "list": self._convert_variables_to_grafana()
                },
                "time": {
                    "from": "now-6h",
                    "to": "now"
        ...
        system_dashboard = Dashboard(
            ...
            panels=[
                ...
                },
                {
                    ...
                    "type": "gauge",
                    "targets": [{"expr": "rate(gl_pipeline_runs_total{status='success'}[5m])"}],
