...
class RateLimiter:
    ...
    def get_client_id(self, tenant_id: Optional[str] = None, 
    ...
    def _check_token_bucket(self, client_id: str) -> RateLimitStatus:
        ...
        with self.lock:
            ...
            self.tokens[client_id] = min(
                capacity,
                self.tokens[client_id] + time_passed * rate
            )
            ...
            if self.tokens[client_id] >= 1:
                ...
                return RateLimitStatus(
                    ...
                    reset_at=datetime.fromtimestamp(now + (capacity - self.tokens[client_id]) / rate)
                )
            else:
...
class TenantRateLimiter(RateLimiter):
    ...
    def get_tenant_config(self, tenant_id: str, subscription_tier: str) -> RateLimitConfig:
        """Get rate limit config for tenant"""
        return self.tier_configs.get(subscription_tier, self.config)
    
    def check_tenant_limit(self, tenant_id: str, subscription_tier: str,
