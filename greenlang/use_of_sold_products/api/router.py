...
Example:
    ...
    >>> from greenlang.use_of_sold_products.api.router import usp_router
    >>> app = FastAPI()
...
def get_service():
    ...
    if _service_instance is None:
        try:
            from greenlang.use_of_sold_products.service import (
...
async def calculate_direct_fuel(
    ...
    try:
        ...
        logger.error(f"Validation error in calculate_direct_fuel: {e}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e),
        )
...
async def calculate_fuels_sold(
    ...
    try:
        ...
        logger.error(f"Validation error in calculate_fuels_sold: {e}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e),
        )
...
async def get_energy_profiles(
    ...
    """
    try:
        logger.info(f"Getting product energy profiles: category={category}")
...
async def get_refrigerant_gwps(
    ...
    """
    try:
        logger.info(f"Getting refrigerant GWPs: version={gwp_version}")
...
async def get_fuel_factors(
    ...
    """
    try:
        logger.info(f"Getting fuel factors: type={fuel_type}")
        ...
        logger.error(f"Error in get_fuel_factors: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_lifetime_estimates(
    ...
    """
    try:
        logger.info(f"Getting lifetime estimates: category={category}")
