...
Example:
    >>> from greenlang.upstream_leased_assets.setup import get_service
    >>> service = get_service()
...
Integration:
    >>> from greenlang.upstream_leased_assets.setup import get_router
...
class UpstreamLeasedAssetsService:
    ...
    def __init__(self) -> None:
        ...
        self._database_engine = self._init_engine(
            "greenlang.upstream_leased_assets.upstream_leased_database",
            "UpstreamLeasedDatabaseEngine",
        )
        self._building_engine = self._init_engine(
            "greenlang.upstream_leased_assets.building_calculator",
            "BuildingCalculatorEngine",
        )
        self._vehicle_engine = self._init_engine(
            "greenlang.upstream_leased_assets.vehicle_fleet_calculator",
            "VehicleFleetCalculatorEngine",
        )
        self._equipment_engine = self._init_engine(
            "greenlang.upstream_leased_assets.equipment_calculator",
            "EquipmentCalculatorEngine",
        )
        self._it_engine = self._init_engine(
            "greenlang.upstream_leased_assets.it_assets_calculator",
            "ITAssetsCalculatorEngine",
        )
        self._compliance_engine = self._init_engine(
            "greenlang.upstream_leased_assets.compliance_checker",
            "ComplianceCheckerEngine",
        )
        self._pipeline_engine = self._init_engine(
            "greenlang.upstream_leased_assets.upstream_leased_pipeline",
