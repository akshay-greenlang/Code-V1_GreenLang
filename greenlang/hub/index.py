...
class HubIndex:
    ...
    def __init__(self, index_url: Optional[str] = None, cache_dir: Optional[Path] = None):
        """
    ...
    def load(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if not force_refresh and self.cache_file.exists():
            try:
                ...
                cache_time = datetime.fromisoformat(cache_data.get("updated_at", "1970-01-01T00:00:00"))
        ...
        for url in self.index_urls:
            try:
                ...
                logger.info(f"Successfully loaded index with {len(index_data.get('packs', {}))} packs")
        ...
        self._index_data = {"version": "1.0", "updated_at": datetime.now().isoformat(), "packs": {}}
        return self._index_data
    ...
    def search(self, query: str = "", tags: List[str] = None, org: str = None) -> List[IndexEntry]:
        """
        ...
        for pack_key, pack_data in index_data.get("packs", {}).items():
            ...
            entry = IndexEntry(
                ...
                oci_ref=pack_data.get("oci_ref", f"ghcr.io/{pack_data.get('org', '')}/{pack_data.get('slug', '')}")
            )
    ...
    def create_local_index(self, output_path: Path, packs_dir: Path) -> None:
            ...
            try:
                ...
                if card_path.exists():
                    with open(card_path) as f:
                        ...
                        lines = f.readlines()[:5]
                        card_summary = " ".join(line.strip() for line in lines if line.strip())[:200]
        ...
        logger.info(f"Created index with {len(index_data['packs'])} packs at {output_path}")