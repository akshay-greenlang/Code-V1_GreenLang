...
@click.option('--registry', default='https://hub.greenlang.io', help='Registry URL')
def push(pack_path: str, tags: tuple, description: str, sign: bool, key: str, registry: str):
...
@click.option('--registry', default='https://hub.greenlang.io', help='Registry URL')
def search(query: str, tag: tuple, author: str, limit: int, output_json: bool, registry: str):
...
def init(pack_dir: str, name: str, version: str, description: str, 
...
@click.option('--sort', type=click.Choice(['relevance', 'downloads', 'stars', 
                                           'updated', 'created', 'name']),
              default='relevance', help='Sort order')
@click.option('--limit', default=20, help='Maximum results')
@click.option('--json', 'output_json', is_flag=True, help='Output as JSON')
def discover_search(query: str, category: tuple, tag: tuple, author: str,
                   license: str, min_stars: int, min_downloads: int,
                   verified: bool, official: bool, sort: str, limit: int,
                   output_json: bool):
    ...
    try:
        ...
        packs = index.search(query=query, filters=filters, 
...
def discover_recommend(installed: tuple, limit: int, output_json: bool):
    ...
    try:
        ...
        packs = index.get_recommendations(
            user_packs=list(installed) if installed else None,
            limit=limit
        )
