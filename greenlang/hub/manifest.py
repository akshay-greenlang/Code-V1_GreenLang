...
class PackManifest(BaseModel):
    ...
    # Compatibility
    greenlang_version: Optional[str] = Field(None, description="Required GreenLang version")
    ...
    def dict(self, **kwargs) -> Dict[str, Any]:
        ...
        if data.get('author'):
            data['author'] = data['author'] if isinstance(data['author'], dict) else data['author'].dict()
...
def load_manifest(pack_path: Path) -> PackManifest:
    ...
    if pack_path.is_file():
        ...
        manifest_path = None
        for name in ['manifest.json', 'manifest.yaml', 'manifest.yml', 'greenlang.json']:
...
def save_manifest(pack_path: Path, 
                 manifest,
...
def create_manifest(pack_path: Path,
...
def compare_manifests(manifest1: PackManifest, manifest2: PackManifest) -> Dict[str, Any]:
    """
