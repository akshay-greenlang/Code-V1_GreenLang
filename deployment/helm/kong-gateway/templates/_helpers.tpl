{{/*
=============================================================================
Kong API Gateway - Helm Template Helpers
GreenLang Climate OS | infra-006
=============================================================================
*/}}

{{/*
Expand the name of the chart.
Truncated to 63 characters because Kubernetes name fields are limited to
this length (DNS label standard, RFC 1123).
*/}}
{{- define "kong-gateway.name" -}}
{{- default .Chart.Name .Values.nameOverride | trunc 63 | trimSuffix "-" }}
{{- end }}

{{/*
Create a default fully qualified app name.
We truncate at 63 chars because some Kubernetes name fields are limited to
this (by the DNS naming spec). If release name contains the chart name it
will be used as a full name.
*/}}
{{- define "kong-gateway.fullname" -}}
{{- if .Values.fullnameOverride }}
{{- .Values.fullnameOverride | trunc 63 | trimSuffix "-" }}
{{- else }}
{{- $name := default .Chart.Name .Values.nameOverride }}
{{- if contains $name .Release.Name }}
{{- .Release.Name | trunc 63 | trimSuffix "-" }}
{{- else }}
{{- printf "%s-%s" .Release.Name $name | trunc 63 | trimSuffix "-" }}
{{- end }}
{{- end }}
{{- end }}

{{/*
Create chart name and version as used by the chart label.
*/}}
{{- define "kong-gateway.chart" -}}
{{- printf "%s-%s" .Chart.Name .Chart.Version | replace "+" "_" | trunc 63 | trimSuffix "-" }}
{{- end }}

{{/*
Common labels applied to every resource created by this chart.
*/}}
{{- define "kong-gateway.labels" -}}
helm.sh/chart: {{ include "kong-gateway.chart" . }}
{{ include "kong-gateway.selectorLabels" . }}
{{- if .Chart.AppVersion }}
app.kubernetes.io/version: {{ .Chart.AppVersion | quote }}
{{- end }}
app.kubernetes.io/managed-by: {{ .Release.Service }}
app.kubernetes.io/part-of: greenlang-climate-os
app.kubernetes.io/component: api-gateway
{{- if .Values.global }}
{{- if .Values.global.environment }}
greenlang.io/environment: {{ .Values.global.environment }}
{{- end }}
{{- end }}
{{- end }}

{{/*
Selector labels used in matchLabels and service selectors.
*/}}
{{- define "kong-gateway.selectorLabels" -}}
app.kubernetes.io/name: {{ include "kong-gateway.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end }}

{{/*
Create the name of the service account to use.
*/}}
{{- define "kong-gateway.serviceAccountName" -}}
{{- if .Values.serviceAccount }}
{{- if .Values.serviceAccount.create }}
{{- default (include "kong-gateway.fullname" .) .Values.serviceAccount.name }}
{{- else }}
{{- default "default" .Values.serviceAccount.name }}
{{- end }}
{{- else }}
{{- include "kong-gateway.fullname" . }}
{{- end }}
{{- end }}

{{/*
Return the appropriate image tag.
*/}}
{{- define "kong-gateway.imageTag" -}}
{{- default .Chart.AppVersion .Values.kong.image.tag }}
{{- end }}

{{/*
Return the full image reference.
*/}}
{{- define "kong-gateway.image" -}}
{{- printf "%s:%s" .Values.kong.image.repository (include "kong-gateway.imageTag" .) }}
{{- end }}

{{/*
Return the namespace to deploy into.
*/}}
{{- define "kong-gateway.namespace" -}}
{{- default .Release.Namespace .Values.namespaceOverride }}
{{- end }}

{{/*
Annotations shared across all resources.
*/}}
{{- define "kong-gateway.annotations" -}}
greenlang.io/project: {{ default "greenlang" .Values.global.project }}
greenlang.io/component: {{ default "api-gateway" .Values.global.component }}
{{- end }}
