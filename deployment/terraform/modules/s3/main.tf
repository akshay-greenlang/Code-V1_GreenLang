...
# -----------------------------------------------------------------------------
# Reports Bucket (CSRD/CBAM/EUDR compliance reports)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Data Lake Raw Bucket (unprocessed ingestion data)
# -----------------------------------------------------------------------------
resource "aws_s3_bucket" "data_lake_raw" {
  count = var.create_data_lake_buckets ? 1 : 0

  bucket = "${var.name_prefix}-data-lake-raw-${data.aws_caller_identity.current.account_id}"

  tags = merge(var.tags, {
    Name    = "${var.name_prefix}-data-lake-raw"
...
resource "aws_s3_bucket_versioning" "data_lake_raw" {
  ...
  bucket = aws_s3_bucket.data_lake_raw[0].id
...
# -----------------------------------------------------------------------------
# Data Lake Processed Bucket (transformed/curated data)
# -----------------------------------------------------------------------------
resource "aws_s3_bucket" "data_lake_processed" {
  count = var.create_data_lake_buckets ? 1 : 0

  bucket = "${var.name_prefix}-data-lake-processed-${data.aws_caller_identity.current.account_id}"

  tags = merge(var.tags, {
    Name    = "${var.name_prefix}-data-lake-processed"
...
resource "aws_s3_bucket_versioning" "data_lake_processed" {
  ...
  bucket = aws_s3_bucket.data_lake_processed[0].id
...
# Static assets lifecycle policy
...
# Access logging for new buckets
resource "aws_s3_bucket_logging" "reports" {
  count = var.create_reports_bucket && var.create_logs_bucket && var.enable_access_logging ? 1 : 0

  bucket        = aws_s3_bucket.reports[0].id
  target_bucket = aws_s3_bucket.logs[0].id
  target_prefix = "s3-access-logs/reports/"
}

resource "aws_s3_bucket_logging" "data_lake_raw" {
  count = var.create_data_lake_buckets && var.create_logs_bucket && var.enable_access_logging ? 1 : 0

  bucket        = aws_s3_bucket.data_lake_raw[0].id
  target_bucket = aws_s3_bucket.logs[0].id
  target_prefix = "s3-access-logs/data-lake-raw/"
}

resource "aws_s3_bucket_logging" "data_lake_processed" {
  count = var.create_data_lake_buckets && var.create_logs_bucket && var.enable_access_logging ? 1 : 0

  bucket        = aws_s3_bucket.data_lake_processed[0].id
  target_bucket = aws_s3_bucket.logs[0].id
  target_prefix = "s3-access-logs/data-lake-processed/"
}
