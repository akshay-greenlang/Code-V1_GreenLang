    ...
    def update_status(
        ...
        if task_id in self.state:
            ...
            if status in [TaskStatus.COMPLETED, TaskStatus.FAILED]:
                execution.end_time = DeterministicClock.now()
            self._persist_state()
...
class ProgressTracker:
    ...
    def update_progress(
        ...
        self.progress[task_id] = {
            ...
            "metrics": metrics or {},
            "last_updated": DeterministicClock.now()
        }
...
class TaskExecutor:
    ...
    async def _default_executor(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            checkpoint_id = await self.checkpoint_manager.save_checkpoint(
                task.task_id,
                {"task": task, "timestamp": DeterministicClock.now()}
            )
    ...
    async def cleanup_old_checkpoints(self, days: int = 7) -> int:
        ...
        cutoff = DeterministicClock.now() - timedelta(days=days)
        cleaned = 0
