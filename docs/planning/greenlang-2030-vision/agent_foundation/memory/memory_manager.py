...
class MemoryManager:
    ...
    async def consolidate_memories(
        ...
        try:
            ...
            self.metrics.last_consolidation = DeterministicClock.now()
            self.stats["consolidations_performed"] += 1

        except Exception as e:
            logger.error(f"Consolidation failed: {e}")

        processing_time = (DeterministicClock.now() - start_time).total_seconds()
    ...
    async def prune_memories(
        ...
        try:
            ...
            self.metrics.last_pruning = DeterministicClock.now()
