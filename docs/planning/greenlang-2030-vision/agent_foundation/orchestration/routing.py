...
class RouteRule(BaseModel):
    ...
    rule_id: str = Field(default_factory=lambda: str(deterministic_uuid(__name__, str(DeterministicClock.now()))))
...
class MessageRouter:
    ...
    async def _apply_strategy(
        ...
        if strategy == RoutingStrategy.ROUND_ROBIN:
            ...
            return [deterministic_random().choice(candidates)]
    ...
    def _affinity_based(self, message: Message, candidates: List[str]) -> List[str]:
        ...
        target = deterministic_random().choice(candidates)
