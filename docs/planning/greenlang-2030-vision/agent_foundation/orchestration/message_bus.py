...
class MessageMetadata(BaseModel):
    ...
    correlation_id: str = Field(default_factory=lambda: str(deterministic_uuid(__name__, str(DeterministicClock.now()))))
...
class Message(BaseModel):
    ...
    message_id: str = Field(default_factory=lambda: str(deterministic_uuid(__name__, str(DeterministicClock.now()))))
...
class MessageBus:
    ...
    async def subscribe(
        ...
        Yields:
            Incoming messages
        """
        consumer_id = f"{group_id or 'default'}_{deterministic_uuid(__name__, str(DeterministicClock.now())).hex[:8]}"
