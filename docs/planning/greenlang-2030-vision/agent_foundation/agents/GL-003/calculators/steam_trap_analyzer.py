...
class SteamTrapAnalyzer:
    ...
    def analyze_trap(
        ...
        return TrapAnalysisResult(
            ...
            steam_loss_rate_kg_hr=FinancialDecimal.from_string(steam_loss),
            energy_loss_rate_kw=FinancialDecimal.from_string(energy_loss_kw),
    ...
    def analyze_trap_population(
        ...
        return {
            ...
            'total_steam_loss_kg_hr': FinancialDecimal.from_string(total_loss_kg_hr),
            'total_annual_cost_loss': FinancialDecimal.from_string(total_cost),
            'high_priority_repairs': len(high_priority),
            'medium_priority_repairs': len(medium_priority),
            'estimated_payback_months': self._calculate_payback_period(FinancialDecimal.from_string(total_cost)),
            'recommendations': self._generate_fleet_recommendations(
                len(traps),
                failure_count,
                FinancialDecimal.from_string(total_cost)
            )
        }
