...
class DistributionEfficiencyCalculator:
    ...
    def calculate_distribution_efficiency(
        ...
        tracker.record_step(
            ...
            inputs={'segment_losses': [FinancialDecimal.from_string(l['total_loss_kw']) for l in segment_losses]},
        ...
        economic_thickness = self._calculate_economic_insulation_thickness(
            ...
            FinancialDecimal.from_string(annual_cost / Decimal(str(total_length))) if total_length > 0 else 0,
            tracker
        )
        ...
        result = DistributionResults(
            total_heat_loss_kw=FinancialDecimal.from_string(total_heat_loss_kw),
            ...
            annual_cost_loss=FinancialDecimal.from_string(annual_cost),
    ...
    def _calculate_segment_heat_loss(
        ...
        result = {
            ...
            'thermal_resistance_km_w': FinancialDecimal.from_string(R_total.quantize(Decimal('0.0001'), rounding=ROUND_HALF_UP))
        }
    ...
    def _generate_recommendations(
        ...
        avg_loss = sum(FinancialDecimal.from_string(l['loss_per_meter_w']) for l in losses) / len(losses) if losses else 0
