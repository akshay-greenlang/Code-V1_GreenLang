...
def sample_leak_scenario():
    ...
    def _generate(severity='medium'):
        ...
        return {
            'location': f"Section-{deterministic_random().randint(1, 10)}-Valve-{deterministic_random().randint(1, 20)}",
            'timestamp': DeterministicClock.utcnow(),
            'severity': severity,
            'estimated_loss_kg_hr': scenario['loss_rate'],
            'priority': scenario['priority'],
            'confidence': random.uniform(0.75, 0.99),
            'detection_method': deterministic_random().choice(['pressure_drop', 'thermal_imaging', 'acoustic']),
...
def sample_steam_trap_data():
    ...
    def _generate(num_traps=20):
        ...
        for i in range(num_traps):
            status = deterministic_random().choice(statuses)

            trap_data = {
                'trap_id': f"ST-{i+1:03d}",
                'location': f"Line-{deterministic_random().randint(1, 5)}-Segment-{deterministic_random().randint(1, 10)}",
                'timestamp': DeterministicClock.utcnow(),
                'status': status,
                ...
                'last_maintenance': DeterministicClock.utcnow() - timedelta(days=deterministic_random().randint(30, 365)),
...
def sample_time_series_data():
    ...
    def _generate(duration_hours=24, interval_minutes=5):
        data_points = []
        start_time = DeterministicClock.utcnow() - timedelta(hours=duration_hours)
...
def performance_monitor():
    ...
    class PerformanceMonitor:
        ...
        def checkpoint(self, name: str):
            """Record a checkpoint."""
            if self.start_time:
                elapsed = (DeterministicClock.utcnow() - self.start_time).total_seconds()
