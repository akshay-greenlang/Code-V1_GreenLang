#!/bin/bash
# GL-011 FUELCRAFT - Rollback Script
# Rolls back GL-011 deployment to previous revision
...
print_banner() {
    echo ""
    echo "=================================================="
    echo "  GL-011 FUELCRAFT Kubernetes Rollback"
    echo "=================================================="
    echo ""
...
usage() {
    ...
    echo "  EMERGENCY=true           Skip confirmations (emergency mode)"
    echo ""
    echo "Examples:"
    ...
    echo "  EMERGENCY=true $0 prod     # Emergency rollback (no confirmation)"
    echo ""
    exit 1
}
...
check_prerequisites() {
    ...
    if ! kubectl get deployment gl-011-fuelcraft -n "$NAMESPACE" &> /dev/null; then
        log_error "Deployment 'gl-011-fuelcraft' not found in namespace '$NAMESPACE'."
        exit 1
    fi
...
show_rollout_history() {
    ...
    kubectl rollout history deployment/gl-011-fuelcraft -n "$NAMESPACE"
...
get_revision_details() {
    ...
    local current_revision=$(kubectl rollout history deployment/gl-011-fuelcraft -n "$NAMESPACE" | tail -n 1 | awk '{print $1}')
    ...
    if ! kubectl rollout history deployment/gl-011-fuelcraft -n "$NAMESPACE" --revision="$REVISION" &> /dev/null; then
    ...
    kubectl rollout history deployment/gl-011-fuelcraft -n "$NAMESPACE" --revision="$REVISION"
}
...
confirm_rollback() {
    ...
    if [ "$ENVIRONMENT" = "production" ]; then
        ...
        echo ""
        echo "You are about to rollback GL-011 FUELCRAFT in PRODUCTION environment."
        echo "This will affect live fuel optimization operations."
        echo ""
        echo "Environment:  $ENVIRONMENT"
        echo "Namespace:    $NAMESPACE"
...
execute_rollback() {
    ...
    if [ -n "$REVISION" ]; then
        ...
        if kubectl rollout undo deployment/gl-011-fuelcraft -n "$NAMESPACE" --to-revision="$REVISION" $dry_run_flag; then
        ...
        if kubectl rollout undo deployment/gl-011-fuelcraft -n "$NAMESPACE" $dry_run_flag; then
            log_success "Rollback initiated"
        else
            log_error "Rollback failed"
            exit 1
...
verify_rollback() {
    ...
    local new_revision=$(kubectl rollout history deployment/gl-011-fuelcraft -n "$NAMESPACE" | tail -n 1 | awk '{print $1}')
