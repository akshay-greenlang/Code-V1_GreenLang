...
class TestSecurity(AgentTestCase):
    ...
    def test_jwt_authentication(self):
        ...
        payload = {
            'agent_id': 'GL-001',
            'user_id': 'user123',
            'exp': DeterministicClock.utcnow() + timedelta(hours=1),
            'iat': DeterministicClock.utcnow(),
        ...
        expired_payload = {
            'agent_id': 'GL-001',
            'exp': DeterministicClock.utcnow() - timedelta(hours=1),
            'iat': DeterministicClock.utcnow() - timedelta(hours=2)
        }
    ...
    def test_audit_logging(self):
        ...
        # Log security events
        security_events = [
            {'event': 'authentication_success', 'user': 'user123', 'timestamp': DeterministicClock.utcnow()},
            {'event': 'authorization_failure', 'user': 'user456', 'resource': 'GL-001', 'timestamp': DeterministicClock.utcnow()},
            {'event': 'suspicious_activity', 'ip': '192.168.1.100', 'details': 'Multiple failed attempts', 'timestamp': DeterministicClock.utcnow()}
        ]
