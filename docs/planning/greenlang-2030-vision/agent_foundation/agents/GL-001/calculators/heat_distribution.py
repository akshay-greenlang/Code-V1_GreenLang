...
class HeatDistributionOptimizer:
    ...
    def optimize(
        ...
        result = {
            ...
            'total_heat_delivered_kw': FinancialDecimal.from_string(solution['total_heat_delivered']),
            'total_heat_losses_kw': FinancialDecimal.from_string(sum(heat_losses.values())),
            'total_cost_per_hour': FinancialDecimal.from_string(total_cost),
    ...
    def _solve_optimization(self, model: Dict, tracker: ProvenanceTracker) -> Dict:
        ...
        for i in range(num_sources):
            ...
            source_utilization[f"source_{i}"] = FinancialDecimal.from_string(total_from_source)
    ...
    def _verify_energy_balance(
        ...
        energy_balance = {
            ...
            'total_demand_kw': FinancialDecimal.from_string(total_demand),
        ...
        tracker.record_step(
            ...
            inputs={
                'input': FinancialDecimal.from_string(total_input),
                'output': FinancialDecimal.from_string(total_demand),
                'losses': FinancialDecimal.from_string(total_losses)
            },
