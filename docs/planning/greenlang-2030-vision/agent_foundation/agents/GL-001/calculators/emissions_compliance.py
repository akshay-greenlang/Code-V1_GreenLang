    ...
    def _correct_to_reference_o2(
        ...
        for measurement in measurements:
            ...
            corrected.append({
                ...
                'correction_factor': FinancialDecimal.from_string(correction_factor),
        ...
        for limit in limits:
            ...
            if key in averages:
                ...
                results.append({
                    ...
                    'exceedance_factor': FinancialDecimal.from_string(value / limit_value) if limit_value > 0 else 0
                })
    ...
    def _calculate_total_emissions(
        ...
        for m in measurements:
            ...
            if pollutant not in totals:
                totals[pollutant] = 0
            totals[pollutant] += FinancialDecimal.from_string(total_kg)
        ...
        if data.fuel_type in self.EMISSION_FACTORS:
            ...
            for pollutant, factor in factors.items():
                if pollutant == 'CO2':
                    ...
                    totals['CO2'] = FinancialDecimal.from_string(co2_kg)
    ...
    def _calculate_emission_intensities(
        ...
        for pollutant, total_kg in totals.items():
            ...
            intensities[pollutant] = {
                'kg_per_mwh': FinancialDecimal.from_string(total / energy_output_mwh) if energy_output_mwh > 0 else 0,
                'kg_per_tonne_fuel': FinancialDecimal.from_string(total / fuel_consumption_tonnes) if fuel_consumption_tonnes > 0 else 0,
                'g_per_kwh': FinancialDecimal.from_string((total * 1000) / (energy_output_mwh * 1000)) if energy_output_mwh > 0 else 0
            }
