...
class FuelOptimizationCalculator:
    ...
    def calculate_fuel_to_steam_efficiency(
        ...
        result = {
            ...
            'fuel_consumption_kg_hr': FinancialDecimal.from_string(fuel_rate_kg_hr),
            'specific_fuel_consumption': FinancialDecimal.from_string(specific_consumption),
            'steam_cost_per_kg': FinancialDecimal.from_string(steam_cost_per_kg),
            'fuel_cost_per_hr': FinancialDecimal.from_string(fuel_cost_per_hr),
            'btu_efficiency_percent': float(btu_efficiency),
            'heat_rate_kj_per_kg_steam': FinancialDecimal.from_string(heating_value * specific_consumption),
    ...
    def optimize_fuel_blend(
        ...
        if best_blend.get('feasible', True):
            result = {
                ...
                'total_fuel_cost_per_hr': FinancialDecimal.from_string(best_blend['cost']),
                'total_fuel_rate_kg_hr': FinancialDecimal.from_string(best_blend['total_fuel_rate']),
                ...
                'co2_emissions_kg_hr': FinancialDecimal.from_string(best_blend['co2_emissions']),
    ...
    def calculate_btu_optimization(
        ...
        result = {
            ...
            'total_recoverable_heat_kj_kg': FinancialDecimal.from_string(total_recoverable),
    ...
    def _evaluate_single_fuel(
        ...
        # Check availability
        if FinancialDecimal.from_string(fuel_rate) > fuel.availability_kg_per_hr:
            return None
    ...
    def _optimize_two_fuel_blend(
        ...
        for blend1_percent in range(0, 101, 5):
            ...
            # Check availability
            if FinancialDecimal.from_string(fuel1_rate) > fuel1.availability_kg_per_hr:
                continue
            if FinancialDecimal.from_string(fuel2_rate) > fuel2.availability_kg_per_hr:
                continue
