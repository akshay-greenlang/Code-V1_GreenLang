...
class EmissionsCalculator:
    ...
    def calculate_carbon_tax(
        ...
        tracker = ProvenanceTracker(
            calculation_id=f"carbon_tax_{DeterministicClock.now().timestamp()}",
        ...
        result = {
            'total_co2_tonnes': FinancialDecimal.from_string(co2),
            'taxable_co2_tonnes': float(taxable_co2),
            'exempt_co2_tonnes': float(co2 - taxable_co2),
            'carbon_price_per_tonne': FinancialDecimal.from_string(price),
            'total_carbon_tax': FinancialDecimal.from_string(carbon_tax),
            'effective_rate_per_tonne': FinancialDecimal.from_string(carbon_tax / co2) if co2 > 0 else 0,
