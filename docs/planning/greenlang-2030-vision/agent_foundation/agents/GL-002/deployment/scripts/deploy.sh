#!/bin/bash
# GL-002 BoilerEfficiencyOptimizer - Deployment Script
# Deploys GL-002 to Kubernetes with pre-flight checks and verification
...
print_banner() {
    echo ""
    echo "=================================================="
    echo "  GL-002 Kubernetes Deployment"
    echo "=================================================="
    echo ""
...
pre_flight_checks() {
    ...
    if kubectl get secret gl-002-secrets -n "$NAMESPACE" &> /dev/null; then
        log_success "Secrets exist"
    else
        log_error "Secrets not found. Please create gl-002-secrets before deploying."
        echo ""
        echo "To create secrets, run:"
        echo "  kubectl create secret generic gl-002-secrets \\"
    ...
    log_info "Skipping Docker image check (assumes image exists in registry)"
    ...
    if kubectl get deployment gl-002-boiler-efficiency -n "$NAMESPACE" &> /dev/null; then
        ...
        CURRENT_REPLICAS=$(kubectl get deployment gl-002-boiler-efficiency -n "$NAMESPACE" -o jsonpath='{.spec.replicas}')
        CURRENT_IMAGE=$(kubectl get deployment gl-002-boiler-efficiency -n "$NAMESPACE" -o jsonpath='{.spec.template.spec.containers[0].image}')
...
verify_deployment() {
    ...
    local service_ip=$(kubectl get service gl-002-boiler-efficiency -n "$NAMESPACE" -o jsonpath='{.spec.clusterIP}')

    if [ -n "$service_ip" ]; then
        log_info "Service IP: $service_ip"
        # Note: This requires network access to the cluster
        # kubectl run curl-test --image=curlimages/curl:latest --rm -it --restart=Never -- curl -f http://${service_ip}/api/v1/health
    fi
}
...
post_deployment() {
    ...
    log_info "Service:"
    kubectl get service gl-002-boiler-efficiency -n "$NAMESPACE"

    log_info "HPA:"
    kubectl get hpa gl-002-boiler-efficiency-hpa -n "$NAMESPACE" 2>/dev/null || log_warning "HPA not found"

    log_info "PDB:"
    kubectl get pdb gl-002-boiler-efficiency-pdb -n "$NAMESPACE" 2>/dev/null || log_warning "PDB not found"
...
main() {
    ...
    echo "     Then visit: http://localhost:8000/api/v1/health"
    echo ""
}
