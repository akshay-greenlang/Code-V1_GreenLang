...
class MemoryItem(BaseModel):
    ...
    def calculate_provenance(cls, v, values):
        ...
        if not v and 'content' in values:
            ...
            timestamp_str = values.get('timestamp', datetime.now()).isoformat()
...
class WorkingMemory(BaseModel):
    ...
    def _cleanup_expired(self) -> None:
        ...
        cutoff = datetime.now() - timedelta(minutes=self.duration_minutes)
        ...
        try:
            ...
            for key in keys:
                ...
                if value:
                    ...
                    if item.timestamp > datetime.now() - timedelta(minutes=self.working_memory.duration_minutes):
