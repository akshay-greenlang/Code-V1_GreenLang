...
class MockOPCUAServer:
    ...
    def _initialize_nodes(self):
        ...
        for trap_num in range(1, 21):
            self.nodes[f'ns=2;s=TRAP.ST{trap_num:03d}.STATUS'] = {
                'value': random.choice([1, 1, 1, 2, 3]),  # 1=OK, 2=WARN, 3=FAIL
    ...
    async def _notify_subscribers(self, node_id: str, value: Any):
        ...
        for callback in self.subscriptions.get(node_id, []):
            try:
                await callback(node_id, value, datetime.utcnow())
...
class MockModbusServer:
    ...
    def _initialize_registers(self):
        ...
        for i in range(1, 21):
            ...
            self.registers[40100 + i] = random.choice([1, 1, 1, 2, 3])
...
class MockPressureSensorServer:
    ...
    def _initialize_config(self):
        ...
        return {
            ...
            'quality': 'GOOD',
            'last_calibration': datetime.utcnow() - timedelta(days=180)
        }
    ...
    async def get_diagnostics(self) -> Dict[str, Any]:
        ...
        days_since_cal = (datetime.utcnow() - self.config['last_calibration']).days
...
class MockTemperatureSensorServer:
    ...
    def _initialize_config(self):
        ...
        return {
            'sensor_id': self.sensor_id,
            'sensor_type': random.choice(['PT100', 'K-Type', 'J-Type']),
            'range_min': -50.0,
            'range_max': 500.0,
            'unit': 'degC',
            'current_temperature': 184.0,
            'quality': 'GOOD',
            'last_calibration': datetime.utcnow() - timedelta(days=200)
        }
    ...
    async def _publish_loop(self):
        ...
        while self.running:
            ...
            if random.random() < 0.05:  # 5% chance
                leak_alert = {
                    'timestamp': datetime.utcnow().isoformat(),
                    'location': f"Valve-{random.randint(1, 50)}",
