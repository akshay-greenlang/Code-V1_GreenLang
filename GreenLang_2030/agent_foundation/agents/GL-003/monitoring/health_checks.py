"""
Health check endpoints for GL-003 SteamSystemAnalyzer.
...
class HealthChecker:
    """
    Comprehensive health check system for GL-003.
    ...
    async def _check_external_apis(self) -> ComponentHealth:
        ...
        start = time.time()
        try:
            # Check SCADA, Steam Flow Monitoring, Leak Detection APIs
            ...
            return ComponentHealth(
                ...
                details={
                    "scada": "connected",
                    "steam_flow_monitoring": "connected",
                    "leak_detection": "connected",
                    "condensate_tracking": "connected",
                }
            )
        except Exception as e:
            latency = (time.time() - start) * 1000
