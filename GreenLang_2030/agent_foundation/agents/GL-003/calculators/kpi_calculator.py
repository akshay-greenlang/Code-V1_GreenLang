...
class KPICalculator:
    ...
    def calculate_kpis(
        ...
        return KPIDashboard(
            ...
            specific_steam_consumption_gj_per_tonne=float(specific_consumption),
            total_losses_percent=float(losses['total']),
            distribution_loss_percent=float(losses['distribution']),
            trap_loss_percent=float(losses['traps']),
            condensate_return_rate_percent=float(condensate_return),
            steam_trap_failure_rate_percent=float(trap_kpis['failure_rate']),
            trap_performance_index=float(trap_kpis['performance_index']),
            total_savings_opportunity_gj=float(savings['total_gj']),
            total_savings_opportunity_percent=float(savings['percent']),
