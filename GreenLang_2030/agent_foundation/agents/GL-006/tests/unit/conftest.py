...
def mock_pi_historian():
    ...
    mock = Mock()
    mock.get_tag_value = Mock(return_value={"value": 75.5, "timestamp": datetime.now()})
    mock.get_tag_history = Mock(return_value=[
        {"value": 75.0 + i * 0.1, "timestamp": datetime.now() - timedelta(minutes=i)}
        for i in range(60)
    ])
    ...
    def generate_exchanger_network(num_exchangers: int = 5) -> List[Dict[str, Any]]:
        ...
        for i in range(num_exchangers):
            exchangers.append({
                ...
                "hot_side_fluid": random.choice(["steam", "oil", "water", "glycol"]),
                "cold_side_fluid": random.choice(["water", "air", "process_fluid"]),
                "age_years": random.uniform(0, 15),
                "last_cleaned": datetime.now() - timedelta(days=random.randint(30, 365))
            })
