...
class CombustionAnalyzerConnector:
    ...
    async def calibrate_analyzer(self) -> bool:
        ...
        try:
            ...
            self.status.last_calibration_time = datetime.now()
            self.status.next_calibration_time = datetime.now() + timedelta(
        ...
        while self.status.connected:
            try:
                ...
                if self.status.next_calibration_time:
                    time_until_cal = (
                        self.status.next_calibration_time - datetime.now()
                    ).total_seconds()
            ...
            try:
                ...
                if self.status.last_reading_time:
                    seconds_since_reading = (
                        datetime.now() - self.status.last_reading_time
                    ).total_seconds()
