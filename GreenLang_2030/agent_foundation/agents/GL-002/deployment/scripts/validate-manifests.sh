#!/bin/bash
# GL-002 BoilerEfficiencyOptimizer - Manifest Validation Script
...
print_banner() {
    echo ""
    echo "=================================================="
    echo "  GL-002 Kubernetes Manifest Validation"
    echo "=================================================="
    echo ""
}
...
validate_resource_constraints() {
    ...
    ((TOTAL_TESTS++))
    # Check if CPU requests/limits are reasonable
    local cpu_request=$(grep -A 1 "requests:" "$deployment" | grep "cpu:" | awk '{print $2}' | tr -d '"' | head -n1)
    local cpu_limit=$(grep -A 1 "limits:" "$deployment" | grep "cpu:" | awk '{print $2}' | tr -d '"' | head -n1)

    if [ -n "$cpu_request" ] && [ -n "$cpu_limit" ]; then
        log_success "CPU resources defined: request=$cpu_request, limit=$cpu_limit"
    else
        log_warning "CPU resources not fully defined"
    fi

    ((TOTAL_TESTS++))
    # Check if memory requests/limits are reasonable
    local mem_request=$(grep -A 2 "requests:" "$deployment" | grep "memory:" | awk '{print $2}' | tr -d '"' | head -n1)
    local mem_limit=$(grep -A 2 "limits:" "$deployment" | grep "memory:" | awk '{print $2}' | tr -d '"' | head -n1)

    if [ -n "$mem_request" ] && [ -n "$mem_limit" ]; then
        log_success "Memory resources defined: request=$mem_request, limit=$mem_limit"
    else
        log_warning "Memory resources not fully defined"
    fi
