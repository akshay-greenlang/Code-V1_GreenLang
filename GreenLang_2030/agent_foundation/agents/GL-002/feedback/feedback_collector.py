...
class FeedbackCollector:
    ...
    async def _check_and_create_alerts(
        ...
        if feedback.rating <= 2:
            alert = FeedbackAlert(
                alert_id=f"low_rating_{feedback.optimization_id}_{int(datetime.utcnow().timestamp())}",
                severity="warning",
        ...
        if feedback.savings_accuracy is not None and feedback.savings_accuracy < self.alert_threshold_accuracy:
            alert = FeedbackAlert(
                alert_id=f"low_accuracy_{feedback.optimization_id}_{int(datetime.utcnow().timestamp())}",
                severity="warning",
