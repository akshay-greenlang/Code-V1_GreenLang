...
class StructuredLogger:
    ...
    def _setup_cloudwatch(self, config: Dict[str, Any]):
        ...
        self.cloudwatch_stream = config.get(
            'log_stream',
            f"{self.name}-{datetime.utcnow().strftime('%Y%m%d')}"
        )
    ...
    def _create_log_entry(
        ...
        entry = {
            ...
            'logger': self.name,
            'message': message,
            'correlation_id': LogContext.get_correlation_id() or str(uuid.uuid4()),
    ...
    def _log_to_elasticsearch(self, entry: Dict[str, Any]):
        ...
        self.es_buffer.append({
            '_index': f"{self.es_index}-{datetime.utcnow().strftime('%Y.%m.%d')}",
            '_source': entry
        })
