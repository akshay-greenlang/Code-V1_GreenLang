# -*- coding: utf-8 -*-
"""
Unit tests for ProvenanceTracker - AGENT-EUDR-030
...
class TestComputeHashBytes:
    ...
    def test_hash_bytes_different_data(self):
        ...
        h1 = ProvenanceTracker.compute_hash_bytes(b"data1")
        h2 = ProvenanceTracker.compute_hash_bytes(b"data2")
        assert h1 != h2
...
class TestCreateEntry:
    ...
    def test_create_entry_returns_dict(self, provenance_tracker):
        ...
        assert entry["step"] == "generate_dds"
        assert entry["source"] == "eudr_027"
        assert entry["input_hash"] == GENESIS_HASH
        assert entry["output_hash"] == "a" * 64
        assert entry["actor"] == "AGENT-EUDR-030"
...
class TestVerifyChain:
    ...
    def test_chain_with_missing_hash_fields(self, provenance_tracker):
        ...
        # Should handle gracefully - empty strings are falsy
...
class TestBuildChain:
    ...
    def test_build_chain_multiple_steps_linked(self, provenance_tracker):
        ...
        chain = provenance_tracker.build_chain(steps)
        assert len(chain) == 3
        # Each entry's input_hash should match previous entry's output_hash
        assert chain[0]["input_hash"] == GENESIS_HASH
    ...
    def test_build_chain_preserves_step_info(self, provenance_tracker):
        ...
        steps = [
            {"step": "generate_dds", "source": "eudr_027", "data": {"test": 1}},
            {"step": "validate_dds", "source": "eudr_030", "data": {"test": 2}},
        ]
        chain = provenance_tracker.build_chain(steps)
        assert chain[0]["step"] == "generate_dds"
        assert chain[0]["source"] == "eudr_027"
        assert chain[1]["step"] == "validate_dds"
        assert chain[1]["source"] == "eudr_030"
