# -*- coding: utf-8 -*-
"""
Unit Tests for ProvenanceTracker (AGENT-FOUND-008)
...
class ProvenanceTracker:
    """Tracks provenance chains for verifications and computations."""
        ...
        for i in range(1, len(chain)):
            ...
            if actual_prev != expected_prev:
                return {
                    ...
                    "is_valid": False,
                    "length": len(chain),
                    "message": f"Chain broken at entry {i}: expected {expected_prev[:16]}..., "
                               f"got {actual_prev[:16]}...",
                    "tampered_at": i,
                }

        # Verify chain hashes are valid
...
class TestRecord:
    ...
    def test_record_creates_entry(self):
        ...
        entry = tracker.record("entity-001", "verification", "hash_computed", "abc123")
        assert entry.entity_id == "entity-001"
        assert entry.entity_type == "verification"
        assert entry.action == "hash_computed"
...
class TestVerifyChain:
    ...
    def test_verify_chain_tampered_hash_recompute(self):
        tracker = ProvenanceTracker()
        tracker.record("e1", "test", "create", "h1")
        # Tamper with data after recording
...
class TestMultipleEntities:
    """Test provenance with multiple entities."""
