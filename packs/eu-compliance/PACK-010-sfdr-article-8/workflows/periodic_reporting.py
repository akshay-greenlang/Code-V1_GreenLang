...
Five-phase workflow for generating SFDR Annex IV periodic disclosures for
Article 8 financial products. Orchestrates data collection, performance
assessment, PAI calculation, template generation, and filing package
assembly into a single auditable pipeline.

Regulatory Context:
    Per EU SFDR Regulation 2019/2088 and Delegated Regulation 2022/1288 (RTS):
    - Article 11: Periodic disclosure requirements for Article 8 products.
    - Annex IV: Template for Article 8 periodic disclosures.
    - Reporting must cover the entire reference period (typically 1 year).
    - Comparison of actual E/S characteristics attainment vs pre-contractual
      commitments is mandatory.
    - All applicable PAI indicators must be reported with year-over-year data.
    - Top 15 investments by value must be disclosed.
    - Sector and geographic allocation breakdowns required.

Phases:
    1. DataCollection - Gather portfolio holdings, emissions/ESG data, and
       governance assessments at the reporting date
    2. PerformanceAssessment - Measure actual E/S characteristics attainment
       vs pre-contractual commitments, calculate sustainable investment proportion
    3. PAICalculation - Calculate all applicable PAI indicators, year-over-year
       comparison, and coverage ratio assessment
    4. TemplateGeneration - Generate Annex IV template with actual figures,
       top 15 investments, sector/geographic allocation
    5. FilingPackage - Assemble filing-ready package with supporting evidence
...
class PortfolioHolding(BaseModel):
    ...
    isin: Optional[str] = Field(None, description="ISIN of the security")
    sector: str = Field(default="", description="NACE sector classification")
    country: str = Field(default="", description="Country of domicile (ISO)")
    ...
    is_sustainable_investment: bool = Field(default=False)
...
class PAIIndicatorData(BaseModel):
    ...
    category: str = Field(default="climate", description="PAI category")
...
class PrecontractualCommitment(BaseModel):
    ...
    name: str = Field(..., description="Commitment name")
    commitment_type: str = Field(
        default="minimum_proportion",
        description="Type: minimum_proportion, exclusion, threshold"
    )
...
class PeriodicReportingInput(BaseModel):
    ...
    portfolio_holdings: List[PortfolioHolding] = Field(
        default_factory=list, description="Holdings at reporting date"
    )
    pai_indicators: List[PAIIndicatorData] = Field(
        default_factory=list, description="PAI indicator data"
    )
    precontractual_commitments: List[PrecontractualCommitment] = Field(
        default_factory=list, description="Commitments to track"
    )
    total_portfolio_value_eur: float = Field(
        default=0.0, ge=0.0, description="Total portfolio NAV"
    )
    ...
    reference_benchmark_performance: Optional[Dict[str, Any]] = Field(None)
...
class PeriodicReportingResult(WorkflowResult):
    ...
    commitments_met: int = Field(default=0)
    commitments_total: int = Field(default=0)
...
class DataCollectionPhase:
    ...
    Gathers portfolio holdings at reporting date, collects emissions and
    ESG data for PAI calculations, and compiles governance assessments.
    """
    ...
    async def execute(self, context: WorkflowContext) -> PhaseResult:
        ...
        Args:
            context: Workflow context with portfolio data.
        ...
        try:
            ...
            if reported_pct < 50.0:
                warnings.append(
                    f"Only {reported_pct:.1f}% of holdings have reported "
                    f"(non-estimated) data. Consider improving data sourcing."
                )
            ...
            outputs["top_15_investments"] = [
                {
                    ...
                    "portfolio_weight_pct": h.get("portfolio_weight_pct", 0.0),
            ...
            outputs["pai_data_completeness_pct"] = round(
                pai_with_data / max(len(pai_indicators), 1) * 100, 1
            )
            ...
            for h in holdings:
                ...
                if s2 is not None:
                    total_scope2 += s2 * (
                        h.get("portfolio_weight_pct", 0.0) / 100.0
                    )
                if s3 is not None:
                    total_scope3 += s3 * (
                        h.get("portfolio_weight_pct", 0.0) / 100.0
...
class PerformanceAssessmentPhase:
    ...
    Measures actual E/S characteristics attainment versus pre-contractual
    commitments and calculates actual sustainable investment proportion.
    """
    ...
    async def execute(self, context: WorkflowContext) -> PhaseResult:
        """
        Execute performance assessment phase.
        ...
        try:
            ...
            holdings = config.get("portfolio_holdings", [])
            commitments = config.get("precontractual_commitments", [])

            # Calculate actual sustainable investment proportion
            si_holdings = [
                h for h in holdings
                if h.get("is_sustainable_investment", False)
            ]
            si_weight = sum(
                h.get("portfolio_weight_pct", 0.0) for h in si_holdings
            )
            outputs["actual_sustainable_investment_pct"] = round(si_weight, 2)

            # Calculate actual taxonomy alignment
            taxonomy_weight = sum(
                h.get("portfolio_weight_pct", 0.0)
                * h.get("taxonomy_aligned_pct", 0.0) / 100.0
                for h in holdings
            )
            outputs["actual_taxonomy_aligned_pct"] = round(taxonomy_weight, 2)

            # E/S characteristics holdings
            es_weight = sum(
                h.get("portfolio_weight_pct", 0.0) for h in holdings
                if h.get("esg_rating") is not None
                and h.get("esg_rating", 0) > 0
            )
            outputs["es_characteristics_pct"] = round(es_weight, 2)
            ...
            for commitment in commitments:
                ...
                target = commitment.get("target_value", 0.0)
                actual = commitment.get("actual_value")

                # Auto-resolve actual values from computed data
                c_type = commitment.get("commitment_type", "")
                if actual is None:
                    if "taxonomy" in name.lower():
                        actual = taxonomy_weight
                    elif "sustainable" in name.lower():
                        actual = si_weight
                    else:
                        actual = 0.0

                if actual >= target:
                    attainment = AttainmentStatus.MET.value
                    commitments_met += 1
                elif actual >= target * 0.8:
                    attainment = AttainmentStatus.PARTIALLY_MET.value
                else:
                    attainment = AttainmentStatus.NOT_MET.value
            ...
            outputs["commitment_results"] = commitment_results
            outputs["commitments_met"] = commitments_met
            outputs["commitments_total"] = len(commitments)
            outputs["commitment_attainment_pct"] = round(
                commitments_met / max(len(commitments), 1) * 100, 1
            )

            # Pre-contractual vs actual comparison
            ...
            outputs["comparison_table"] = {
                "sustainable_investment": {
                    ...
                    "met": si_weight >= si_commitment,
                },
                "taxonomy_alignment": {
                    "committed_pct": tax_commitment,
                    "actual_pct": round(taxonomy_weight, 2),
                    "variance_pct": round(taxonomy_weight - tax_commitment, 2),
                    "met": taxonomy_weight >= tax_commitment,
                },
            }
            ...
            if si_weight < si_commitment:
                warnings.append(
                    f"Sustainable investment proportion ({si_weight:.1f}%) "
                    f"below commitment ({si_commitment:.1f}%)"
                )
            if taxonomy_weight < tax_commitment:
                warnings.append(
                    f"Taxonomy alignment ({taxonomy_weight:.1f}%) "
                    f"below commitment ({tax_commitment:.1f}%)"
                )
            ...
            if rated_holdings:
                avg_esg = sum(
                    h.get("esg_rating", 0) for h in rated_holdings
                ) / len(rated_holdings)
            ...
            logger.error(
                "PerformanceAssessment failed: %s", exc, exc_info=True
            )
...
class PAICalculationPhase:
    ...
    Calculates all applicable Principal Adverse Impact indicators,
    performs year-over-year comparison, and assesses coverage ratios.
    """
    ...
    async def execute(self, context: WorkflowContext) -> PhaseResult:
        ...
        Args:
            context: Workflow context with emissions and ESG data.
        ...
        try:
            ...
            previous_data = config.get("previous_period_data", {})
            ...
            for indicator_input in pai_inputs:
                indicator_id = indicator_input.get("indicator_id", "")
                current_val = indicator_input.get("current_period_value")
                previous_val = indicator_input.get("previous_period_value")
                coverage = indicator_input.get("coverage_pct", 0.0)
                ...
                pai_result = {
                    ...
                    "indicator_name": indicator_input.get(
                        "indicator_name", ""
                    ),
                    "category": indicator_input.get("category", "climate"),
                    "current_period_value": current_val,
                    "previous_period_value": previous_val,
                    "yoy_change": yoy_change,
                    "yoy_change_pct": yoy_change_pct,
                    "unit": indicator_input.get("unit", ""),
                    "coverage_pct": coverage,
                    "data_quality": indicator_input.get(
                        "data_quality", "REPORTED"
                    ),
                    "has_data": current_val is not None,
                }
                pai_results.append(pai_result)
            ...
            if missing_mandatory:
                warnings.append(
                    ...
                    f"missing data: {', '.join(missing_mandatory[:5])}"
                    + ("..." if len(missing_mandatory) > 5 else "")
                )

            # Category breakdown
            category_summary: Dict[str, Dict[str, Any]] = {}
            for result in pai_results:
                cat = result.get("category", "other")
                if cat not in category_summary:
                    category_summary[cat] = {
                        "count": 0, "with_data": 0, "avg_coverage": 0.0
                    }
                category_summary[cat]["count"] += 1
                if result.get("has_data"):
                    category_summary[cat]["with_data"] += 1
                    category_summary[cat]["avg_coverage"] += result.get(
                        "coverage_pct", 0.0
                    )

            for cat_data in category_summary.values():
                if cat_data["with_data"] > 0:
                    cat_data["avg_coverage"] = round(
                        cat_data["avg_coverage"] / cat_data["with_data"], 1
                    )
...
class TemplateGenerationPhase:
    ...
    Generates the Annex IV template with actual figures, top 15 investments,
    sector and geographic allocation, and comparison tables.
    """
    ...
    async def execute(self, context: WorkflowContext) -> PhaseResult:
        ...
        Args:
            context: Workflow context with all computed data.
        ...
        try:
            config = context.config
            data_output = context.get_phase_output("data_collection")
            performance_output = context.get_phase_output(
            ...
            template = {
                ...
                "es_characteristics_attainment": {
                    "description": (
                        f"This section details how {product_name} met its "
                        f"environmental and social characteristics during "
                        f"the reporting period."
                    ),
                    "commitment_results": performance_output.get(
                        "commitment_results", []
                    ),
                    "comparison_table": performance_output.get(
                        "comparison_table", {}
                    ),
                },
                "top_investments": {
                    "top_15": data_output.get("top_15_investments", []),
                    "total_holdings": data_output.get("total_holdings", 0),
                    "total_value_eur": data_output.get(
                        "total_portfolio_value_eur", 0.0
                    ),
                },
                "proportion_of_investments": {
                    "sustainable_investment_actual_pct": performance_output.get(
                        "actual_sustainable_investment_pct", 0.0
                    ),
                    "taxonomy_aligned_actual_pct": performance_output.get(
                        "actual_taxonomy_aligned_pct", 0.0
                    ),
                    "es_characteristics_pct": performance_output.get(
                        "es_characteristics_pct", 0.0
                    ),
                },
            ...
            benchmark_data = config.get("reference_benchmark_performance")
            if benchmark_data:
                template["benchmark_comparison"] = benchmark_data
            else:
                template["benchmark_comparison"] = {
                    "applicable": False,
                    "description": "No reference benchmark designated.",
                }
            ...
            logger.error("TemplateGeneration failed: %s", exc, exc_info=True)
...
class FilingPackagePhase:
    ...
    Assembles a filing-ready package with the completed Annex IV template,
    supporting evidence, and a compliance certificate.
    """
    ...
    async def execute(self, context: WorkflowContext) -> PhaseResult:
        ...
        Args:
            context: Workflow context with template and all prior outputs.
        ...
        try:
            ...
            outputs["package_manifest"] = {
                ...
                "documents": [
                    {
                        "document_type": "annex_iv_template",
            ...
            commitments_met = performance_output.get("commitments_met", 0)
            commitments_total = performance_output.get("commitments_total", 0)
            pai_reported = pai_output.get("pai_indicators_reported", 0)
            missing_mandatory = pai_output.get(
                "missing_mandatory_indicators", []
            )

            is_compliant = (
                commitments_met == commitments_total
            ...
            compliance_certificate = {
                ...
                "checks": {
                    "all_commitments_met": commitments_met == commitments_total,
                    "commitments_met": commitments_met,
                    "commitments_total": commitments_total,
                    "mandatory_pai_complete": len(missing_mandatory) == 0,
            ...
            if not is_compliant:
                if commitments_met < commitments_total:
                    compliance_certificate["recommendations"].append(
                        f"{commitments_total - commitments_met} commitment(s) "
                        f"not met. Review portfolio alignment."
                    )
                if missing_mandatory:
                    compliance_certificate["recommendations"].append(
                        f"{len(missing_mandatory)} mandatory PAI indicator(s) "
                        f"lack data. Improve data sourcing."
                    )
            ...
            outputs["evidence_index"] = {
                ...
                "performance_data_hash": _hash_data(
                    performance_output.get("commitment_results", [])
                ),
                "template_hash": _hash_data(
                    template_output.get("annex_iv_template", {})
                ),
            }

            outputs["filing_status"] = (
                "ready_for_filing" if is_compliant else "requires_review"
            )

            if not is_compliant:
                warnings.append(
                    "Filing package generated but compliance issues identified. "
                    "Review compliance certificate for details."
                )

            status = PhaseStatus.COMPLETED

        except Exception as exc:
            logger.error("FilingPackage failed: %s", exc, exc_info=True)
...
class PeriodicReportingWorkflow:
    """
    Five-phase periodic reporting workflow for SFDR Article 8.

    Orchestrates data collection through filing package assembly for
    Annex IV periodic disclosures. Supports checkpoint/resume and
    phase skipping.
    ...
    Example:
        ...
        >>> input_data = PeriodicReportingInput(
    ...
    WORKFLOW_NAME = "periodic_reporting"
    ...
    def __init__(
        ...
        """
        Initialize the periodic reporting workflow.
    ...
    async def run(
        self, input_data: PeriodicReportingInput
    ) -> PeriodicReportingResult:
        """
        Execute the complete 5-phase periodic reporting workflow.
            ...
            try:
                ...
                if phase_result.status == PhaseStatus.COMPLETED:
                    ...
                    if phase_name == "data_collection":
                        ...
                        logger.error(
                            "Critical phase '%s' failed, aborting", phase_name
                        )
                        break
        ...
        logger.info(
            "Periodic reporting workflow %s finished status=%s in %.1fs",
            self.workflow_id, overall_status.value, total_duration,
        )

        return PeriodicReportingResult(
            workflow_id=self.workflow_id,
            ...
            commitments_met=summary.get("commitments_met", 0),
            commitments_total=summary.get("commitments_total", 0),
    ...
    def _build_config(
        ...
        if input_data.portfolio_holdings:
            ...
            for h in config["portfolio_holdings"]:
                h["data_quality"] = h["data_quality"].value if isinstance(
                    h["data_quality"], DataQualityTier
                ) else h["data_quality"]
        if input_data.pai_indicators:
            ...
            for p in config["pai_indicators"]:
                p["data_quality"] = p["data_quality"].value if isinstance(
                    p["data_quality"], DataQualityTier
                ) else p["data_quality"]
        if input_data.precontractual_commitments:
            config["precontractual_commitments"] = [
                c.model_dump() for c in input_data.precontractual_commitments
            ]
        return config
    ...
    def _build_summary(self, context: WorkflowContext) -> Dict[str, Any]:
        ...
        perf_out = context.get_phase_output("performance_assessment")
        ...
        return {
            ...
            "actual_sustainable_investment_pct": perf_out.get(
                "actual_sustainable_investment_pct", 0.0
            ),
            "actual_taxonomy_aligned_pct": perf_out.get(
                "actual_taxonomy_aligned_pct", 0.0
            ),
            "commitments_met": perf_out.get("commitments_met", 0),
            "commitments_total": perf_out.get("commitments_total", 0),
            ...
            "filing_package_id": filing_out.get("filing_package_id", ""),
        }
