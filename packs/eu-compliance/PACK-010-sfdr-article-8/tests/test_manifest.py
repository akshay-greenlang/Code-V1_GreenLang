# -*- coding: utf-8 -*-
"""
PACK-010 SFDR Article 8 Pack - Manifest Tests
===============================================

Tests the pack.yaml manifest for PACK-010 to ensure:
- Pack metadata is correct (ID, version, tier=standalone, category=eu-compliance)
- SFDR regulation references are present (EU 2019/2088, RTS 2022/1288)
- All 8 SFDR engines are listed
- All 8 SFDR workflows are listed
- All 8 SFDR templates are listed
- All 10 SFDR integrations are listed
- All 5 deployment presets are defined (asset_manager, insurance, bank,
...
class TestPackManifest:
    """Test suite for PACK-010 pack.yaml manifest."""
    ...
    def test_pack_yaml_metadata_fields(self, pack_yaml: Dict[str, Any]):
        ...
        assert "PACK-010" in meta["name"], (
            f"Expected PACK-010 in metadata.name, got {meta['name']}"
        )
        assert meta["display_name"] == "SFDR Article 8 Pack", (
            f"Expected 'SFDR Article 8 Pack', got {meta['display_name']}"
        )
    ...
    def test_pack_yaml_regulation_references(self, pack_yaml: Dict[str, Any]):
        """Test that SFDR regulation references are present."""
        meta = pack_yaml.get("metadata", {})
        ...
        # Check compliance_references for RTS
        ...
        assert len(compliance_refs) >= 3, (
            f"Expected at least 3 compliance references, found {len(compliance_refs)}"
        )
    ...
    def test_pack_yaml_engines_section(self, pack_yaml: Dict[str, Any]):
        """Test that components.engines contains exactly 8 SFDR engines."""
    ...
    def test_pack_yaml_workflows_section(self, pack_yaml: Dict[str, Any]):
        """Test that components.workflows contains exactly 8 SFDR workflows."""
        components = pack_yaml.get("components", {})
    ...
    def test_pack_yaml_templates_section(self, pack_yaml: Dict[str, Any]):
        """Test that components.templates contains exactly 8 SFDR templates."""
        components = pack_yaml.get("components", {})
    ...
    # -----------------------------------------------------------------
    # 10. Presets section (5 presets)
    # -----------------------------------------------------------------

    def test_pack_yaml_presets_section(self, pack_yaml: Dict[str, Any]):
        """Test that presets.product_presets contains exactly 5 deployment presets."""
        ...
        for expected_id in SFDR_PRESET_IDS:
    ...
    def test_pack_yaml_all_preset_files_exist(self, pack_yaml: Dict[str, Any]):
        """Test that all 5 product preset YAML files exist on disk."""
        presets = pack_yaml.get("presets", {})
        product_presets = presets.get("product_presets", [])
