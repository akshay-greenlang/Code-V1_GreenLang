# EU Compliance Packs

Solution Packs for European Union environmental and sustainability regulations.

## Packs

| Pack | Name | Tier | Agents | Tests | Status |
|------|------|------|--------|-------|--------|
| PACK-001 | CSRD Starter Pack | Starter | 66+ | 123 | BUILT |
| PACK-002 | CSRD Professional Pack | Professional | 93+ | 313 | BUILT |
| PACK-003 | CSRD Enterprise Pack | Enterprise | 135+ | 355 | BUILT |
| PACK-004 | EUDR Compliance Pack | TBD | TBD | TBD | PLANNED |
| PACK-004 | CBAM Compliance Pack | TBD | TBD | TBD | PLANNED |
| PACK-005 | EU Taxonomy Alignment Pack | TBD | TBD | TBD | PLANNED |

## Shared Dependencies

All EU Compliance Packs share:
- GreenLang Foundation Agents (orchestrator, schema, citations, audit trail)
- Authentication & RBAC (JWT, role-based access)
- Observability Stack (Prometheus, Grafana, OpenTelemetry)
- Data Quality Pipeline (validation, dedup, profiling)
- PostgreSQL + TimescaleDB + pgvector
