# -*- coding: utf-8 -*-
"""
FoundationBridge - FOUND-003/004/005 for PACK-049 Multi-Site Management
=========================================================================

Bridges to AGENT-FOUND agents for unit normalisation, assumption tracking,
and citation management needed for multi-site data integrity.

Integration Points:
    - FOUND-003 Unit & Reference Normaliser: normalise disparate units
      across sites (kWh/GJ, litres/kg, miles/km) to ensure consistent
      aggregation
    - FOUND-004 Assumptions Registry: register and track assumptions
      applied across multi-site calculations (e.g., estimation factors,
      default values for missing site data)
    - FOUND-005 Citations & Evidence Agent: manage citation records for
      emission factors, grid factors, and methodology references used
      across the site portfolio
...
class FoundationBridge:
    """
    Bridge to FOUND-003, FOUND-004, and FOUND-005 for multi-site data integrity.

    Provides unit normalisation for cross-site consistency, assumption
    management for portfolio-wide estimation decisions, and citation
    ...
    Example:
        ...
        >>> result = await bridge.normalise_units(100.0, "kWh", "GJ", "SITE-001")
    ...
    async def normalise_units(
        ...
        Args:
            ...
            to_unit: Target unit.
            site_id: Optional site identifier for tracking.

        Returns:
            NormalisationResult with converted value and factor.
        """
        logger.info(
            "Normalising %.4f %s -> %s for site=%s",
            value, from_unit, to_unit, site_id,
        )
    ...
    async def batch_normalise(
        ...
        Args:
            conversions: List of dicts with keys: value, from_unit, to_unit, site_id.
    ...
    async def register_assumption(
        ...
        return AssumptionRecord(
            ...
            affected_sites=affected_sites or [],
            provenance_hash=_compute_hash({
                "description": description,
                "justification": justification,
                "category": category,
