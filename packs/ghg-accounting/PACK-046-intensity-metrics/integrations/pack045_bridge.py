# -*- coding: utf-8 -*-
"""
Pack045Bridge - PACK-045 Base Year Management Bridge for PACK-046
====================================================================

Bridges to PACK-045 Base Year Management for base year emissions data,
base year denominator values, recalculation-adjusted historical time
series, and recalculation flags. Provides the reference data needed
to compute base year intensity metrics and track progress.

Integration Points:
    - PACK-045 base year emission totals by scope
    - PACK-045 base year activity data (denominators)
    - PACK-045 recalculation-adjusted time series
...
class BaseYearDenominators(BaseModel):
    """Base year denominator (activity) data from PACK-045."""

    base_year: str = ""
    denominators: Dict[str, float] = Field(
        default_factory=dict,
        description="Map of denominator_type -> base year value",
    )
...
class BaseYearResponse(BaseModel):
    ...
    denominators: Optional[BaseYearDenominators] = None
...
class Pack045Bridge:
    ...
    Retrieves base year emissions, denominator data, recalculation-
    adjusted time series, and recalculation flags needed for intensity
    metric base year comparisons and target tracking.
    ...
    async def get_base_year_denominators(self) -> BaseYearDenominators:
        """
        Retrieve base year denominator (activity) data.

        Returns:
            BaseYearDenominators with historical denominator values.
        """
        logger.info("Fetching base year denominators")
        return await self._fetch_denominators()

    async def get_adjusted_time_series(
        self,
        scope: str = "scope_1",
        denominator_type: str = "revenue",
    ) -> AdjustedTimeSeries:
        """
        ...
        Args:
            ...
            denominator_type: Denominator for intensity calculation.

        Returns:
            AdjustedTimeSeries with year-by-year data points.
        """
        logger.info(
            "Fetching adjusted time series: scope=%s, denominator=%s",
            scope, denominator_type,
        )
    ...
    async def get_recalculation_flags(self) -> List[RecalculationFlag]:
        ...
        Important for intensity metrics: if the base year was recently
        recalculated, historical intensity values may need to be updated.
    ...
    async def _fetch_denominators(self) -> BaseYearDenominators:
        """Fetch base year denominators from PACK-045."""
        logger.debug("Fetching base year denominators")
        return BaseYearDenominators(
            provenance_hash=_compute_hash({"action": "fetch_denominators"}),
        )
