# -*- coding: utf-8 -*-
"""
Pack042043Bridge - Combined Scope 3 Bridge for PACK-047 GHG Benchmark
=========================================================================
...
upstream vs downstream split for value chain analysis, and sector-
specific benchmarking data per category.
...
# Categories classified as upstream (1-8) or downstream (9-15)
...
class Pack042043Bridge:
    ...
    totals with coverage flags, upstream vs downstream split, and sector-
    specific benchmarking data for all 15 GHG Protocol categories.
    ...
    Example:
        ...
        >>> print(response.upstream_tco2e, response.downstream_tco2e)
    """
    ...
    async def get_scope3_totals(self, period: str) -> Scope3Response:
        ...
        categories with coverage flags, materiality indicators, and
        upstream/downstream split for value chain analysis.
        ...
        try:
            ...
            logger.info(
                "Scope 3 totals from %s: %.2f tCO2e, %d categories "
                "(upstream=%.2f, downstream=%.2f) in %.1fms",
                source.value, response.total_scope3_tco2e,
                response.categories_included, response.upstream_tco2e,
                response.downstream_tco2e, response.duration_ms,
            )
            return response
    ...
    async def get_upstream_downstream_split(
        self, period: str
    ) -> Dict[str, float]:
        """Get upstream vs downstream emissions split.

        Args:
            period: Reporting period.

        Returns:
            Dictionary with upstream_tco2e and downstream_tco2e.
        """
        response = await self.get_scope3_totals(period)
        return {
            "upstream_tco2e": response.upstream_tco2e,
            "downstream_tco2e": response.downstream_tco2e,
            "total_scope3_tco2e": response.total_scope3_tco2e,
        }
