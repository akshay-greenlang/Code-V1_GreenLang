# -*- coding: utf-8 -*-
"""
MRVEnMSBridge - Bridge to MRV Agents for EnMS Emissions Calculations
======================================================================

This module routes energy consumption and savings data from the ISO 50001
EnMS pipeline to the appropriate MRV (Monitoring, Reporting, Verification)
agents for emissions calculation. It maps energy sources and SEUs to
Scope 1, 2, and 3 emissions across the 30 MRV agents.

Routing Table:
    Natural gas combustion    --> MRV-001 (Stationary Combustion)
    Refrigerant leakage       --> MRV-002 (Refrigerants & F-Gas)
    Grid electricity          --> MRV-009 (Scope 2 Location-Based)
    Green tariff electricity  --> MRV-010 (Scope 2 Market-Based)
    Purchased steam/heat      --> MRV-011 (Steam/Heat Purchase)
    District cooling          --> MRV-012 (Cooling Purchase)
    Data freshness checks     --> MRV-016 (Data Freshness)

Features:
    - Route energy data to correct MRV agent for emissions calculation
    - Convert energy savings (kWh/MWh) to avoided emissions (tCO2e)
    - EnMS-specific emission factor selection per energy source
...
SAVINGS_ROUTING_TABLE: List[SavingsToEmissionsMapping] = [
    ...
    SavingsToEmissionsMapping(
        category=MeasureCategory.STATIONARY_COMBUSTION, mrv_agent_id="MRV-001",
        ...
        description="On-site fuel combustion for process and heating",
    ),
    SavingsToEmissionsMapping(
        category=MeasureCategory.PROCESS_HEATING, mrv_agent_id="MRV-001",
        mrv_agent_name="Stationary Combustion", scope=MRVScope.SCOPE_1,
        energy_carrier="natural_gas",
        module_path="greenlang.agents.mrv.stationary_combustion",
        description="Process heating from gas-fired equipment",
    ),
    SavingsToEmissionsMapping(
        ...
        energy_carrier="natural_gas",
        module_path="greenlang.agents.mrv.stationary_combustion",
        description="Gas-fired HVAC and boiler systems",
    ),
    SavingsToEmissionsMapping(
        category=MeasureCategory.BOILER_STEAM, mrv_agent_id="MRV-001",
        mrv_agent_name="Stationary Combustion", scope=MRVScope.SCOPE_1,
        energy_carrier="natural_gas",
        module_path="greenlang.agents.mrv.stationary_combustion",
        description="On-site steam generation from fuel combustion",
    ),
    # Scope 1 -- Refrigerant
    SavingsToEmissionsMapping(
        category=MeasureCategory.REFRIGERANT_LEAKAGE, mrv_agent_id="MRV-002",
        ...
        description="Refrigerant leak repair and system maintenance",
    ),
    # Scope 2 -- Electricity (location-based)
    SavingsToEmissionsMapping(
        category=MeasureCategory.GRID_ELECTRICITY, mrv_agent_id="MRV-009",
    ...
    SavingsToEmissionsMapping(
        ...
        energy_carrier="electricity",
        module_path="greenlang.agents.mrv.scope2_location_based",
        description="Compressed air system electricity consumption",
    ),
    SavingsToEmissionsMapping(
        ...
        energy_carrier="electricity",
        module_path="greenlang.agents.mrv.scope2_location_based",
        description="Motor and variable speed drive electricity",
    ),
    SavingsToEmissionsMapping(
        ...
        energy_carrier="electricity",
        module_path="greenlang.agents.mrv.scope2_location_based",
        description="Facility lighting electricity consumption",
    ),
    # Scope 2 -- Electricity (market-based)
    SavingsToEmissionsMapping(
        category=MeasureCategory.GREEN_TARIFF, mrv_agent_id="MRV-010",
        mrv_agent_name="Scope 2 Market-Based", scope=MRVScope.SCOPE_2,
        energy_carrier="electricity_green",
        module_path="greenlang.agents.mrv.scope2_market_based",
        description="Green tariff or renewable energy certificate electricity",
    ),
    # Scope 2 -- Purchased steam/cooling
    SavingsToEmissionsMapping(
        category=MeasureCategory.PURCHASED_STEAM, mrv_agent_id="MRV-011",
    ...
    SavingsToEmissionsMapping(
        category=MeasureCategory.DISTRICT_COOLING, mrv_agent_id="MRV-012",
        ...
        description="District cooling and chilled water purchase",
    ),
]
...
class MRVEnMSBridge:
    ...
    Routes energy consumption and savings data to the appropriate MRV agent
    and converts energy performance improvements into emissions reductions
    (tCO2e) following ISO 50001 Clause 6.3 energy review requirements.
    ...
    Example:
        >>> bridge = MRVEnMSBridge()
        >>> result = bridge.route_energy_data(
        ...     {"category": "grid_electricity", "consumption_kwh": 500000}
        ... )
        >>> print(f"Emissions: {result.emissions_tco2e} tCO2e")
    """

    def __init__(self, config: Optional[MRVRouteConfig] = None) -> None:
        """Initialize the MRV EnMS Bridge.
        ...
        self.logger.info(
            "MRVEnMSBridge initialized: %d/%d agents available, country=%s",
    ...
    def route_energy_data(
        self,
        energy_data: Dict[str, Any],
    ) -> RoutingResult:
        """Route energy consumption data to the appropriate MRV agent.

        Args:
            energy_data: Dict with 'category', 'consumption_kwh' or
                         'savings_kwh', and optional 'emission_factor_override'.
        ...
        category_str = energy_data.get("category", "")
        consumption_kwh = float(energy_data.get("consumption_kwh", 0.0))
        savings_kwh = float(energy_data.get("savings_kwh", 0.0))
        ...
        # Get emission factor
        ef = self._get_emission_factor(route, energy_data)
    ...
    def convert_savings_to_emissions(
        self,
        savings_kwh: float,
        ...
        Args:
            ...
            energy_carrier: Energy carrier type for EF lookup.

        Returns:
            Avoided emissions in tCO2e as Decimal.
        """
        ef = DEFAULT_EMISSION_FACTORS.get(
            energy_carrier, self.config.grid_emission_factor_kgco2_per_kwh
        )
    ...
    def get_emission_factor(
        ...
        """Get the emission factor for an energy source.

        Args:
            energy_source: Energy carrier type.
            region: Region code (for future regional EF support).

        Returns:
            Emission factor in kg CO2e per kWh.
        """
        if energy_source == "electricity":
            return self.config.grid_emission_factor_kgco2_per_kwh
        elif energy_source == "electricity_green":
            return self.config.market_based_ef_kgco2_per_kwh
        elif energy_source == "natural_gas":
            return self.config.natural_gas_ef_kgco2_per_kwh
        return DEFAULT_EMISSION_FACTORS.get(energy_source, 0.0)
    ...
    def get_applicable_agents(
        ...
        Args:
            measure_category: EnMS measure category.
    ...
    def _get_emission_factor(
        self, route: SavingsToEmissionsMapping, data: Dict[str, Any]
    ) -> float:
        """Determine emission factor for a routing operation.

        Args:
            route: Routing table entry.
            data: Input data with optional override.

        Returns:
            Emission factor in kg CO2e per kWh.
        """
        ef_override = data.get("emission_factor_override")
        if ef_override is not None:
            return float(ef_override)
        if route.energy_carrier == "electricity":
            return self.config.grid_emission_factor_kgco2_per_kwh
        elif route.energy_carrier == "electricity_green":
            return self.config.market_based_ef_kgco2_per_kwh
        elif route.energy_carrier == "natural_gas":
            return self.config.natural_gas_ef_kgco2_per_kwh
        return DEFAULT_EMISSION_FACTORS.get(route.energy_carrier, 0.0)
