# -*- coding: utf-8 -*-
"""
HealthCheck - 20-Category System Health Verification for PACK-038
===================================================================

This module implements a comprehensive health check system that validates the
operational readiness of the Peak Shaving Pack across engines, workflows,
integrations, external system connectivity, and infrastructure components.

Check Categories (20):
    1.  profile_analysis_engine       -- Load profile analysis engine
    2.  peak_identification_engine    -- Peak demand identification engine
    3.  demand_charge_engine          -- Demand charge analysis engine
    4.  bess_sizing_engine            -- BESS sizing engine
    5.  load_shifting_engine          -- Load shifting optimization engine
    6.  cp_management_engine          -- Coincident peak management engine
    7.  ratchet_analysis_engine       -- Demand ratchet analysis engine
    8.  power_factor_engine           -- Power factor correction engine
    9.  financial_modeling_engine     -- Financial modeling engine
    10. reporting_engine              -- Peak shaving report engine
    11. workflows                     -- Verify peak shaving workflows
    ...
    16. meter_data                    -- Check meter data feed quality
    17. bess_control                  -- Check BESS control connectivity
    18. bms_control                   -- Check BMS control connectivity
    19. tariff_data                   -- Check tariff data currency
...
class HealthCategory(str, Enum):
    ...
    PROFILE_ANALYSIS_ENGINE = "profile_analysis_engine"
    PEAK_IDENTIFICATION_ENGINE = "peak_identification_engine"
    DEMAND_CHARGE_ENGINE = "demand_charge_engine"
    BESS_SIZING_ENGINE = "bess_sizing_engine"
    LOAD_SHIFTING_ENGINE = "load_shifting_engine"
    CP_MANAGEMENT_ENGINE = "cp_management_engine"
    RATCHET_ANALYSIS_ENGINE = "ratchet_analysis_engine"
    POWER_FACTOR_ENGINE = "power_factor_engine"
    FINANCIAL_MODELING_ENGINE = "financial_modeling_engine"
...
QUICK_CHECK_CATEGORIES = {
    HealthCategory.PROFILE_ANALYSIS_ENGINE,
    HealthCategory.BESS_SIZING_ENGINE,
    HealthCategory.METER_DATA,
    HealthCategory.BESS_CONTROL,
    HealthCategory.BMS_CONTROL,
    HealthCategory.TARIFF_DATA,
}
...
PS_ENGINES = [
    ...
    "peak_identification_engine",
    "demand_charge_engine",
    "bess_sizing_engine",
    "load_shifting_engine",
    "cp_management_engine",
    "ratchet_analysis_engine",
    "power_factor_engine",
...
PS_WORKFLOWS = [
    "full_peak_shaving_workflow",
    "bess_dispatch_workflow",
    "load_shifting_workflow",
    "cp_management_workflow",
    "ratchet_avoidance_workflow",
    "power_factor_workflow",
...
class HealthCheck:
    """20-category health check for Peak Shaving Pack.
    ...
    def __init__(self, config: Optional[HealthCheckConfig] = None) -> None:
        ...
        self._check_handlers: Dict[HealthCategory, Callable[[], List[HealthResult]]] = {
            HealthCategory.PROFILE_ANALYSIS_ENGINE: self._check_engine("profile_analysis_engine"),
            HealthCategory.PEAK_IDENTIFICATION_ENGINE: self._check_engine("peak_identification_engine"),
            HealthCategory.DEMAND_CHARGE_ENGINE: self._check_engine("demand_charge_engine"),
            HealthCategory.BESS_SIZING_ENGINE: self._check_engine("bess_sizing_engine"),
            HealthCategory.LOAD_SHIFTING_ENGINE: self._check_engine("load_shifting_engine"),
            HealthCategory.CP_MANAGEMENT_ENGINE: self._check_engine("cp_management_engine"),
            HealthCategory.RATCHET_ANALYSIS_ENGINE: self._check_engine("ratchet_analysis_engine"),
            HealthCategory.POWER_FACTOR_ENGINE: self._check_engine("power_factor_engine"),
            HealthCategory.FINANCIAL_MODELING_ENGINE: self._check_engine("financial_modeling_engine"),
            ...
            HealthCategory.BESS_CONTROL: self._check_bridge("bess_control_bridge"),
            HealthCategory.BMS_CONTROL: self._check_bridge("bms_bridge"),
            HealthCategory.TARIFF_DATA: self._check_bridge("utility_rate_bridge"),
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "engines_count": len(PS_ENGINES),
            "workflows_count": len(PS_WORKFLOWS),
            "integrations_count": len(PS_INTEGRATIONS),
        }
    ...
    def _check_engine(self, engine_name: str) -> Callable[[], List[HealthResult]]:
        ...
        def handler() -> List[HealthResult]:
            ...
            exists = fpath.exists()
            category = HealthCategory(engine_name) if engine_name in [e.value for e in HealthCategory] else HealthCategory.PROFILE_ANALYSIS_ENGINE
            return [HealthResult(
    ...
    def _check_workflows(self) -> List[HealthResult]:
        """Check that all peak shaving workflows exist."""
        checks: List[HealthResult] = []
        start = time.monotonic()
        base = PACK_BASE_DIR / "workflows"
        for name in PS_WORKFLOWS:
