...
profiles, and billing analysis data into the Demand Response pipeline.
...
Use in DR:
    Rate structures drive DR event value calculation. Higher on-peak rates
    mean greater savings from curtailment. Demand charge reduction through
    DR participation can yield significant annual savings.
...
class TariffData(BaseModel):
    """Detailed tariff data for DR value calculation."""
    ...
    peak_months: List[int] = Field(default_factory=list, description="Peak season months")
    annual_energy_cost_usd: float = Field(default=0.0, ge=0.0)
    annual_demand_cost_usd: float = Field(default=0.0, ge=0.0)
    dr_savings_potential_usd: float = Field(default=0.0, ge=0.0)
    provenance_hash: str = Field(default="")
...
class DemandChargeProfile(BaseModel):
    ...
    annual_demand_cost_usd: float = Field(default=0.0, ge=0.0)
    ratchet_pct: float = Field(default=0.0, ge=0.0, le=100.0, description="Demand ratchet %")
    ratchet_kw: float = Field(default=0.0, ge=0.0)
    dr_reduction_potential_kw: float = Field(default=0.0, ge=0.0)
    dr_savings_potential_usd: float = Field(default=0.0, ge=0.0)
    provenance_hash: str = Field(default="")
...
class Pack036Bridge:
    """Bridge to PACK-036 Utility Analysis data for demand response.

    Imports utility rate structures, TOU period definitions, demand charge
    profiles, and billing analysis from PACK-036 to calculate DR event
    value and demand charge reduction opportunities.
    ...
    def get_rate_structure(self, facility_id: str) -> RateStructure:
        ...
        result = RateStructure(
            ...
            tariff_name="Commercial TOU - Large",
    ...
    def get_tariff_data(self, facility_id: str) -> TariffData:
        """Get detailed tariff data for DR value calculation.

        Args:
            facility_id: Facility identifier.

        Returns:
            TariffData with rate components and DR savings potential.
        """
        start = time.monotonic()
        ...
        result = TariffData(
            ...
            peak_months=[6, 7, 8, 9],
            annual_energy_cost_usd=720_000.0,
            annual_demand_cost_usd=180_000.0,
            dr_savings_potential_usd=45_000.0,
        )
    ...
    def get_demand_profile(self, facility_id: str) -> DemandChargeProfile:
        """Get demand charge profile for DR optimization.
        ...
        result = DemandChargeProfile(
            ...
            peak_demand_kw=2200.0,
            peak_demand_month=7,
            avg_monthly_demand_kw=1800.0,
            demand_charge_type=DemandChargeType.ON_PEAK_DEMAND,
            demand_charge_rate_usd_per_kw=18.00,
            annual_demand_cost_usd=180_000.0,
            ratchet_pct=80.0,
            ratchet_kw=1760.0,
            dr_reduction_potential_kw=400.0,
            dr_savings_potential_usd=86_400.0,
        )
    ...
    def calculate_event_value(
        ...
        """Calculate the financial value of a DR event using tariff data.

        Deterministic calculation:
            event_value = curtailment_kwh * period_rate

        Args:
            curtailment_kwh: Energy curtailed during event.
            event_period: TOU period of event (on_peak, critical_peak, etc.).
            facility_id: Facility identifier.

        Returns:
            Dict with event value calculation.
        """
        ...
        rate_map = {
            "on_peak": tariff.on_peak_rate_usd_per_kwh,
            "mid_peak": tariff.mid_peak_rate_usd_per_kwh,
            "off_peak": tariff.off_peak_rate_usd_per_kwh,
            "critical_peak": tariff.critical_peak_rate_usd_per_kwh,
        }
        rate = rate_map.get(event_period, tariff.on_peak_rate_usd_per_kwh)

        # Zero-hallucination: direct arithmetic
        event_value = Decimal(str(curtailment_kwh)) * Decimal(str(rate))
        ...
        result = {
            ...
            "event_value_usd": float(event_value.quantize(Decimal("0.01"))),
