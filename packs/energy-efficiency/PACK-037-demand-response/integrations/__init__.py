# -*- coding: utf-8 -*-
"""
PACK-037 Demand Response Pack - Integration Layer
====================================================

Phase 4 integration layer for the Demand Response Pack that provides
DR pipeline orchestration, MRV emissions bridging, DATA agent routing,
grid signal integration, BMS control, AMI meter data, DER asset
coordination, PACK-036/033 data import, 20-category health verification,
9-step DR setup wizard, and multi-channel alert management.

Components:
    - DemandResponseOrchestrator: 12-phase pipeline with DAG dependency
      resolution, parallel execution, retry with exponential backoff,
      and SHA-256 provenance tracking
    - MRVBridge: Routes DR curtailment data to MRV agents (MRV-009/010/013)
      using marginal emission factors for accurate peak-period accounting
    - DataBridge: Routes data intake to DATA agents for meter data,
      utility bills, quality profiling, gap filling, and reconciliation
    - GridSignalBridge: External grid signal integration for OpenADR 2.0b,
      ISO/RTO dispatch, price signals, and carbon intensity feeds
    - BMSControlBridge: BMS bidirectional control for HVAC setpoints,
      lighting, equipment shedding with comfort boundary enforcement
    - MeterDataBridge: AMI integration for 15-minute interval data,
      Green Button parsing, and baseline calculation
    - DERAssetBridge: DER asset coordination for battery, solar, EV,
      generator, and thermal storage dispatch
    - Pack036Bridge: Imports utility rate structures, TOU periods, and
      demand charge profiles from PACK-036
    - Pack033Bridge: Imports quick win measures that enable DR flexibility
      from PACK-033
    - HealthCheck: 20-category system health verification
    - SetupWizard: 9-step guided DR configuration with 8 facility presets
    - AlertBridge: Multi-channel alerting with escalation rules for DR
      event dispatch, performance, and settlement notifications

Architecture:
    External Systems --> Integration Bridges --> Platform Components
                              |
                              v
    PACK-037 Demand Response <-- Composition <-- Zero Duplication
                              |
                              v
    MRV Agents <-- DATA Agents <-- FOUND Agents

Platform Integrations:
    - greenlang/agents/mrv/* (MRV-001, 009, 010, 013)
    - greenlang/agents/data/* (DATA-001, 002, 003, 010, 014, 015)
    - greenlang/agents/foundation/* (10 FOUND agents)
    - packs/energy-efficiency/PACK-036 (Utility Analysis)
    - packs/energy-efficiency/PACK-033 (Quick Wins Identifier)

Author: GreenLang Platform Team
Date: March 2026
Pack: PACK-037 Demand Response
Status: Production Ready
"""

import logging

logger = logging.getLogger(__name__)

__version__: str = "1.0.0"
__pack_id__: str = "PACK-037"
__pack_name__: str = "Demand Response Pack"
__integrations_count__: int = 12

_loaded_integrations: list[str] = []

# ---------------------------------------------------------------------------
# Pack Orchestrator
# ---------------------------------------------------------------------------
try:
    from .pack_orchestrator import (
        DemandResponseOrchestrator,
        ExecutionStatus,
        FacilityType,
        OrchestratorPhase,
        PARALLEL_PHASE_GROUPS,
        PHASE_DEPENDENCIES,
        PHASE_EXECUTION_ORDER,
        PhaseProvenance,
        PhaseResult,
        PipelineConfig,
        PipelineResult,
        RetryConfig,
    )
    _loaded_integrations.append("DemandResponseOrchestrator")
except ImportError as e:
    logger.debug("Integration 1 (DemandResponseOrchestrator) not available: %s", e)

# ---------------------------------------------------------------------------
# MRV Bridge
# ---------------------------------------------------------------------------
try:
    from .mrv_bridge import (
        DREmissionCategory,
        EmissionFactorSet,
        MARGINAL_EMISSION_FACTORS,
        MRVBridge,
        MRVRequest,
        MRVResponse,
        MRVRouteConfig,
        MRVScope,
    )
    _loaded_integrations.append("MRVBridge")
except ImportError as e:
    logger.debug("Integration 2 (MRVBridge) not available: %s", e)

# ---------------------------------------------------------------------------
# Data Bridge
# ---------------------------------------------------------------------------
try:
    from .data_bridge import (
        DataBridge,
        DataQualityReport,
        DataRequest,
        DataResponse,
        DataRouteConfig,
        DRDataSource,
    )
    _loaded_integrations.append("DataBridge")
except ImportError as e:
    logger.debug("Integration 3 (DataBridge) not available: %s", e)

# ---------------------------------------------------------------------------
# Grid Signal Bridge
# ---------------------------------------------------------------------------
try:
    from .grid_signal_bridge import (
        CarbonIntensitySignal,
        CarbonSource,
        DispatchNotification,
        GridRegion,
        GridSignal,
        GridSignalBridge,
        GridSignalConfig,
        PriceSignal,
        SignalSeverity,
        SignalStatus,
        SignalType,
    )
    _loaded_integrations.append("GridSignalBridge")
except ImportError as e:
    logger.debug("Integration 4 (GridSignalBridge) not available: %s", e)

# ---------------------------------------------------------------------------
# BMS Control Bridge
# ---------------------------------------------------------------------------
try:
    from .bms_control_bridge import (
        BMSControlBridge,
        BMSControlConfig,
        BMSEndpoint,
        ConnectionStatus,
        ControlAction,
        ControlCommand,
        ControlResponse,
        ControlStatus,
        DeviceCategory,
        DeviceStatus,
        ProtocolType,
    )
    _loaded_integrations.append("BMSControlBridge")
except ImportError as e:
    logger.debug("Integration 5 (BMSControlBridge) not available: %s", e)

# ---------------------------------------------------------------------------
# Meter Data Bridge
# ---------------------------------------------------------------------------
try:
    from .meter_data_bridge import (
        BaselineData,
        DataQuality,
        IntervalData,
        IntervalLength,
        MeterChannel,
        MeterConfig,
        MeterDataBridge,
        MeterProtocol,
        MeterReading,
    )
    _loaded_integrations.append("MeterDataBridge")
except ImportError as e:
    logger.debug("Integration 6 (MeterDataBridge) not available: %s", e)

# ---------------------------------------------------------------------------
# DER Asset Bridge
# ---------------------------------------------------------------------------
try:
    from .der_asset_bridge import (
        DERAssetBridge,
        DERAssetConnection,
        DERAssetType,
        DERBridgeConfig,
        DERCommand,
        DERConnectionStatus,
        DEROperatingMode,
        DERProtocol,
        DERStatus,
    )
    _loaded_integrations.append("DERAssetBridge")
except ImportError as e:
    logger.debug("Integration 7 (DERAssetBridge) not available: %s", e)

# ---------------------------------------------------------------------------
# PACK-036 Bridge
# ---------------------------------------------------------------------------
try:
    from .pack036_bridge import (
        DemandChargeProfile,
        DemandChargeType,
        Pack036Bridge,
        Pack036Config,
        RateStructure,
        RateType,
        TariffData,
        TOUPeriod,
    )
    _loaded_integrations.append("Pack036Bridge")
except ImportError as e:
    logger.debug("Integration 8 (Pack036Bridge) not available: %s", e)

# ---------------------------------------------------------------------------
# PACK-033 Bridge
# ---------------------------------------------------------------------------
try:
    from .pack033_bridge import (
        DREnablementMeasure,
        Pack033Bridge,
        Pack033Config,
        Pack033ImportResult,
        QuickWinDRCategory,
        QuickWinMeasure,
    )
    _loaded_integrations.append("Pack033Bridge")
except ImportError as e:
    logger.debug("Integration 9 (Pack033Bridge) not available: %s", e)

# ---------------------------------------------------------------------------
# Health Check
# ---------------------------------------------------------------------------
try:
    from .health_check import (
        HealthCategory,
        HealthCheck,
        HealthCheckConfig,
        HealthResult,
        HealthSeverity,
        HealthStatus,
        SystemHealth,
    )
    _loaded_integrations.append("HealthCheck")
except ImportError as e:
    logger.debug("Integration 10 (HealthCheck) not available: %s", e)

# ---------------------------------------------------------------------------
# Setup Wizard
# ---------------------------------------------------------------------------
try:
    from .setup_wizard import (
        FacilityDRProfile,
        PresetConfig,
        SetupResult,
        SetupWizard,
        StepStatus,
        WizardConfig,
        WizardStep,
        WizardStepState,
    )
    _loaded_integrations.append("SetupWizard")
except ImportError as e:
    logger.debug("Integration 11 (SetupWizard) not available: %s", e)

# ---------------------------------------------------------------------------
# Alert Bridge
# ---------------------------------------------------------------------------
try:
    from .alert_bridge import (
        AlertBridge,
        AlertChannel,
        AlertConfig,
        AlertMessage,
        AlertSeverity,
        AlertType,
        EscalationLevel,
        EscalationRule,
        NotificationResult,
    )
    _loaded_integrations.append("AlertBridge")
except ImportError as e:
    logger.debug("Integration 12 (AlertBridge) not available: %s", e)


__all__: list[str] = [
    # Module metadata
    "__version__",
    "__pack_id__",
    "__pack_name__",
    "__integrations_count__",
    # --- Pack Orchestrator ---
    "DemandResponseOrchestrator",
    "PipelineConfig",
    "RetryConfig",
    "OrchestratorPhase",
    "FacilityType",
    "ExecutionStatus",
    "PhaseProvenance",
    "PhaseResult",
    "PipelineResult",
    "PHASE_DEPENDENCIES",
    "PHASE_EXECUTION_ORDER",
    "PARALLEL_PHASE_GROUPS",
    # --- MRV Bridge ---
    "MRVBridge",
    "MRVRouteConfig",
    "MRVRequest",
    "MRVResponse",
    "EmissionFactorSet",
    "DREmissionCategory",
    "MRVScope",
    "MARGINAL_EMISSION_FACTORS",
    # --- Data Bridge ---
    "DataBridge",
    "DataRouteConfig",
    "DataRequest",
    "DataResponse",
    "DataQualityReport",
    "DRDataSource",
    # --- Grid Signal Bridge ---
    "GridSignalBridge",
    "GridSignalConfig",
    "GridSignal",
    "DispatchNotification",
    "PriceSignal",
    "CarbonIntensitySignal",
    "SignalType",
    "SignalStatus",
    "SignalSeverity",
    "GridRegion",
    "CarbonSource",
    # --- BMS Control Bridge ---
    "BMSControlBridge",
    "BMSControlConfig",
    "BMSEndpoint",
    "ControlCommand",
    "ControlResponse",
    "DeviceStatus",
    "ControlAction",
    "ControlStatus",
    "ConnectionStatus",
    "DeviceCategory",
    "ProtocolType",
    # --- Meter Data Bridge ---
    "MeterDataBridge",
    "MeterConfig",
    "MeterReading",
    "IntervalData",
    "BaselineData",
    "MeterProtocol",
    "IntervalLength",
    "MeterChannel",
    "DataQuality",
    # --- DER Asset Bridge ---
    "DERAssetBridge",
    "DERBridgeConfig",
    "DERAssetConnection",
    "DERCommand",
    "DERStatus",
    "DERAssetType",
    "DERProtocol",
    "DEROperatingMode",
    "DERConnectionStatus",
    # --- PACK-036 Bridge ---
    "Pack036Bridge",
    "Pack036Config",
    "RateStructure",
    "TariffData",
    "DemandChargeProfile",
    "RateType",
    "TOUPeriod",
    "DemandChargeType",
    # --- PACK-033 Bridge ---
    "Pack033Bridge",
    "Pack033Config",
    "Pack033ImportResult",
    "QuickWinMeasure",
    "DREnablementMeasure",
    "QuickWinDRCategory",
    # --- Health Check ---
    "HealthCheck",
    "HealthCheckConfig",
    "SystemHealth",
    "HealthResult",
    "HealthSeverity",
    "HealthStatus",
    "HealthCategory",
    # --- Setup Wizard ---
    "SetupWizard",
    "WizardStep",
    "StepStatus",
    "FacilityDRProfile",
    "PresetConfig",
    "WizardStepState",
    "WizardConfig",
    "SetupResult",
    # --- Alert Bridge ---
    "AlertBridge",
    "AlertConfig",
    "AlertMessage",
    "EscalationRule",
    "AlertSeverity",
    "AlertChannel",
    "AlertType",
    "EscalationLevel",
    "NotificationResult",
]


def get_loaded_integrations() -> list[str]:
    """Return list of integration class names that loaded successfully."""
    return list(_loaded_integrations)


logger.info(
    "PACK-037 Demand Response integrations: %d/%d loaded",
    len(_loaded_integrations),
    __integrations_count__,
)
