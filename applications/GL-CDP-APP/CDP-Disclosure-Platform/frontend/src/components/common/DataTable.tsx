...
function DataTable<T>({
  columns, rows, rowKey, searchable = true,
  searchPlaceholder = 'Search...', defaultSort, defaultOrder = 'asc',
  rowsPerPageOptions = [10, 25, 50], onRowClick, dense = false, toolbar,
}: DataTableProps<T>) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) =>
      columns.some((col) => {
        const val = col.getValue ? col.getValue(row) : (row as Record<string, unknown>)[col.id];
  ...
  const sortedRows = useMemo(() => {
    ...
    return [...filteredRows].sort((a, b) => {
      const aVal = col?.getValue ? col.getValue(a) : (a as Record<string, unknown>)[orderBy];
      const bVal = col?.getValue ? col.getValue(b) : (b as Record<string, unknown>)[orderBy];
