...
import {
  ...
  TableSortLabel, TablePagination, Paper, TextField, Box, Typography, InputAdornment,
} from '@mui/material';
...
function DataTable<T>({
  columns, data, keyAccessor, title, searchPlaceholder = 'Search...',
  defaultSortColumn, defaultSortDirection = 'asc', onRowClick,
  rowsPerPageOptions = [10, 25, 50], emptyMessage = 'No data available', toolbar,
}: DataTableProps<T>) {
  ...
  return (
    <Paper sx={{ width: '100%' }}>
      <Box sx={{ p: 2, display: 'flex', alignItems: 'center', gap: 2 }}>
        {title && <Typography variant="h6" sx={{ fontWeight: 600, flexShrink: 0 }}>{title}</Typography>}
        <TextField
          ...
          <TableBody>
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((row) => (
                <TableRow key={keyAccessor(row)} hover onClick={onRowClick ? () => onRowClick(row) : undefined}
                  sx={{ cursor: onRowClick ? 'pointer' : 'default' }}>
                  {columns.map((col) => (
                    <TableCell key={col.id} align={col.align || 'left'}>{col.accessor(row)}</TableCell>
                  ))}
                </TableRow>
              ))
            )}
      ...
      <TablePagination
        component="div" count={filteredAndSortedData.length} page={page}
