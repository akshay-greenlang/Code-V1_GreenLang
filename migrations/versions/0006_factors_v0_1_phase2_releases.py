"""factors v0.1 phase 2 — api_keys + entitlements + release_manifests
(mirror of V504)

Revision ID: 0006_factors_v0_1_phase2_releases
Revises: 0005_factors_v0_1_phase2_provenance_changelog
Create Date: 2026-04-27

Operator-facing Alembic mirror of:
    upgrade()    -> reads V504__factors_v0_1_phase2_apikeys_entitlements_releases.sql
    downgrade()  -> reads V504__factors_v0_1_phase2_apikeys_entitlements_releases_DOWN.sql

The two SQL files are the source of truth. The factor catalog access-control
plane (`api_keys` + `entitlements` + `release_manifests`) is independent of
WS5 activity (V502); this revision can apply on any topology where 0005 is
already applied.
...
revision: str = "0006_factors_v0_1_phase2_releases"
...
UP_SQL_FILENAME = "V504__factors_v0_1_phase2_apikeys_entitlements_releases.sql"
DOWN_SQL_FILENAME = (
...
def _execute_sql_file(filename: str) -> None:
    ...
    if not sql_path.exists():
        raise FileNotFoundError(
            ...
            "Phase 2 V504 (factors api_keys + entitlements + releases) "
...
def upgrade() -> None:
    """Apply the V504 Phase 2 api_keys + entitlements + releases schema."""
    _execute_sql_file(UP_SQL_FILENAME)


def downgrade() -> None:
    """Revert the V504 Phase 2 api_keys + entitlements + releases schema."""
