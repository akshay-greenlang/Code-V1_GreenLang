...
def verify(
    ...
    if artifact_path.exists():
            ...
            if sbom_path.exists():
                ...
                try:
                    ...
                    if verbose:
                        ...
                        console.print(f"  Format: {sbom_data.get('spdxVersion', 'Unknown')}")
                        console.print(f"  Created: {sbom_data.get('creationInfo', {}).get('created', 'Unknown')}")
        ...
        if provenance_path.exists():
            ...
            try:
                ...
                if verbose:
                    ...
                    console.print(f"  Builder: {prov_data.get('builder', {}).get('id', 'Unknown')}")
        ...
        console.print(f"[cyan]Verifying pack: {pack.manifest.name} v{pack.manifest.version}[/cyan]\n")
...
def show_sbom(
    ...
    format: str = typer.Option("table", "--format", "-f", help="Output format (table|json|spdx)")
):
...
def show_provenance(
    artifact: str = typer.Argument(..., help="Artifact to inspect")
):
