...
def run_golden_test(
    ...
    comparison = compare_outputs(
        result.data,
        test_spec["expected_outputs"],
        test_spec.get("tolerance", 1e-6)
    )
    ...
    def compare_values(a, b, path=""):
        ...
        if isinstance(a, (int, float)):
            ...
            if set(a.keys()) != set(b.keys()):
                return False, f"Keys mismatch at {path}: {set(a.keys())} vs {set(b.keys())}"
...
def generate_golden_output(
    ...
    for i in range(runs):
        ...
        if not result.success:
            raise RuntimeError(f"Pipeline execution failed on run {i+1}: {result.error}")
