"""
GL-001 ThermalCommand Orchestrator - Cascade Control Module
================================================================================

Master-slave PID cascade control hierarchy implementation for
process heat systems. Provides coordinated control across multiple
equipment units with proper gain scheduling and anti-windup.

CONTROL THEORY FOUNDATIONS
--------------------------

**Standard PID Algorithm (Positional Form):**

The PID controller output u(t) is computed as:

    u(t) = K_p * e(t) + K_i * integral(e(t)) dt + K_d * de(t)/dt

Or in terms of controller gains:

    u(t) = K_p * [ e(t) + (1/T_i) * integral(e(t)) dt + T_d * de(t)/dt ]

Where:
    - e(t)    : Error = SP - PV (setpoint minus process value)
    - K_p     : Proportional gain (dimensionless or %/unit)
    - K_i     : Integral gain = K_p / T_i (1/seconds)
    - K_d     : Derivative gain = K_p * T_d (seconds)
    - T_i     : Integral time constant (seconds)
    - T_d     : Derivative time constant (seconds)

**Discrete-Time Implementation (Velocity Form):**

For digital implementation with sample time dt:

    Proportional:  P(k) = K_p * e(k)

    Integral:      I(k) = I(k-1) + K_i * e(k) * dt

    Derivative:    D(k) = K_d * (e(k) - e(k-1)) / dt
                   OR (derivative on PV, preferred):
                   D(k) = -K_d * (PV(k) - PV(k-1)) / dt

    Output:        u(k) = P(k) + I(k) + D(k)

**Derivative on PV (Setpoint Kick Prevention):**

Using derivative on PV instead of error prevents "setpoint kick"
when SP changes abruptly:

    D(k) = -K_d * (PV(k) - PV(k-1)) / dt

This is the preferred implementation for process control.

**Derivative Filter:**

A first-order filter on the derivative term reduces noise sensitivity:

    D_filtered(k) = alpha * D(k) + (1 - alpha) * D_filtered(k-1)

Where alpha is the filter coefficient (0 < alpha <= 1).
Typical value: alpha = 0.1 (heavy filtering).

ANTI-WINDUP MECHANISM
---------------------

**Problem:** Integral windup occurs when the controller output saturates
but the integral term continues to accumulate, causing overshoot when
the error changes sign.

**Solution: Back-Calculation Anti-Windup**

When output is clamped:

    u_raw = P + I + D
    u_clamped = clamp(u_raw, u_min, u_max)

    Anti-windup correction:
    I(k) = I(k) + K_aw * (u_clamped - u_raw)

Where K_aw is the anti-windup gain (typically 1.0).

This "back-calculates" the integral to prevent windup while
maintaining the clamped output.

Reference: Astrom & Hagglund, "PID Controllers: Theory, Design,
and Tuning" (1995), Section 3.5.

GAIN SCHEDULING
---------------

**Concept:** Process dynamics change with operating point (temperature,
pressure, load). Gain scheduling adjusts PID tuning to match.

**Implementation:**

For operating regions R_1, R_2, ..., R_n with tuning sets T_1, T_2, ..., T_n:

    if PV in R_i:
        active_tuning = blend(current_tuning, T_i, rate)

Where blend performs smooth transition:

    K_p_new = (1 - rate) * K_p_current + rate * K_p_target

Typical regions for thermal processes:
- Startup: Aggressive tuning, high K_p
- Low load: Conservative tuning
- Normal: Balanced tuning
- High load: Faster response
- Shutdown: Safe, slow response

BUMPLESS TRANSFER
-----------------

**Problem:** Switching between Auto/Manual/Cascade modes can cause
output discontinuities ("bumps") that upset the process.

**Solution:** Back-calculate integral term to match current output.

**Manual to Auto Transfer:**

    When switching from Manual to Auto:
    I(k) = u_manual - P(k) - D(k)

This ensures the controller output equals the manual output
at the moment of transfer.

**Cascade Mode:**

In cascade mode, the slave controller's setpoint comes from
the master controller's output:

    SP_slave = f(u_master)

The slave controller tracks the master output for smooth
transitions when cascade is enabled/disabled.

CASCADE CONTROL STRUCTURE
-------------------------

**Typical Configuration:**

    +----------+     +----------+     +---------+     +-----------+
    |  Master  |---->|  Slave   |---->| Process |---->| Sensor    |
    |   PID    |     |   PID    |     |(Actuator)|    |           |
    +----------+     +----------+     +---------+     +-----------+
         ^                                                  |
         |                                                  |
         +--------------------------------------------------+
                         (Primary Variable)

**Example: Temperature-to-Flow Cascade**

- Master: Temperature controller (TIC-101)
  - SP: Desired temperature
  - PV: Process temperature
  - OUT: Setpoint for flow controller

- Slave: Flow controller (FIC-101)
  - SP: From master output (cascade)
  - PV: Fuel flow rate
  - OUT: Valve position

**Benefits:**
1. Faster rejection of disturbances in inner loop
2. Linearization of nonlinear actuators
3. Protection of inner loop from process upsets

Key Features:
    - Master-slave PID cascade architecture
    - Gain scheduling based on operating region
    - Anti-windup with back-calculation
    - Feedforward compensation
    - Bumpless transfer between modes
    - Auto/Manual/Cascade mode switching
    - Rate limiting and output clamping
    - Comprehensive audit trail

Reference Standards:
    - ISA-5.1 Instrumentation Symbols and Identification
    - ISA-88 Batch Control
    - IEC 61131-3 PLC Programming Languages

Example:
    >>> from greenlang.agents.process_heat.gl_001_thermal_command.cascade_control import (
    ...     CascadeController, PIDController, PIDTuning
    ... )
    >>>
    >>> # Create master temperature controller
    >>> master = PIDController(
    ...     name="TC-101",
    ...     tuning=PIDTuning(kp=2.0, ki=0.1, kd=0.5),
    ...     output_min=0.0, output_max=100.0
    ... )
    >>>
    >>> # Create slave flow controller
    >>> slave = PIDController(
    ...     name="FC-101",
    ...     tuning=PIDTuning(kp=1.0, ki=0.5, kd=0.0),
    ...     output_min=0.0, output_max=100.0
    ... )
    >>>
    >>> # Create cascade
    >>> cascade = CascadeController(master=master, slave=slave)
    >>> output = cascade.calculate(temp_pv=450.0, temp_sp=500.0, flow_pv=75.0)

Author: GreenLang Control Systems Team
Version: 1.0.0
"""

from abc import ABC, abstractmethod
from dataclasses import dataclass, field
from datetime import datetime, timezone, timedelta
from enum import Enum, auto
from typing import Any, Callable, Dict, List, Optional, Tuple, Union
import hashlib
import logging
import math
import uuid

from pydantic import BaseModel, Field, field_validator

logger = logging.getLogger(__name__)


# =============================================================================
# ENUMERATIONS
# =============================================================================

class ControlMode(str, Enum):
    """
    Controller operating mode per ISA-5.1 conventions.

    Each mode defines how the controller generates its output:
    - AUTO: Output computed from PID algorithm
    - MANUAL: Output set directly by operator
    - CASCADE: Setpoint comes from upstream controller
    - REMOTE: Setpoint comes from supervisory system
    - LOCAL: Setpoint set at local panel
    - TRACK: Output tracks external signal (for bumpless transfer)
    """
    AUTO = "auto"           # Automatic control active
    MANUAL = "manual"       # Manual output control
    CASCADE = "cascade"     # Cascade from master controller
    REMOTE = "remote"       # Remote setpoint from DCS
    LOCAL = "local"         # Local setpoint
    TRACK = "track"         # Tracking external signal


class ControlAction(str, Enum):
    """
    Controller action direction (direct/reverse acting).

    DIRECT: Increasing PV requires increasing output
            Example: Heating with electric heater
            Error = SP - PV, positive error -> increase output

    REVERSE: Increasing PV requires decreasing output (most common)
             Example: Cooling with cooling water valve
             Error = SP - PV, positive error -> decrease output
             Implementation: Negate error internally
    """
    DIRECT = "direct"       # Increasing PV requires increasing output
    REVERSE = "reverse"     # Increasing PV requires decreasing output


class AlarmPriority(str, Enum):
    """
    Alarm priority levels per ISA-18.2 standard.

    Priority determines response requirements:
    - LOW: Operator awareness, no immediate action required
    - MEDIUM: Action required within shift
    - HIGH: Immediate action required
    - CRITICAL: Immediate action, potential safety impact
    - EMERGENCY: Safety critical, automated response may trigger
    """
    LOW = "low"
    MEDIUM = "medium"
    HIGH = "high"
    CRITICAL = "critical"
    EMERGENCY = "emergency"


class GainScheduleRegion(str, Enum):
    """
    Operating regions for gain scheduling.

    Each region may have different PID tuning to account for
    nonlinear process dynamics:

    STARTUP: Process heating up, aggressive control needed
             Typical: Higher K_p, lower K_i (avoid overshoot)

    LOW_LOAD: Operating below 30% capacity
              Typical: Conservative tuning, process may be sluggish

    NORMAL: Operating in 30-80% range
            Typical: Balanced tuning from design calculations

    HIGH_LOAD: Operating above 80% capacity
               Typical: Faster response, tighter control

    SHUTDOWN: Process cooling down
              Typical: Very conservative, safety-focused
    """
    STARTUP = "startup"
    LOW_LOAD = "low_load"
    NORMAL = "normal"
    HIGH_LOAD = "high_load"
    SHUTDOWN = "shutdown"


# =============================================================================
# DATA MODELS
# =============================================================================

class PIDTuning(BaseModel):
    """
    PID controller tuning parameters.

    TUNING PARAMETER RELATIONSHIPS
    ------------------------------

    Standard (ISA) Form:
        u(t) = K_p * [ e(t) + (1/T_i) * integral(e) + T_d * de/dt ]

    Parallel (Independent) Form (used here):
        u(t) = K_p * e(t) + K_i * integral(e) + K_d * de/dt

    Conversions:
        K_i = K_p / T_i
        K_d = K_p * T_d

    TUNING GUIDELINES (Ziegler-Nichols)
    -----------------------------------

    For a First-Order Plus Dead Time (FOPDT) process with:
    - K: Process gain
    - tau: Time constant
    - theta: Dead time

    Ultimate Gain Method:
        K_u = Ultimate gain (at sustained oscillation)
        T_u = Ultimate period

        P only:   K_p = 0.5 * K_u
        PI:       K_p = 0.45 * K_u, T_i = T_u / 1.2
        PID:      K_p = 0.6 * K_u, T_i = T_u / 2, T_d = T_u / 8

    Cohen-Coon (for processes with dead time):
        K_p = (1.35 / K) * (tau / theta + 0.18)
        T_i = theta * (2.5 - 2 * theta / (tau + 0.185 * theta))
        T_d = 0.37 * theta / (tau / theta + 0.185)

    Lambda Tuning (for stability):
        lambda_cl = Desired closed-loop time constant
        K_p = tau / (K * (lambda_cl + theta))
        T_i = tau
        T_d = 0 (PI only for robustness)

    Attributes:
        kp: Proportional gain (dimensionless or engineering units)
        ki: Integral gain (1/seconds)
        kd: Derivative gain (seconds)
        td_filter: Derivative filter coefficient (0-1)
        anti_windup_gain: Back-calculation gain for anti-windup
    """
    kp: float = Field(
        default=1.0,
        description="Proportional gain K_p"
    )
    ki: float = Field(
        default=0.1,
        ge=0,
        description="Integral gain K_i = K_p / T_i (1/seconds)"
    )
    kd: float = Field(
        default=0.0,
        ge=0,
        description="Derivative gain K_d = K_p * T_d (seconds)"
    )
    td_filter: float = Field(
        default=0.1,
        ge=0,
        le=1,
        description="Derivative filter coefficient alpha (0=heavy filter, 1=no filter)"
    )
    anti_windup_gain: float = Field(
        default=1.0,
        ge=0,
        description="Anti-windup back-calculation gain K_aw"
    )

    @property
    def ti(self) -> float:
        """
        Get integral time T_i = K_p / K_i.

        Returns:
            Integral time in seconds, inf if K_i = 0
        """
        return self.kp / self.ki if self.ki > 0 else float('inf')

    @property
    def td(self) -> float:
        """
        Get derivative time T_d = K_d / K_p.

        Returns:
            Derivative time in seconds
        """
        return self.kd / self.kp if self.kp > 0 else 0.0


class GainScheduleEntry(BaseModel):
    """
    Gain schedule entry for a specific operating region.

    Defines the PID tuning to use when the process variable
    falls within the specified range.

    TRANSITION STRATEGY
    -------------------

    When transitioning between regions, gains are smoothly blended:

        K_new = (1 - rate) * K_current + rate * K_target

    This prevents sudden changes in controller behavior.
    Typical transition_rate: 0.05-0.2 per calculation cycle.
    """
    region: GainScheduleRegion = Field(..., description="Operating region identifier")
    pv_low: float = Field(..., description="Lower PV bound for this region")
    pv_high: float = Field(..., description="Upper PV bound for this region")
    tuning: PIDTuning = Field(..., description="PID tuning for this region")
    transition_rate: float = Field(
        default=0.1,
        ge=0,
        le=1,
        description="Rate of transition to these gains (0-1 per cycle)"
    )


class ControllerState(BaseModel):
    """
    Internal state of a PID controller.

    Maintains the persistent values needed for discrete-time
    PID calculation between cycles.

    STATE VARIABLES
    ---------------

    integral: Accumulated integral term sum(K_i * e * dt)
              This is the "memory" of the controller.

    derivative_filter: Filtered derivative value for noise rejection.
              Updated each cycle: D_f = alpha * D + (1-alpha) * D_f

    last_error: Previous error value e(k-1) for derivative calculation.

    last_pv: Previous process value PV(k-1) for derivative-on-PV.

    last_output: Previous output u(k-1) for rate limiting.

    last_calculation_time: Timestamp for calculating dt.
    """
    integral: float = Field(default=0.0, description="Integral accumulator sum(K_i * e * dt)")
    derivative_filter: float = Field(default=0.0, description="Filtered derivative value")
    last_error: float = Field(default=0.0, description="Previous error e(k-1)")
    last_pv: float = Field(default=0.0, description="Previous PV for derivative-on-PV")
    last_output: float = Field(default=0.0, description="Previous output u(k-1)")
    last_calculation_time: Optional[datetime] = Field(
        default=None,
        description="Time of last calculation for dt"
    )


class ControllerAlarm(BaseModel):
    """
    Controller alarm definition per ISA-18.2.

    ALARM LOGIC
    -----------

    For HIGH alarm:
        Active when: PV > setpoint
        Clear when: PV < setpoint - deadband

    For LOW alarm:
        Active when: PV < setpoint
        Clear when: PV > setpoint + deadband

    TIME DELAY
    ----------

    Alarm activation can be delayed to prevent nuisance alarms:
        - Record timestamp when condition first detected
        - Only activate if condition persists for delay_seconds
        - Clear timestamp if condition clears before activation
    """
    alarm_id: str = Field(
        default_factory=lambda: str(uuid.uuid4())[:8],
        description="Alarm identifier"
    )
    name: str = Field(..., description="Alarm name/description")
    priority: AlarmPriority = Field(..., description="Alarm priority per ISA-18.2")
    setpoint: float = Field(..., description="Alarm setpoint (trip point)")
    deadband: float = Field(default=0.0, ge=0, description="Hysteresis deadband")
    direction: str = Field(default="high", description="'high' or 'low' alarm")
    delay_seconds: float = Field(default=0.0, ge=0, description="Time delay before activation")
    active: bool = Field(default=False, description="Alarm currently active")
    acknowledged: bool = Field(default=False, description="Alarm acknowledged by operator")
    activation_time: Optional[datetime] = Field(default=None, description="When alarm condition first detected")


class ControlOutput(BaseModel):
    """
    Output from a PID control calculation.

    Contains the controller output and diagnostic information
    for troubleshooting and audit trail.

    OUTPUT DECOMPOSITION
    --------------------

    The total output is the sum of components:

        u(k) = P(k) + I(k) + D(k) + FF

    Where:
        P(k) = K_p * e(k)                    Proportional term
        I(k) = I(k-1) + K_i * e(k) * dt      Integral term
        D(k) = K_d * d(PV)/dt (filtered)     Derivative term
        FF   = Feedforward contribution       Feedforward

    This decomposition helps identify:
    - Which term is dominating control action
    - If integral windup is occurring
    - If derivative is too aggressive (noisy)
    """
    output_id: str = Field(
        default_factory=lambda: str(uuid.uuid4())[:8],
        description="Output calculation ID"
    )
    controller_name: str = Field(..., description="Controller name/tag")
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="Calculation timestamp"
    )
    setpoint: float = Field(..., description="Setpoint value SP")
    process_value: float = Field(..., description="Process value PV")
    error: float = Field(..., description="Error e = SP - PV (or -(SP-PV) for reverse)")
    output: float = Field(..., description="Controller output u (0-100%)")
    output_clamped: bool = Field(
        default=False,
        description="Was output clamped to limits?"
    )
    mode: ControlMode = Field(..., description="Operating mode")
    p_term: float = Field(default=0.0, description="Proportional contribution P(k)")
    i_term: float = Field(default=0.0, description="Integral contribution I(k)")
    d_term: float = Field(default=0.0, description="Derivative contribution D(k)")
    feedforward: float = Field(default=0.0, description="Feedforward contribution FF")
    dt_seconds: float = Field(default=0.0, ge=0, description="Time step dt")
    gain_region: Optional[GainScheduleRegion] = Field(
        default=None,
        description="Active gain schedule region"
    )
    provenance_hash: str = Field(default="", description="SHA-256 audit hash")

    def model_post_init(self, __context: Any) -> None:
        """Calculate provenance hash."""
        if not self.provenance_hash:
            self.provenance_hash = self._calculate_provenance()

    def _calculate_provenance(self) -> str:
        """Calculate SHA-256 provenance hash for audit trail."""
        provenance_str = (
            f"{self.controller_name}|{self.timestamp.isoformat()}|"
            f"{self.setpoint:.6f}|{self.process_value:.6f}|{self.output:.6f}"
        )
        return hashlib.sha256(provenance_str.encode()).hexdigest()


class CascadeOutput(BaseModel):
    """
    Output from a cascade control calculation.

    Contains outputs from both master and slave controllers
    plus the final combined output.
    """
    cascade_id: str = Field(
        default_factory=lambda: str(uuid.uuid4())[:8],
        description="Cascade calculation ID"
    )
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="Calculation timestamp"
    )
    master_output: ControlOutput = Field(..., description="Master controller output")
    slave_output: ControlOutput = Field(..., description="Slave controller output")
    final_output: float = Field(..., description="Final control output (from slave)")
    cascade_active: bool = Field(
        default=True,
        description="Is cascade mode active?"
    )
    provenance_hash: str = Field(default="", description="SHA-256 audit hash")


# =============================================================================
# PID CONTROLLER
# =============================================================================

class PIDController:
    """
    Industrial PID Controller with anti-windup and gain scheduling.

    Implements a full-featured PID controller suitable for process heat
    applications with:
    - Configurable P, PI, PD, or PID action
    - Derivative on PV (not error) to avoid setpoint kick
    - Anti-windup via back-calculation
    - Output rate limiting
    - Bumpless transfer between modes
    - Gain scheduling
    - Feedforward support

    PID ALGORITHM (DISCRETE TIME)
    =============================

    For each calculation cycle k with sample time dt:

    1. ERROR CALCULATION:
       e(k) = SP - PV                          (direct action)
       e(k) = -(SP - PV) = PV - SP             (reverse action)

    2. PROPORTIONAL TERM:
       P(k) = K_p * e(k)

    3. INTEGRAL TERM (with anti-windup):
       I(k) = I(k-1) + K_i * e(k) * dt

       If output is clamped:
           I(k) = I(k) + K_aw * (u_clamped - u_raw)

    4. DERIVATIVE TERM (on PV, filtered):
       d_pv = (PV(k) - PV(k-1)) / dt
       D_raw(k) = -K_d * d_pv                   (negative for stability)
       D(k) = alpha * D_raw(k) + (1-alpha) * D(k-1)   (filter)

    5. OUTPUT:
       u_raw(k) = P(k) + I(k) + D(k) + FF
       u(k) = clamp(u_raw, u_min, u_max)

    6. RATE LIMITING (optional):
       if |u(k) - u(k-1)| > rate_limit * dt:
           u(k) = u(k-1) + sign(u(k) - u(k-1)) * rate_limit * dt

    TUNING PARAMETER GUIDANCE
    =========================

    For process heat applications (typical values):

    Temperature Control:
        K_p: 1-5 %/degF (or degC)
        T_i: 60-300 seconds
        T_d: 10-60 seconds (or 0 for PI)

    Pressure Control:
        K_p: 5-20 %/psi
        T_i: 30-120 seconds
        T_d: 0-30 seconds

    Flow Control:
        K_p: 0.5-2 %/%
        T_i: 10-60 seconds
        T_d: 0 (typically PI only)

    Example:
        >>> tuning = PIDTuning(kp=2.0, ki=0.1, kd=0.5)
        >>> controller = PIDController(
        ...     name="TC-101",
        ...     tuning=tuning,
        ...     output_min=0.0,
        ...     output_max=100.0,
        ...     action=ControlAction.REVERSE
        ... )
        >>> output = controller.calculate(setpoint=500.0, pv=450.0)
        >>> print(f"Output: {output.output:.1f}%")
    """

    def __init__(
        self,
        name: str,
        tuning: PIDTuning,
        output_min: float = 0.0,
        output_max: float = 100.0,
        action: ControlAction = ControlAction.REVERSE,
        sample_time_seconds: float = 1.0,
        rate_limit_per_second: Optional[float] = None,
        derivative_on_pv: bool = True,
        enable_gain_scheduling: bool = False
    ) -> None:
        """
        Initialize PID Controller.

        Args:
            name: Controller tag name (e.g., "TIC-101")
            tuning: PID tuning parameters (K_p, K_i, K_d)
            output_min: Minimum output value (typically 0%)
            output_max: Maximum output value (typically 100%)
            action: Controller action (direct/reverse)
            sample_time_seconds: Expected sample time for dt calculation
            rate_limit_per_second: Maximum output change rate (%/second)
            derivative_on_pv: Use derivative on PV (recommended True)
            enable_gain_scheduling: Enable gain scheduling feature
        """
        self.name = name
        self.tuning = tuning
        self.output_min = output_min
        self.output_max = output_max
        self.action = action
        self.sample_time = sample_time_seconds
        self.rate_limit = rate_limit_per_second
        self.derivative_on_pv = derivative_on_pv
        self.enable_gain_scheduling = enable_gain_scheduling

        # Internal state
        self._state = ControllerState()
        self._mode = ControlMode.AUTO
        self._manual_output = 50.0  # Default manual output
        self._setpoint = 0.0
        self._gain_schedules: List[GainScheduleEntry] = []
        self._active_tuning = tuning
        self._alarms: List[ControllerAlarm] = []
        self._feedforward_value = 0.0
        self._history: List[ControlOutput] = []

        # Tracking for bumpless transfer
        self._tracking_value: Optional[float] = None

        logger.info(
            "PID Controller initialized: %s (Kp=%.2f, Ki=%.2f, Kd=%.2f)",
            name, tuning.kp, tuning.ki, tuning.kd
        )

    # =========================================================================
    # CORE CALCULATION
    # =========================================================================

    def calculate(
        self,
        setpoint: float,
        pv: float,
        dt_seconds: Optional[float] = None,
        feedforward: float = 0.0
    ) -> ControlOutput:
        """
        Calculate PID output.

        ALGORITHM STEPS
        ---------------

        1. Determine time step dt (from argument or elapsed time)
        2. Handle manual mode separately
        3. Update gain scheduling if enabled
        4. Calculate error (with action direction)
        5. Calculate P, I, D terms
        6. Apply feedforward
        7. Clamp output to limits
        8. Apply anti-windup correction
        9. Apply rate limiting
        10. Update state variables
        11. Check alarms
        12. Return result with diagnostics

        Args:
            setpoint: Desired setpoint SP
            pv: Current process value PV
            dt_seconds: Time step (None uses elapsed time)
            feedforward: Feedforward contribution FF

        Returns:
            ControlOutput with calculation details
        """
        now = datetime.now(timezone.utc)

        # =================================================================
        # STEP 1: Calculate dt
        # =================================================================
        if dt_seconds is None:
            if self._state.last_calculation_time:
                dt_seconds = (now - self._state.last_calculation_time).total_seconds()
            else:
                dt_seconds = self.sample_time

        dt_seconds = max(dt_seconds, 0.001)  # Prevent division by zero

        # Update setpoint and feedforward
        self._setpoint = setpoint
        self._feedforward_value = feedforward

        # =================================================================
        # STEP 2: Handle manual mode
        # =================================================================
        if self._mode == ControlMode.MANUAL:
            return self._manual_calculate(setpoint, pv, dt_seconds, now)

        # =================================================================
        # STEP 3: Update gain scheduling
        # =================================================================
        if self.enable_gain_scheduling and self._gain_schedules:
            self._update_active_gains(pv)

        # =================================================================
        # STEP 4: Calculate error
        # =================================================================
        # Error = SP - PV (positive error means we're below setpoint)
        error = setpoint - pv

        # For REVERSE action (most common): negate error
        # This makes positive error -> positive output change
        # which closes the loop correctly for cooling/heating
        if self.action == ControlAction.REVERSE:
            error = -error

        # =================================================================
        # STEP 5: Calculate P term
        # =================================================================
        # P(k) = K_p * e(k)
        p_term = self._active_tuning.kp * error

        # =================================================================
        # STEP 6: Calculate I term (with accumulation)
        # =================================================================
        # I(k) = I(k-1) + K_i * e(k) * dt
        self._state.integral += self._active_tuning.ki * error * dt_seconds
        i_term = self._state.integral

        # =================================================================
        # STEP 7: Calculate D term (on PV, filtered)
        # =================================================================
        if self.derivative_on_pv:
            # Derivative on PV to avoid setpoint kick
            # d_pv = (PV(k) - PV(k-1)) / dt
            d_pv = (pv - self._state.last_pv) / dt_seconds
            # D(k) = -K_d * d_pv (negative because increasing PV should decrease output)
            d_term = -self._active_tuning.kd * d_pv
        else:
            # Derivative on error (not recommended)
            d_error = (error - self._state.last_error) / dt_seconds
            d_term = self._active_tuning.kd * d_error

        # Apply derivative filter: D_f = alpha * D + (1-alpha) * D_f
        self._state.derivative_filter = (
            self._active_tuning.td_filter * d_term +
            (1 - self._active_tuning.td_filter) * self._state.derivative_filter
        )
        d_term = self._state.derivative_filter

        # =================================================================
        # STEP 8: Calculate raw output
        # =================================================================
        # u_raw = P + I + D + FF
        raw_output = p_term + i_term + d_term + feedforward

        # =================================================================
        # STEP 9: Clamp output
        # =================================================================
        output_clamped = False
        output = raw_output
        if output > self.output_max:
            output = self.output_max
            output_clamped = True
        elif output < self.output_min:
            output = self.output_min
            output_clamped = True

        # =================================================================
        # STEP 10: Anti-windup via back-calculation
        # =================================================================
        # When output is clamped, adjust integral to prevent windup:
        # I(k) = I(k) + K_aw * (u_clamped - u_raw)
        if output_clamped:
            windup_correction = (output - raw_output) * self._active_tuning.anti_windup_gain
            self._state.integral += windup_correction

        # =================================================================
        # STEP 11: Apply rate limiting
        # =================================================================
        if self.rate_limit is not None:
            max_change = self.rate_limit * dt_seconds
            change = output - self._state.last_output
            if abs(change) > max_change:
                output = self._state.last_output + math.copysign(max_change, change)

        # =================================================================
        # STEP 12: Update state
        # =================================================================
        self._state.last_error = error
        self._state.last_pv = pv
        self._state.last_output = output
        self._state.last_calculation_time = now

        # =================================================================
        # STEP 13: Determine gain region
        # =================================================================
        gain_region = None
        if self.enable_gain_scheduling and self._gain_schedules:
            for schedule in self._gain_schedules:
                if schedule.pv_low <= pv <= schedule.pv_high:
                    gain_region = schedule.region
                    break

        # =================================================================
        # STEP 14: Build result
        # =================================================================
        result = ControlOutput(
            controller_name=self.name,
            timestamp=now,
            setpoint=setpoint,
            process_value=pv,
            # Report error in engineering units (positive = below SP)
            error=error if self.action == ControlAction.DIRECT else -error,
            output=output,
            output_clamped=output_clamped,
            mode=self._mode,
            p_term=p_term,
            i_term=i_term,
            d_term=d_term,
            feedforward=feedforward,
            dt_seconds=dt_seconds,
            gain_region=gain_region,
        )

        # Store history (ring buffer)
        self._history.append(result)
        if len(self._history) > 1000:
            self._history = self._history[-500:]

        # =================================================================
        # STEP 15: Check alarms
        # =================================================================
        self._check_alarms(pv)

        return result

    def _manual_calculate(
        self,
        setpoint: float,
        pv: float,
        dt_seconds: float,
        now: datetime
    ) -> ControlOutput:
        """
        Calculate in manual mode with bumpless transfer preparation.

        BUMPLESS TRANSFER LOGIC
        -----------------------

        In manual mode, we back-calculate the integral term so that
        when switching to auto, the output matches the manual output:

            P(k) = K_p * e(k)
            D(k) = 0 (no derivative in manual)
            I(k) = u_manual - P(k)

        This ensures: u_auto = P + I + D = P + (u_manual - P) + 0 = u_manual
        """
        # Calculate error for P term
        error = setpoint - pv
        if self.action == ControlAction.REVERSE:
            error = -error

        # P term at current error
        p_term = self._active_tuning.kp * error
        d_term = 0.0  # No derivative in manual

        # Back-calculate integral to match manual output
        # This enables bumpless transfer: u = P + I + D = P + (u_man - P) + 0 = u_man
        self._state.integral = self._manual_output - p_term

        # Update state
        self._state.last_error = error
        self._state.last_pv = pv
        self._state.last_output = self._manual_output
        self._state.last_calculation_time = now

        return ControlOutput(
            controller_name=self.name,
            timestamp=now,
            setpoint=setpoint,
            process_value=pv,
            error=error if self.action == ControlAction.DIRECT else -error,
            output=self._manual_output,
            output_clamped=False,
            mode=ControlMode.MANUAL,
            p_term=p_term,
            i_term=self._state.integral,
            d_term=0.0,
            feedforward=0.0,
            dt_seconds=dt_seconds,
        )

    # =========================================================================
    # MODE CONTROL
    # =========================================================================

    def set_mode(self, mode: ControlMode) -> None:
        """
        Set controller mode with bumpless transfer.

        BUMPLESS TRANSFER IMPLEMENTATION
        ---------------------------------

        Manual -> Auto: Integral already back-calculated in _manual_calculate
        Auto -> Manual: Set manual output to current auto output
        Any -> Cascade: Prepare for external setpoint
        Cascade -> Auto: Setpoint reverts to local control

        Args:
            mode: New operating mode
        """
        if mode == self._mode:
            return

        old_mode = self._mode

        # Bumpless transfer preparation
        if mode == ControlMode.AUTO and old_mode == ControlMode.MANUAL:
            # Integral already back-calculated in _manual_calculate
            pass

        elif mode == ControlMode.MANUAL and old_mode == ControlMode.AUTO:
            # Set manual output to current auto output
            self._manual_output = self._state.last_output

        elif mode == ControlMode.CASCADE:
            # Prepare for external setpoint
            pass

        self._mode = mode
        logger.info(
            "Controller %s mode changed: %s -> %s",
            self.name, old_mode.value, mode.value
        )

    def set_manual_output(self, output: float) -> None:
        """Set manual output value (clamped to limits)."""
        self._manual_output = max(self.output_min, min(self.output_max, output))

    @property
    def mode(self) -> ControlMode:
        """Get current mode."""
        return self._mode

    # =========================================================================
    # GAIN SCHEDULING
    # =========================================================================

    def add_gain_schedule(self, entry: GainScheduleEntry) -> None:
        """
        Add a gain schedule entry.

        Entries are sorted by pv_low for efficient lookup.

        Args:
            entry: GainScheduleEntry with region bounds and tuning
        """
        self._gain_schedules.append(entry)
        # Sort by pv_low for efficient lookup
        self._gain_schedules.sort(key=lambda x: x.pv_low)
        logger.info(
            "Gain schedule added for %s: %s (PV %.1f-%.1f)",
            self.name, entry.region.value, entry.pv_low, entry.pv_high
        )

    def _update_active_gains(self, pv: float) -> None:
        """
        Update active tuning based on current PV.

        GAIN BLENDING
        -------------

        When PV enters a new region, gains are smoothly blended:

            K_new = (1 - rate) * K_current + rate * K_target

        This prevents sudden control action changes.
        """
        for schedule in self._gain_schedules:
            if schedule.pv_low <= pv <= schedule.pv_high:
                # Smooth transition to new gains
                rate = schedule.transition_rate
                self._active_tuning = PIDTuning(
                    kp=self._active_tuning.kp * (1 - rate) + schedule.tuning.kp * rate,
                    ki=self._active_tuning.ki * (1 - rate) + schedule.tuning.ki * rate,
                    kd=self._active_tuning.kd * (1 - rate) + schedule.tuning.kd * rate,
                    td_filter=schedule.tuning.td_filter,
                    anti_windup_gain=schedule.tuning.anti_windup_gain,
                )
                return

    # =========================================================================
    # ALARMS
    # =========================================================================

    def add_alarm(self, alarm: ControllerAlarm) -> None:
        """Add an alarm to the controller."""
        self._alarms.append(alarm)
        logger.info(
            "Alarm added to %s: %s (%s, setpoint=%.1f)",
            self.name, alarm.name, alarm.priority.value, alarm.setpoint
        )

    def _check_alarms(self, pv: float) -> None:
        """Check all alarms against current PV."""
        now = datetime.now(timezone.utc)

        for alarm in self._alarms:
            # Determine if alarm condition is met
            if alarm.direction == "high":
                in_alarm = pv > alarm.setpoint
                clear_point = alarm.setpoint - alarm.deadband
                is_clear = pv < clear_point
            else:  # low alarm
                in_alarm = pv < alarm.setpoint
                clear_point = alarm.setpoint + alarm.deadband
                is_clear = pv > clear_point

            # Handle alarm activation with delay
            if in_alarm and not alarm.active:
                # Alarm activating
                if alarm.delay_seconds > 0 and alarm.activation_time is None:
                    # Start delay timer
                    alarm.activation_time = now
                elif alarm.activation_time is None or (
                    now - alarm.activation_time
                ).total_seconds() >= alarm.delay_seconds:
                    # Delay elapsed or no delay configured
                    alarm.active = True
                    alarm.acknowledged = False
                    logger.warning(
                        "ALARM ACTIVATED: %s.%s (PV=%.1f, SP=%.1f)",
                        self.name, alarm.name, pv, alarm.setpoint
                    )

            elif is_clear and alarm.active:
                # Alarm clearing (with deadband)
                alarm.active = False
                alarm.activation_time = None
                logger.info(
                    "Alarm cleared: %s.%s",
                    self.name, alarm.name
                )

    def get_active_alarms(self) -> List[ControllerAlarm]:
        """Get all active alarms."""
        return [a for a in self._alarms if a.active]

    # =========================================================================
    # STATE AND HISTORY
    # =========================================================================

    def reset(self) -> None:
        """Reset controller state (integral, derivative filter, etc.)."""
        self._state = ControllerState()
        self._active_tuning = self.tuning
        logger.info("Controller %s reset", self.name)

    def get_history(self, limit: int = 100) -> List[ControlOutput]:
        """Get calculation history."""
        return list(reversed(self._history[-limit:]))

    def get_status(self) -> Dict[str, Any]:
        """Get controller status summary."""
        return {
            "name": self.name,
            "mode": self._mode.value,
            "setpoint": self._setpoint,
            "last_output": self._state.last_output,
            "last_pv": self._state.last_pv,
            "integral": self._state.integral,
            "active_tuning": {
                "kp": self._active_tuning.kp,
                "ki": self._active_tuning.ki,
                "kd": self._active_tuning.kd,
            },
            "active_alarms": len(self.get_active_alarms()),
        }


# =============================================================================
# CASCADE CONTROLLER
# =============================================================================

class CascadeController:
    """
    Master-Slave Cascade Controller.

    Implements a cascade control structure where the master controller
    output becomes the setpoint for the slave controller. This provides
    improved rejection of disturbances affecting the inner loop.

    CASCADE CONTROL THEORY
    ======================

    **Transfer Function Analysis:**

    For a cascade system with:
    - G_m: Master controller transfer function
    - G_s: Slave controller transfer function
    - P_o: Outer process (master)
    - P_i: Inner process (slave)

    Closed-loop transfer function:

                    G_m * G_s * P_i * P_o
        Y/R = -------------------------------
              1 + G_s * P_i + G_m * G_s * P_i * P_o

    **Disturbance Rejection:**

    For a disturbance D entering the inner loop:

                    P_i * P_o
        Y/D = -----------------------
              1 + G_s * P_i + G_m * G_s * P_i * P_o

    The slave controller G_s provides rejection of inner loop
    disturbances before they affect the outer loop.

    **Design Guidelines:**

    1. Inner loop (slave) should be 3-10x faster than outer loop (master)
    2. Tune slave first with master in manual
    3. Then tune master with slave in cascade
    4. Master should be more conservative than standalone

    TYPICAL APPLICATIONS
    ====================

    Temperature -> Flow Cascade:
    - Master: Temperature controller (TIC)
    - Slave: Flow controller (FIC) for fuel
    - Benefit: Fast rejection of fuel pressure disturbances

    Pressure -> Flow Cascade:
    - Master: Pressure controller (PIC)
    - Slave: Flow controller (FIC) for steam
    - Benefit: Fast rejection of demand changes

    Level -> Flow Cascade:
    - Master: Level controller (LIC)
    - Slave: Flow controller (FIC) for makeup
    - Benefit: Smooth level control despite flow disturbances

    Example:
        >>> # Master: Temperature controller
        >>> master = PIDController(
        ...     name="TIC-101",
        ...     tuning=PIDTuning(kp=2.0, ki=0.1, kd=0.5),
        ...     output_min=0.0, output_max=100.0
        ... )
        >>>
        >>> # Slave: Flow controller
        >>> slave = PIDController(
        ...     name="FIC-101",
        ...     tuning=PIDTuning(kp=1.0, ki=0.5, kd=0.0),
        ...     output_min=0.0, output_max=100.0
        ... )
        >>>
        >>> cascade = CascadeController(master=master, slave=slave)
        >>> cascade.set_master_setpoint(500.0)
        >>> result = cascade.calculate(
        ...     master_pv=450.0,
        ...     slave_pv=75.0
        ... )
    """

    def __init__(
        self,
        master: PIDController,
        slave: PIDController,
        slave_sp_min: Optional[float] = None,
        slave_sp_max: Optional[float] = None,
        ratio: float = 1.0
    ) -> None:
        """
        Initialize Cascade Controller.

        Args:
            master: Master (outer loop) controller
            slave: Slave (inner loop) controller
            slave_sp_min: Minimum slave setpoint (clamp master output)
            slave_sp_max: Maximum slave setpoint (clamp master output)
            ratio: Ratio applied to master output for slave SP
                   SP_slave = ratio * output_master
        """
        self.master = master
        self.slave = slave
        self.slave_sp_min = slave_sp_min or slave.output_min
        self.slave_sp_max = slave_sp_max or slave.output_max
        self.ratio = ratio

        self._master_setpoint = 0.0
        self._cascade_active = True
        self._history: List[CascadeOutput] = []

        logger.info(
            "Cascade controller initialized: %s (master) -> %s (slave)",
            master.name, slave.name
        )

    # =========================================================================
    # CORE CALCULATION
    # =========================================================================

    def calculate(
        self,
        master_pv: float,
        slave_pv: float,
        master_feedforward: float = 0.0,
        slave_feedforward: float = 0.0,
        dt_seconds: Optional[float] = None
    ) -> CascadeOutput:
        """
        Calculate cascade control output.

        CASCADE CALCULATION SEQUENCE
        ----------------------------

        1. Calculate master controller (outer loop):
           - SP: Set by operator or supervisory system
           - PV: Primary process variable (e.g., temperature)
           - OUT: Becomes slave setpoint

        2. Determine slave setpoint:
           - If cascade active: SP_slave = ratio * master_output
           - If not cascade: SP_slave = local slave setpoint
           - Apply slave_sp_min/max limits

        3. Calculate slave controller (inner loop):
           - SP: From master output (cascade) or local
           - PV: Secondary process variable (e.g., flow)
           - OUT: Final control output (e.g., valve position)

        Args:
            master_pv: Master process value (e.g., temperature)
            slave_pv: Slave process value (e.g., flow)
            master_feedforward: Feedforward to master controller
            slave_feedforward: Feedforward to slave controller
            dt_seconds: Time step (shared by both controllers)

        Returns:
            CascadeOutput with both controller outputs
        """
        # Calculate master controller (outer loop)
        master_output = self.master.calculate(
            setpoint=self._master_setpoint,
            pv=master_pv,
            dt_seconds=dt_seconds,
            feedforward=master_feedforward
        )

        # Determine slave setpoint from master output
        if self._cascade_active and self.master.mode != ControlMode.MANUAL:
            # Cascade mode: slave SP comes from master output
            slave_sp = master_output.output * self.ratio
            # Clamp slave setpoint to limits
            slave_sp = max(self.slave_sp_min, min(self.slave_sp_max, slave_sp))
        else:
            # Manual or decoupled mode: slave uses its own setpoint
            slave_sp = self.slave._setpoint

        # Calculate slave controller (inner loop)
        slave_output = self.slave.calculate(
            setpoint=slave_sp,
            pv=slave_pv,
            dt_seconds=dt_seconds,
            feedforward=slave_feedforward
        )

        # Build cascade output
        result = CascadeOutput(
            timestamp=datetime.now(timezone.utc),
            master_output=master_output,
            slave_output=slave_output,
            final_output=slave_output.output,
            cascade_active=self._cascade_active,
        )

        # Calculate provenance hash
        provenance_str = (
            f"{self.master.name}|{self.slave.name}|"
            f"{result.timestamp.isoformat()}|"
            f"{result.final_output:.4f}"
        )
        result.provenance_hash = hashlib.sha256(provenance_str.encode()).hexdigest()

        # Store history
        self._history.append(result)
        if len(self._history) > 1000:
            self._history = self._history[-500:]

        return result

    # =========================================================================
    # SETPOINT AND MODE CONTROL
    # =========================================================================

    def set_master_setpoint(self, setpoint: float) -> None:
        """Set master controller setpoint."""
        self._master_setpoint = setpoint
        logger.debug(
            "Cascade %s master SP set to %.2f",
            self.master.name, setpoint
        )

    def set_cascade_active(self, active: bool) -> None:
        """
        Enable or disable cascade mode.

        BUMPLESS TRANSFER
        -----------------

        When entering cascade: Slave switches to CASCADE mode
        When exiting cascade: Slave switches to AUTO with current SP
        """
        if active != self._cascade_active:
            self._cascade_active = active
            if active:
                # Entering cascade - bumpless transfer
                self.slave.set_mode(ControlMode.CASCADE)
            else:
                # Exiting cascade - slave to auto with current SP
                self.slave.set_mode(ControlMode.AUTO)

            logger.info(
                "Cascade %s -> %s: cascade mode %s",
                self.master.name, self.slave.name,
                "enabled" if active else "disabled"
            )

    def set_ratio(self, ratio: float) -> None:
        """Set cascade ratio (SP_slave = ratio * output_master)."""
        self.ratio = ratio
        logger.info(
            "Cascade %s ratio set to %.2f",
            self.master.name, ratio
        )

    # =========================================================================
    # STATUS AND HISTORY
    # =========================================================================

    def get_status(self) -> Dict[str, Any]:
        """Get cascade controller status."""
        return {
            "master": self.master.get_status(),
            "slave": self.slave.get_status(),
            "cascade_active": self._cascade_active,
            "ratio": self.ratio,
            "master_setpoint": self._master_setpoint,
        }

    def get_history(self, limit: int = 100) -> List[CascadeOutput]:
        """Get calculation history."""
        return list(reversed(self._history[-limit:]))


# =============================================================================
# CASCADE COORDINATOR
# =============================================================================

class CascadeCoordinator:
    """
    Coordinator for multiple cascade control loops.

    Manages a system of cascade controllers with coordinated
    setpoint changes and mode transitions.

    Example:
        >>> coordinator = CascadeCoordinator()
        >>> coordinator.add_cascade("furnace_1", cascade_1)
        >>> coordinator.add_cascade("furnace_2", cascade_2)
        >>> coordinator.set_all_master_setpoints(500.0)
        >>> results = coordinator.calculate_all(pv_data)
    """

    def __init__(self, name: str = "CascadeCoordinator") -> None:
        """Initialize cascade coordinator."""
        self.name = name
        self._cascades: Dict[str, CascadeController] = {}
        self._calculation_history: List[Dict[str, CascadeOutput]] = []

        logger.info("CascadeCoordinator initialized: %s", name)

    def add_cascade(self, cascade_id: str, cascade: CascadeController) -> None:
        """Add a cascade controller."""
        self._cascades[cascade_id] = cascade
        logger.info(
            "Cascade added to coordinator: %s (%s -> %s)",
            cascade_id, cascade.master.name, cascade.slave.name
        )

    def remove_cascade(self, cascade_id: str) -> bool:
        """Remove a cascade controller."""
        if cascade_id in self._cascades:
            del self._cascades[cascade_id]
            return True
        return False

    def calculate_all(
        self,
        pv_data: Dict[str, Tuple[float, float]],
        dt_seconds: Optional[float] = None
    ) -> Dict[str, CascadeOutput]:
        """
        Calculate all cascades.

        Args:
            pv_data: Dict of cascade_id -> (master_pv, slave_pv)
            dt_seconds: Time step

        Returns:
            Dict of cascade_id -> CascadeOutput
        """
        results = {}

        for cascade_id, cascade in self._cascades.items():
            if cascade_id in pv_data:
                master_pv, slave_pv = pv_data[cascade_id]
                results[cascade_id] = cascade.calculate(
                    master_pv=master_pv,
                    slave_pv=slave_pv,
                    dt_seconds=dt_seconds
                )

        self._calculation_history.append(results)
        if len(self._calculation_history) > 1000:
            self._calculation_history = self._calculation_history[-500:]

        return results

    def set_all_master_setpoints(self, setpoint: float) -> None:
        """Set master setpoint for all cascades."""
        for cascade in self._cascades.values():
            cascade.set_master_setpoint(setpoint)

    def set_all_cascade_active(self, active: bool) -> None:
        """Enable/disable cascade mode for all."""
        for cascade in self._cascades.values():
            cascade.set_cascade_active(active)

    def get_all_status(self) -> Dict[str, Dict[str, Any]]:
        """Get status of all cascades."""
        return {
            cascade_id: cascade.get_status()
            for cascade_id, cascade in self._cascades.items()
        }


# =============================================================================
# FACTORY FUNCTIONS
# =============================================================================

def create_temperature_flow_cascade(
    tag_prefix: str,
    temp_sp_default: float = 500.0,
    flow_max: float = 100.0
) -> CascadeController:
    """
    Factory function to create temperature-to-flow cascade.

    TUNING RATIONALE
    ----------------

    Master (Temperature):
    - K_p = 2.0: Moderate gain for temperature
    - K_i = 0.05: Slow integral (T_i = 40s) to avoid overshoot
    - K_d = 1.0: Some derivative for disturbance rejection
    - Sample time = 10s: Slow outer loop
    - Rate limit = 5%/s: Prevent thermal shock

    Slave (Flow):
    - K_p = 1.0: Moderate gain for flow
    - K_i = 0.5: Faster integral (T_i = 2s) for tight control
    - K_d = 0.0: No derivative (flow is noisy)
    - Sample time = 1s: Fast inner loop

    Args:
        tag_prefix: Tag prefix for controllers (e.g., "FZ-101")
        temp_sp_default: Default temperature setpoint
        flow_max: Maximum flow setpoint

    Returns:
        Configured CascadeController
    """
    # Master: Temperature controller (slow, outer loop)
    master = PIDController(
        name=f"TIC-{tag_prefix}",
        tuning=PIDTuning(kp=2.0, ki=0.05, kd=1.0),
        output_min=0.0,
        output_max=100.0,
        action=ControlAction.REVERSE,
        sample_time_seconds=10.0,  # Slow loop
        rate_limit_per_second=5.0,  # 5%/sec max
    )

    # Slave: Flow controller (fast, inner loop)
    slave = PIDController(
        name=f"FIC-{tag_prefix}",
        tuning=PIDTuning(kp=1.0, ki=0.5, kd=0.0),
        output_min=0.0,
        output_max=100.0,
        action=ControlAction.REVERSE,
        sample_time_seconds=1.0,  # Fast loop
    )

    cascade = CascadeController(
        master=master,
        slave=slave,
        slave_sp_min=0.0,
        slave_sp_max=flow_max,
    )

    cascade.set_master_setpoint(temp_sp_default)

    return cascade


def create_pressure_flow_cascade(
    tag_prefix: str,
    pressure_sp_default: float = 150.0,
    flow_max: float = 100.0
) -> CascadeController:
    """
    Factory function to create pressure-to-flow cascade.

    TUNING RATIONALE
    ----------------

    Master (Pressure):
    - K_p = 1.5: Moderate gain
    - K_i = 0.1: Medium integral (T_i = 15s)
    - K_d = 0.3: Light derivative
    - Sample time = 5s: Medium outer loop

    Slave (Flow):
    - K_p = 1.0: Standard gain
    - K_i = 0.8: Fast integral (T_i = 1.25s)
    - K_d = 0.0: No derivative
    - Sample time = 1s: Fast inner loop

    Args:
        tag_prefix: Tag prefix for controllers
        pressure_sp_default: Default pressure setpoint (psig)
        flow_max: Maximum flow setpoint

    Returns:
        Configured CascadeController
    """
    # Master: Pressure controller
    master = PIDController(
        name=f"PIC-{tag_prefix}",
        tuning=PIDTuning(kp=1.5, ki=0.1, kd=0.3),
        output_min=0.0,
        output_max=100.0,
        action=ControlAction.REVERSE,
        sample_time_seconds=5.0,
    )

    # Slave: Flow controller
    slave = PIDController(
        name=f"FIC-{tag_prefix}",
        tuning=PIDTuning(kp=1.0, ki=0.8, kd=0.0),
        output_min=0.0,
        output_max=100.0,
        action=ControlAction.REVERSE,
        sample_time_seconds=1.0,
    )

    cascade = CascadeController(
        master=master,
        slave=slave,
        slave_sp_min=0.0,
        slave_sp_max=flow_max,
    )

    cascade.set_master_setpoint(pressure_sp_default)

    return cascade
