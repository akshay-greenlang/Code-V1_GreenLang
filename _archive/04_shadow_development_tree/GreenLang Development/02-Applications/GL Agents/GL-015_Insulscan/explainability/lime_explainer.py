# -*- coding: utf-8 -*-
"""
GL-015 Insulscan - LIME Explainer for Insulation Assessment

Local Interpretable Model-agnostic Explanations for insulation condition
score predictions. Provides local surrogate model explanations and
counterfactual analysis with deterministic seed management.
...
- Seed management ensures reproducible explanations
...
class SeedManager:
    """
    Manages random seeds for deterministic LIME explanations.

    Ensures reproducibility by deriving seeds from asset IDs
    and maintaining consistent random state across runs.
    """
    ...
    def get_seed_for_asset(self, asset_id: str) -> int:
        """Get deterministic seed for a specific asset."""
        if asset_id in self._seed_cache:
            return self._seed_cache[asset_id]

        # Derive seed from asset_id hash
        hash_val = int(hashlib.sha256(asset_id.encode()).hexdigest()[:8], 16)
        seed = (self.base_seed + hash_val) % (2**31 - 1)
        self._seed_cache[asset_id] = seed
        return seed

    def reset_random_state(self, seed: int) -> None:
        """Reset numpy random state with given seed."""
        np.random.seed(seed)


class InsulationLIMEExplainer:
    """
    LIME-based explainer for insulation condition score predictions.
    ...
    Features:
    - Local interpretable explanations for condition scores
    - Feature perturbation specific to insulation parameters
    - Visual explanation generation for operators
    - Deterministic with seed management
    - Human-readable explanations for operators

    Example:
        >>> config = LIMEConfig(num_features=10)
        >>> explainer = InsulationLIMEExplainer(config)
    ...
    def __init__(self, config: Optional[LIMEConfig] = None) -> None:
        ...
        logger.info(
            f"InsulationLIMEExplainer initialized with config: "
    ...
    def explain_prediction(
        ...
        # Set deterministic seed for this asset
        seed = self.seed_manager.get_seed_for_asset(asset_id)
        self.seed_manager.reset_random_state(seed)
        ...
        lime_result = self._compute_lime_explanation(
            features, feature_names, predict_fn, prediction_type, asset_id
        )
        ...
        stability_metrics = self._assess_stability(
            model, features, feature_names, lime_result, asset_id
        )
        ...
        logger.info(
            f"LIME explanation generated for asset {asset_id} "
    ...
    def generate_counterfactual(
        ...
        prediction value (e.g., better condition score).

        For insulation: "What if insulation was thicker?"
        ...
        return CounterfactualExplanation(
            ...
            cost_estimate=self._estimate_intervention_cost(feature_changes, feature_names),
    ...
    def generate_visual_explanation(
        ...
        """
        Generate visual explanation data for operators.

        Args:
            lime_result: LIME analysis result
            prediction_type: Type of prediction explained

        Returns:
            Dictionary with visualization data for dashboards
        """
        ...
        for name, weight in sorted_features:
            ...
            bar_chart_data.append({
                ...
                "direction": "worsens" if weight > 0 else "improves",
                "category": category.value,
                "color": "#dc3545" if weight > 0 else "#28a745",
            })

        # Build waterfall chart data
        waterfall_data = [
            {"feature": "Base Score", "value": lime_result.intercept}
        ]
        cumulative = lime_result.intercept
        for name, weight in sorted_features:
            cumulative += weight
        ...
        # Build natural language explanation for operators
        operator_explanation = self._generate_operator_explanation(
            sorted_features, prediction_type, lime_result.local_prediction
        )
        ...
        return {
            ...
            "prediction": lime_result.local_prediction,
            "prediction_type": prediction_type.value,
            "timestamp": lime_result.timestamp.isoformat(),
        }
    ...
    def explain_surrogate_model(
        ...
        # Compute LIME explanation
        lime_result = self._compute_lime_explanation(
            features, feature_names, predict_fn, PredictionType.CONDITION_SCORE, asset_id
        )
        ...
        equation = f"score(x) = {lime_result.intercept:.4f} " + " ".join(equation_terms)
    ...
    def _compute_with_lime_library(
        ...
        for feat, weight in explanation.as_list():
            ...
            explanation_text.append(
                f"{feat}: {direction} condition score by {abs(weight):.4f}"
            )
    ...
    def _compute_weighted_regression(
        ...
        # Generate perturbed samples with insulation-specific perturbations
        ...
        for i in indices:
            ...
            direction = "worsens" if weight > 0 else "improves"
            explanation_text.append(
                f"{name}: {direction} condition score by {abs(weight):.4f}"
            )
    ...
    def _generate_training_data(self, reference: np.ndarray) -> np.ndarray:
        ...
        # Generate variations with insulation-specific constraints
    ...
    def _find_counterfactual(
        ...
        # Grid search over changeable features
        for _ in range(self.config.counterfactual_steps):
            ...
            for idx in changeable_indices:
                ...
                # Apply insulation-specific perturbations
                name = feature_names[idx]
                if name in ['thickness', 'r_value', 'thermal_resistance']:
                    # Increasing these improves insulation
                    perturbation = direction * np.random.uniform(0.05, 0.2) * (np.abs(features[idx]) + 1e-10)
                elif name in ['moisture_content', 'gap_count', 'gap_area']:
                    # Decreasing these improves insulation
                    perturbation = -direction * np.random.uniform(0.05, 0.2) * (np.abs(features[idx]) + 1e-10)
                else:
    ...
    def _assess_feasibility(
        ...
        for name, (original, new) in feature_changes.items():
            ...
            # Physical features are easier to change through maintenance
            category = self._get_feature_category(name)
            if category == FeatureCategory.PHYSICAL:
                score *= 1.1
            elif category == FeatureCategory.MOISTURE:
                score *= 0.9  # Moisture can be addressed
            elif category == FeatureCategory.AGE_RELATED:
                score *= 0.3  # Age cannot be changed
    ...
    def _estimate_intervention_cost(
        ...
        """Estimate cost of intervention based on feature changes."""
        if not feature_changes:
            return None

        # Simple cost model based on typical intervention costs
        cost_factors = {
            "thickness": 50.0,  # $/unit increase per m2
            "gap_count": 25.0,  # $ per gap to fill
            "moisture_content": 100.0,  # $ per % reduction
            "jacket_condition": 75.0,  # $ per point improvement
            "coverage_percentage": 30.0,  # $ per % increase
        }

        total_cost = 0.0
        for name, (original, new) in feature_changes.items():
            change = abs(new - original)
            unit_cost = cost_factors.get(name, 20.0)
    ...
    def _generate_counterfactual_text(
        ...
        if not feature_changes:
            return "No feasible changes found to achieve the target condition score."
        ...
        for name, (original, new) in feature_changes.items():
            ...
            # Use operator-friendly descriptions
            if name == "thickness":
                change_descriptions.append(
                    f"{direction} insulation thickness from {original:.2f}mm to {new:.2f}mm"
                )
            elif name == "moisture_content":
                change_descriptions.append(
                    f"{direction} moisture content from {original:.1f}% to {new:.1f}%"
                )
            elif name == "gap_count":
                change_descriptions.append(
                    f"{direction} number of gaps from {int(original)} to {int(new)}"
                )
            else:
                change_descriptions.append(
        ...
        improvement = "improvement" if prediction_change < 0 else "decline"

        return (
            f"To achieve a better condition score: {changes_text}. "
            f"Expected new condition score: {counterfactual_prediction:.2f} "
            f"(an {improvement} of {abs(prediction_change):.2f} points)."
        )

    def _generate_operator_explanation(
        self,
        sorted_features: List[Tuple[str, float]],
        prediction_type: PredictionType,
        prediction_value: float,
    ) -> str:
        """Generate natural language explanation for plant operators."""
        explanation_parts = []

        # Opening statement
        if prediction_type == PredictionType.CONDITION_SCORE:
            ...
            explanation_parts.append(
                f"The insulation is in {status} condition with a score of {prediction_value:.1f}/100."
            )

        # Top contributors
        worsening = [(n, w) for n, w in sorted_features if w > 0][:3]
        improving = [(n, w) for n, w in sorted_features if w < 0][:3]

        if worsening:
            factors = [self._get_operator_friendly_name(n) for n, _ in worsening]
            explanation_parts.append(
                f"Key factors reducing the score: {', '.join(factors)}."
            )

        if improving:
            factors = [self._get_operator_friendly_name(n) for n, _ in improving]
            explanation_parts.append(
                f"Factors maintaining performance: {', '.join(factors)}."
            )

        return " ".join(explanation_parts)

    def _get_operator_friendly_name(self, feature_name: str) -> str:
        """Convert feature name to operator-friendly description."""
        friendly_names = {
            "surface_temperature_delta": "surface temperature difference",
            "moisture_content": "moisture level",
            "installation_age_years": "insulation age",
            "thickness_ratio": "remaining thickness",
            "gap_count": "number of gaps",
            "hot_spot_count": "thermal hot spots",
            "uv_exposure_hours": "UV exposure",
            "compression_ratio": "compression level",
            "jacket_condition": "jacket condition",
            "visible_damage_score": "visible damage",
        }
        return friendly_names.get(feature_name, feature_name.replace("_", " "))
...
def explain_insulation_with_lime(
    model: Any,
    features: np.ndarray,
    feature_names: List[str],
    ...
    Args:
        model: Trained condition prediction model
        features: Feature values for the instance
        feature_names: Names of features
...
def generate_insulation_counterfactual(
    model: Any,
    features: np.ndarray,
    feature_names: List[str],
    target_prediction: float,
