"""
GL-017 CONDENSYNC - Calculation Provenance Module

This module provides SHA-256 hashing and audit trail capabilities for
zero-hallucination, deterministic calculations in condenser optimization.
...
- ASME PTC 12.2 - Steam Condensers Performance Test Code
- HEI Standards for Surface Condensers
...
class ProvenanceTracker:
    ...
    Example:
        >>> tracker = ProvenanceTracker("HeatTransferCalculator", "1.0.0")
        >>> tracker.set_inputs({"temp_in": 25.0, "temp_out": 35.0})
        >>> tracker.add_step(1, "Calculate delta T", "subtract",
        ...                  {"a": 35.0, "b": 25.0}, 10.0, "delta_t")
        >>> tracker.set_outputs({"delta_t": 10.0})
...
def compute_input_fingerprint(inputs: Dict[str, Any]) -> str:
    ...
    Example:
        >>> fingerprint = compute_input_fingerprint({"temp": 25.0})
