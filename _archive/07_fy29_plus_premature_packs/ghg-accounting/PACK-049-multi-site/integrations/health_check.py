# -*- coding: utf-8 -*-
"""
HealthCheck - 20-Category Health Verification for PACK-049
=============================================================
...
site registry, and performance for the GHG Multi-Site Management Pack.

Categories (20):
    1.  Site Registry engine available
    2.  Data Collection engine available
    3.  Site Boundary engine available
    4.  Regional Factor engine available
    5.  Site Consolidation engine available
    6.  Site Allocation engine available
    7.  Site Comparison engine available
    8.  Site Completion engine available
    9.  Site Quality engine available
    10. Multi-Site Reporting engine available
    ...
    17. Seed data present (facility types, allocation methods, quality
        dimensions)
    18. Config valid
    19. Site registry accessible
    20. Performance acceptable

Health Score:
    0-100 scale computed as (healthy * 5 + degraded * 2.5) / total * 100
...
class HealthCheckCategory(str, Enum):
    ...
    # Engines (10)
    ENGINE_SITE_REGISTRY = "engine_site_registry"
    ENGINE_DATA_COLLECTION = "engine_data_collection"
    ENGINE_SITE_BOUNDARY = "engine_site_boundary"
    ENGINE_REGIONAL_FACTOR = "engine_regional_factor"
    ENGINE_SITE_CONSOLIDATION = "engine_site_consolidation"
    ENGINE_SITE_ALLOCATION = "engine_site_allocation"
    ENGINE_SITE_COMPARISON = "engine_site_comparison"
    ENGINE_SITE_COMPLETION = "engine_site_completion"
    ENGINE_SITE_QUALITY = "engine_site_quality"
    ENGINE_MULTI_SITE_REPORTING = "engine_multi_site_reporting"
    # Bridges (4)
...
class SystemHealth(BaseModel):
    ...
    pack_id: str = "PACK-049"
    pack_name: str = "GHG Multi-Site Management"
...
class HealthCheck:
    """
    20-category health verification for PACK-049 GHG Multi-Site Management.

    Performs comprehensive health checks across engines, bridges, and
    infrastructure to verify system readiness for multi-site consolidation
    operations.
    ...
    async def run_critical_checks(self) -> SystemHealth:
        """Run only critical health checks (engines and database).

        Returns:
            SystemHealth with critical component status only.
        """
        critical_cats = [
            c for c in self.config.categories
            if c.value.startswith("engine_") or c in (
                HealthCheckCategory.DATABASE,
                HealthCheckCategory.MIGRATIONS,
            )
        ]
        original_cats = self.config.categories
        self.config.categories = critical_cats
        result = await self.check_all()
        self.config.categories = original_cats
        return result
