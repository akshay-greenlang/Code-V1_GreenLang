# -*- coding: utf-8 -*-
"""
PACK-036 Utility Analysis Pack - Init Tests (test_init.py)
=============================================================
...
def _load_init(subdir_path: Path, mod_name: str):
    ...
    mod_key = f"pack036_init_test.{mod_name}"
...
class TestRootInit:
    ...
    def test_root_init_pack_name(self):
        ...
        assert "036" in str(pack_name) or "Utility" in str(pack_name)
...
class TestEnginesInit:
    ...
    def test_engines_init_imports(self):
        mod = _load_init(ENGINES_DIR, "engines")
        has_parser = hasattr(mod, "UtilityBillParserEngine")
        has_rate = hasattr(mod, "RateStructureAnalyzerEngine")
        has_demand = hasattr(mod, "DemandAnalysisEngine")
        has_benchmark = hasattr(mod, "UtilityBenchmarkEngine")
        assert has_parser or has_rate or has_demand or has_benchmark
...
class TestConfigInit:
    ...
    def test_config_init_exists(self):
        init_path = PACK_ROOT / "config" / "__init__.py"
        if not init_path.exists():
...
class TestPresetsInit:
    ...
    def test_presets_init_exists(self):
        init_path = PACK_ROOT / "config" / "presets" / "__init__.py"
        if not init_path.exists():
